...
- All rules in this title must be applied to both:
...
- For comments within the source code:

  - `` ` `` character must not be used. 
  - `RULE 4` must be ignored.

  Because programming language on that source code does not support Markdown language special characters for the same purpose.

### 📌📌 RULE 1

`Reason for this rule`: It will be more human-readable.

When we write any term which:

- is special term
- is special name
- is a technical term except below and their forks or their abbreviations:
  - database
  - server
  - token
  - request
  ...
  - IDE
  - class
- has been mentioned on any level title of same document

it must inside `` ` ``. Examples:
...
`Reason for this rule`: Spell-checker knows that this is a code.
...
```markdown
Send request from network to "`myDomain.com`".

Send request from network to "`ABC+ Service`".
...
`Reason for this rule`: Is not valid on any language.

`'` or `"` must not be used side by side. Example:

Don't write this:

```markdown
"`XYZ+ Service`"'s port is 8081.
```

Below one must be used (the sentence should be changed):

```markdown
Port of "`XYZ+ Service`" is 8081.
```
...
- `Bold` words must be defined with double underscore character. (Even both `CommonMark` and `GFM` supports using double star as well)
  ...
  `Reason for this rule`: In the future I may search and replace texts.

- `Italic` should not be used. 

  `Reason for this rule`: Italic font cannot be easily distinguished on most Markdown-viewer.

### 📌📌 RULE 5

This rules applies for the terms which mentioned on `Rule 1`.

Some terms has abbreviations or it may have synonyms. But we must used only one on whole repository.

`Reason for this rule`: If the user (who reads the tutorials) selects the term (generally via double-click)

  - he can search that term quickly where else it used
  - viewer (software) highlights the term automatically if it is used elsewhere on the same document.

### 📌📌 RULE 6

This rule applies for the terms which mentioned on `Rule 5`.

If an abbreviation exist then it must be used.
...
If abbreviation not exist then the shortest synonym must be used.

If a synonym or abbreviation exist it must mention only once on whole repository. This explained in `TITLE FORMAT`.
...
Some terms can be used via dash or white space like:

- Spring boot
- Spring-boot

For those cases only 1 style of that term must be used on whole repository.

`Reason for this rule`: Same reasons with `Rule 5`.

## 📌 FILE SIZE

Each file must not be greater than 400 KB.
...
- Source code or any other text output must saved also in multiple formats locally. Because of below reasons:
...
Do not export/convert files as PDF (A4) or any landscape file format.

`Reason for this rule`: The large tables does not fit on the page. Prefer `HTML` which supports landscape tables.

## 📌 TITLE ICON AND LEVELS

Markdown rendered most frequently to `HTML`. `HTML` accepts only one `H1` item. Therefore only one `H1` title must be use on a file.
