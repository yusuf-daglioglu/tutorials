...
- `Univariate time series data (⟷ one-dimensional time series data)`
...
- `Multivariate time series data (⟷ multi-dimensional time series data)`

  `Multivariate` kelime anlamı: Çokdeğişkenli.

  örnek data: saatlik `Londra`'daki sıcaklık derecesi ve nem oranı. time değerinin yanında sadece 2 değer var.
