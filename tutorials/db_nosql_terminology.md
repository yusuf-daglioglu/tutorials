...
diğer tüm `DB`'leri karşılaştırarak gidersek aşağıdaki temel farkları görürüz:

- `Graph DB`'lerde; bir entity'nin ilişkili olduğu entity'lerin fiziksel erişim adresleri, entity'nin kendisi ile aynı fiziksel alandadır. böylece join'li query'ler çok hızlı çalışmaktadır.
...
- veri fiziksel olarak zaman eksenine (tarih sırasına) göre tutulur.
- sorgular çoğunlukla "son 1 saat" şeklindedir.
- append only için optimize edilmiştir.
- time-based partitioning destekler.
- time-based retention (eski data'nın silinmesi) destekler.
- data daha ufak yer kaplar ve sıkıştırma oranı daha yüksektir. örnek:

  `Time series` olmayan `DB`'deki veri:

  ```
  date                  cpu-usage
  2020-02-01 12:00      100000
  2020-02-01 12:01      100001
  2020-02-01 12:03      100002
  ```

  `Time series`'deki `DB`'deki veri:
  ...
  2 avantaj var:
  - data ufak yer kaplar çünkü delta bilgisi tutulabilir.
  - data daha iyi sıkıştırılır, çünkü delta bilgisinde sıkça aynı data dörülebilir (yukarıdaki örnekte `+1` gibi)
...
- `OTEL` trace datası `graph store`'da tutmak mantıklı.