...
- __HashSet__
  - Implements "`Set`".
  - It use `hash-map` under the hood. Stores the each element inside this `hash-map`.
...
- In some cryptocurrencies, the money transfers (transactions) are prioritizing depending on their fee.
