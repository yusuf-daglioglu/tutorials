...
`regex`'e alternatiftir ama çok daha basit bir yapıdır.

`POSIX` standartlarında glob tanımı vardır fakat çoğu yazılım buna uymaz.

## 📌 Grok

`regex`'e alternatiftir ama daha okunaklıdır.

`grok`'un resmi bir standartı yoktur.

https://github.com/thekrakken/java-grok `Java` library'sinden örnek kullanım:
...
Yukarıda `COMBINEDAPACHELOG` bir `Grok pattern`'idir. Resmi standartı yok bu sebeple kütüphaneye kalmış neyi kapsayacağı.

`Map` tipindeki `capture` şu artık şu bilgileri içeriyor olacak:

```properties
clientip=112.169.19.192
ident=-
auth=-
timestamp=06/Mar/2013:01:36:30 +0900
```

## 📌 Grok syntax

`grok`, `regex` syntax'ı kullanır. ek olarak yukarıdaki örnekteki gibi, `COMBINEDAPACHELOG` gibi `regex` dışında keyword'leri vardır. Fakat bunları hiç kullanmazsak `regex` syntax'ı ile uyumlu bir syntax ortaya çıkar.

`Grok` en sıkça `logstash` tarafından kullanılır. `Logstash`'in kendi yazılımında tanımladığı `grok pattern`'lere bakarsak hepsinin %100 `regex` ile uyumlu olduğunu görürürüz:

https://raw.githubusercontent.com/logstash-plugins/logstash-patterns-core/refs/heads/main/patterns/ecs-v1/grok-patterns
...
- `Perl Compatible Regular Expressions (⟷ PCRE)` `kaynak: https://www.pcre.org/ 1inci paragraf`

Her `regex` motorunun, yukarıdaki specification'lara ekstra farklı syntax (formatı) özellikleri de vardır. Bazı farklılıkların listesi buradadır: http://www.greenend.org.uk/rjk/tech/regexp.html
...
| \w                                    | wordly character. yani 1 karakteri temsil eder. Non-Latin letters (like cyrillic ⟷ hindi) do not belong to \w . \W ise bunun tersidir (yani non-wordly character)                                                                                                                   |
...
public class RegexOrnek {
    ...
    public static void main(String[] args) {
        ...
        String metin = "Ali 25 yasinda ve Ayse 30 yasinda.";

        // Sayıları bulmak için regex.
        // \\d+ bir veya daha fazla rakam bulmak için gerekli regex.
        Pattern pattern = Pattern.compile("\\d+");
        Matcher matcher = pattern.matcher(metin);

        int sayac = 0;

        // Kaç tane olduğunu bilmiyoruz, o yüzden find() ile döngü kurduk.
        while (matcher.find()) {
            sayac++;
            System.out.println("Bulunan " + sayac + ". sonuc: " + matcher.group());
        }

        System.out.println("Toplam bulunan sonuc sayisi: " + sayac);
    }
}
```
...
Returns the `string` value of the `regex`.
