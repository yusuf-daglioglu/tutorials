...
- "`http://localhost:8080/actuator/metrics`"
  ...
  current thread count, current memory usage, system update, all Java classes
...
- "`http://localhost:8080/actuator/env`"
  ...
  all environment variables (her prop "app.yml", "bootstrap yml", Kubernetes'ten mi gelip gelmediği detayı ile birlikte yazıyor)
...
- birçok kaynaktan verileri çekerek, bunları kendisinin sunduğu web arayüzünden visualize eden bir yazılımdır. Topladığı data'ları anlık olarak gösterir. Anlık olarak data'ları çektiği için kendi veritabanı yoktur. Grafana herhangi bir veriyi toplayabilir. örnekler:
...
`Grafana` özellikle time-series data yönetimini çok iyi yaptığından dolayı `Prometheus` gibi `data-source`'lardan ve __time series DB (or TSDB)__ konularından çok bahsedilir.
...
`Prometheus` bağımsız bir servis olarak ayağa kalkar. önceden her servisin health (örneğin Spring'deki `Actuator` endpoint'leri) URL'si `Prometheus`'a tanıtılır. `Prometheus` bu URL'lere giderek düzenli olarak data çeker ve kendi database'sine kaydeder.
