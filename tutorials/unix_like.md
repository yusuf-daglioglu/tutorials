...
- `Fedora` (`rpm` based)
  ...
  - `Azure Linux` (`Microsoft`'un bulut platformlarında sunduğu general purpose `OS`)

  - `Mandrake`

  - `Fedora CoreOS (⟷ FCOS)` (`Container OS`)
    
    - `Red Hat Enterprise Linux CoreOS (⟷ RHCOS)` (`Container OS`)
...
- `Gentoo` (package manager: `Portage`) `arch` gibi sistemin paketlerine kadar kullanıcının kendisi kuruyor. arch'a göre daha fazla teknik bilgi gerektiriyor.
  ...
  - `Chrome OS` and `Chromium OS`
    ...
    - `Container-Optimized OS` (by `Google`) (`Container OS`) (ismi çok genel ama özel isi olarak kullanılıyor)

  - `CoreOS Container Linux` (discontinued)

    - `Flatcar Container Linux` (`Container OS`)

      - `Azure Container Linux` (`Container OS` only for `K8s`)
...
- `immutable`:
  ...
  - `OS` `bootloader`'da parametre geçerek hangi layer'ı run edeceğimizi seçebiliyoruz.
...
- tarih (`sleep()` fonksiyonu gibi) API'leri nasıl davranacak
