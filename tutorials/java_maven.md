...
effective_pom.xml'imizi görmek istediğimizde bir tool (örnek IDE) bize pom.xml'imizden "effective pom.xml" otomatik generate eder.
...
her life cycles içinde birçok phase vardır. liste şu şekilde: (kaynak: (source-id: 419))
...
"pom.xml" IDE tarafından okunur ve ilgili işlemlerin koşması sağlanır. fakat Maven IDE'siz de çalıştırılabilir. Maven default command-line uygulaması mevcut. Bu IDE 'lerin içine de gömülebilir, OS'a de kurulabilir. Maven wrapper basit bir script'tir. bu script projenin root'unda "pom.xml"'in yanında bulunur. bu script OS'un komut satırında çalıştırıldığında Maven'ı OS path'inde arar, yoksa download eder ve Maven komutlarını çalıştırmamızı sağlar. portabilite kazandırır. IDE den bağımsızlığı getirir. pom.xml'e uygun Maven sürümünü bulmaya çalışır. her OS için farklı script dosyası olmalıdır, çünkü her OS'un kendi script dili vardır.
