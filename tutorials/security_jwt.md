...
- Access token sadece ilgili sekmeden erişilmesi isteniyor ise:
  ...
  - JavaScript closures with private variable'da.
    ...
    class JWTUtil {
        ...
        // not accesible via other JS classes.
...
- Access token tüm sekmelerden ve tarayıcı restartı sonrası erişilecek ise; local-storage'de tutulmalı.
...
Opsiyonel olarak bu yöntemde kullanılabilir. Aşağıdaki komple yöntem hem refresh hemde access token için ayrı ayrı uygulanabilir.

autantication sırasında back-end bir random UUID oluşturacak. bu UUID'nin SHA256-hash'sı alınıp JWT token'ın içine gömülecek.
...
Artık her back-end'e gelen istekte cookie'den gelen bu değer ile JWT token'larda bu değerler karşılaştırılacak. eğer birbirinden farklı ise, kullanıcı log'out edilecek. burada son kullanıcıya uyarı verilecek (ve hatta kullanıcı geçici bloklanabilir, çünkü oturumu hacklenmiştir).

## 📌 Ek önlem (domain kontrolü)

Token oluştururkne back-end bu token ile hangi URL'lere istek atabilecğeimizi de token içine yazabiliriz. Bu şekilde her web servisi JWT'yi açtığında bu servise yetkisi var mı diye kontrol edebilir.

## 📌 Ek önlem (JWT içindeki data)

token'daki expire-time gibi variable'ların hepsi validade edilmeli.

JWT refrehs ve access içinde kullanmadağımız hiçbir veri tutulmamalı. ihtiyaca göre eklenmeli. olası bir hacklenme durumunda karşı tarafa az bilgi alması daha iyi olacaktır.
