...
- norveç örneği
  ...
  ```java
  new Locale("no", "NO");       // Bokmal (norveç'te en sık kullanılan)
  new Locale("no", "NO", "NY"); // Nynorsk (norveç'te az kullanılan)
  ```

`Locale` sınıfı sadece dil bilgisi tutmaz, bazen variant bilgisi sadece takvim değişikliğine sebep olabilir. Aslında burada neyin değişeceği, variant değişkenini kullanan kütüphanenin kendisine kalmıştır.
...
standarttır. farklı varyasyonları yok. ilk çıkarılan metin zamanlarında farklı varyasyonlar olabilir ama onlar neredeyse hiç kullanılmadı.

Güncel zamanlarda dijital ortamlarda kullanım alanları:

- Kitapların başlangıçlarındaki sunuş ve ön söz gibi bölümlerdeki başlıkların numaralandırılmasında.
- kitapların sayfalandırılmasında
- dizi ve oyun gibi seri yayımlanan isimlerde: Rocky II, Grand Theft Auto V gibi.
- Yüzyıl adlarında: XIX. yüzyıl gibi.
- Aynı adlı padişah ve kralların ayrılmasında: II. Ahmet, XVI. Louis gibi.
- Tarihi olaylarda: II. Viyana Kuşatması, I. Dünya Savaşı gibi. (burada local'izasyon gerekmez. fakat roma rakamları static string olarak tutulur fakat yine de not olarak belirttim)
...
Kurallar:
- Soldaki küçükse, çıkar.
- Soldaki büyükse, topla.
- Her küçük, her büyükten çıkarılamaz. Sadece şunlar çıkarılabilir:
...
- XIV = 14
  ...
  2 kural var, 2 kuralın sonucu toplanıyor:
...
- XXIV = 24
  ...
  3 kural var, 3 kuralın sonucu toplanıyor:
  
  - X = 10
  - X = 10
  - IV = 5-1 = 4

Roma sayılarında aynı sayı farklı ifadelerle de gösterilebilir (denk gelmektedir). örneğin aşağıdaki iki ifade de 8 sayısına denk geliyor:

- `VIII`
- `IIX`

karakterler üzerlerine düz çizgi de alabilirler (veya parantez içine yazılabilirler - bu farklı bir yazım stili sadece). bu durumda o sayının 1000 katı temsil edilir.
...
Özellikle programatik olarak sıralama istendiğinde çok zor durumlarla karşılaşılmaktadır.
...
Sadece metni çevirmek yetmiyor, sayfa boyutu (A4 gibi) bile ülkeden ülkeye değişebiliyor. Bu konunun detayları için buraya bakılmalı: [file:"unix_like_commands.md" - title:"locale"](./unix_like_commands.md)
