...
Spring varsayılan olarak Authentication tarafı için birçok implementasyonu destekliyor: HTTP Basic, HTTP digest, LDAP... Bunların içinde JWT(JSON Web Token) yok. JWT için implementasyonu yazılımcının kendisi yapması gerekiyor. Zaten JWT bir security metodu değildir, JWT data transferi yöntemidir.
...
Main sınıfımıza @EnableGlobalMethodSecurity(securedEnabled = true) eklenmelidir. Daha sonra herhangi bir metodun yetkisini kısıtlamak istediğimizde o metodun tepesine annotation atarız. örnek:
