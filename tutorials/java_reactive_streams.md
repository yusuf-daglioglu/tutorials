...
- bazı listener'larda hatayı yakalamak için try/catch mekanizması gereklidir. fakat reaktifte hatalar onError metotları ile yakalanır.
...
not: reaktif manifesto'da reactive programlama'daki data değişimine tepki gösterme zorunluluğu maddesini içermemektedir.
...
Java 9 ile gelen reactive API'sinin ismi "Reactive Streams" olarak geçer. __java.util.concurrent.Flow.*__ paketi altındaki sınıfları içeriyor.
...
Spring 5.0 ile birlikte, __spring-webflux__ paketi ile Spring MVC'ye Reactor ile reaktif programlama desteği getirmiştir.
