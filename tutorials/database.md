...
__CouchOne (or old-name:CouchIO)__, "__Apache CouchDB__" yazılımını geliştiren şirketin ismidir.
...
Oracle içerisinde gelen bir tablodur. Tablonun içerisinde sadece "DUMMY" isimli, "char" tipinde bir sütun vardır. İçerisinde de sadece 1 kayıt vardır. Onun içeriği de `X` string'i vardır.
...
Bu çözümlerden biri de 'Data Guard'dır. Bu çözümde "Primary database" e hem yazılıp hem okunabiliyor. Fakat farklı data-center'larda olan birden çok Oracle instance'ı "Standby database" olarak çalışıyor. Primary'ya yapılan her işlem otomatik olarak her standby'a da yollanıyor. Standby sürekli olarak sadece read-only hizmet verebiliyor. Fakat eğer primary çökerse, bir standby primary olarak çalışmaya başlıyor.
...
RDBMS'ler "Immediate Consistency" sunarlar. fakat ayarlanır ise; bazı NoSQL veritabanları da "Immediate Consistency" sunabilir.
...
<databaseChangeLog xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    bu tabloda hangi changeSet'lerin işlendiği, author'un kim olduğu, hangi date-time'da database'ye işlendiği
...
Database tarafında __CLOB (or Character large object)__ olarak kullanılırken, __BLOB__ binary objeler tutar.
...
Her veritabanı birçok farklı data type destekliyor. Örneğin; "JSON" tipi destekleniyor olabilir. Böyle durumlarda JSON'un validasyonu da veritabanı tarafında da yapılabilir. JSON manipülasyonları için özel SQL fonksiyonları da sunuluyor olabilir...
...
veritabanı dünyasında obje; tablo, view, index'lerin (ve daha fazlası fakat her şey değil!) grup ismidir. bu durumda; örneğin USER tablosu bir database instance'sidir.
...
Fakat natural ordering ihtiyacı yok ise, ve sadece hızlı read amaçlı index'leme istiyorsak, bazı database'ler byte bazında index'leme yapabiliyor (bunun için "COLLATE BINARY" konusu internette aratılabilir).
...
__key-value DB__'leri, sadece bir key'e karşılık herhangi bir tipte veri tutarlar. Fakat __data structure__ database'leri key-value ile kısıtlı değillerdir. list, set, array gibi daha karmaşık veri yapılarını da destekler. dolayısı ile __data structure__ database'leri aynı zamanda key-value veri yapısını da desteklemiş olur. kaynak: (source-id: 97) "What is Redis?" 2inci ve 3üncü paragraf.
...
- Her Java mikroservisimiz kendi içinde API response'larını cache'leyebilir. Aynı parametrelerle istek atıldığında aynı cevabı döner.
...
- multi-cluster high availability (async or sync backup/replication)
...
- Database table normalization (1NF, 2NF)
