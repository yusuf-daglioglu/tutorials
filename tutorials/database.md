...
(
  ...
  -- scale:2
  -- max value: 999,99
  -- 5-2=3 basamak var tamsayı kısmında.
...
- __Consistency (or Tutarlılık)__
  ...
  "Consistency" terminini ucu açıktır. Çünkü neyin tutarlı olacağına göre değişir. Örneğin; ACID'deki Consistency, datanın primary-key, not-null gibi kurallara uyduğunu belirtmek için kullanılır. Oysa CAP teoremindeki Consistency, bilgisayar bilimlerinde "__linearizable (or doğrusallaştırılabilir)__" olarak kullanılır.
...
- Cassandra'yı 1 node ve 1 cluster ile kullanıp, sadece "lightweight transaction" ile yazma okuma işlemi yaparsak CP yönüne kaydığını görürürüz.
...
- __PA/EL__ system:
  ...
  Partition varsa, A da var (ama C yok). Eğer partitionsuz çalıştırırsak, L var ama C yok.
...
Vertical partitioning pratikte uygulama seviyesinde yapılır. Örneğin customer tablosundaki sütunları 2 farklı tabloya bölerek ve farklı tablolara dağıtarak bu işlemi yapabiliriz. Eğer tüm data'yı çekmek gerekirse 2 datayı ardarda çekmemiz gerekir.
...
Line 5 çalıştığında, DB'nin state'inin line 3 teki duruma gelmesini sağlar. Ama SQL kodu line 6'dan devam eder.
...
- Redis, Hazelcast gibi yazılımların instance'larını distributed data tutması için multi-cluster ayarlayabiliriz.
  ...
  Hazelcast "Map" yapısında bir veri tuttuğunda bu dataları kendi içinde partitioned olarak tutar. Bundan client'ın haberi yoktur. Hazelcast bunları aynı zamanda multi-instance/node üzerinde klonlayabilir(replica) vs...
...
DB'de number'lar çok daha hızlı sortlanır. sortlanınca da indexlenmesi daha hızlıdır ve index'te aramada daha hızlıdır.

fakat string'de arkaplanda binary olaraka tutuluyor. bu durumda o da aynı performansı göstermesini beklemezmiyiz?
...
Fakat natural ordering ihtiyacı yok ise, ve sadece hızlı read amaçlı indexleme istiyorsak, bazı database'ler byte bazında indexleme yapabiliyor (bunun için "COLLATE BINARY" konusu internette aratılabilir).

böylece string sıralama hızlanır ama yinde number sıralama gibi hızlı diildir. bunun bazı sebepleri:

- UTF-8: karakter'e göre 1-4 byte arsında her karakter uzayabilir (bu konu detaylı başka yerde anlatılıyor). Number gibi fixed-lenght olmadığı için, CPU calculation'ları daha yavaş.
- string'de natural order olmasa bile, byte bazında bir karşılaştırma ypılacaktır. 101010 olan bir sayı karşılaştırılmaz. byte karşılaştırma, string'e göre hızlıdır, ama number'a göre daha yavaştır. Çünkü bir "sayı", CPU'da tümüyle tek bir hamlede karşılaştırılıyor. Ynai 9876 sayısı CPU da 1 işlemde karşılaştırılabiliyor. Ama byte karşılaştırması, her byte için, CPU'da ayrı işleme tabi tutuluyor.
...
proxy gibi çalışmaz. hangi redis instance'ları ayakta diye, sentinel sürekli monitoring yapar ve bunu client'lara bildirir. client'lar sentinele sadece hangi redisler ayakta diye sorar.
...
ehcache'im dağıtık (distributed) yapısı yoktur. oysa Infinispan dağıtık yapısı vardır. eklentiler veya configurasyonlarla değiştirilebilir olsa da, en temel fark budur.
...
- Açık kaynaklı.
- Java ile yazılmış.
- in-memory NoSQL kategorisindedir. Fakat SQL (hem manipülasyon, hem query) atılması için de özel bir feature'si mevcut. Fakat arkada RDBMS tarzı tablolar tutmuyor.
...
## 📌 function vs stored procedure (or saklı yordam)
...
Note-2: No need for transaction because it can not manipulate the real database data.
...
- __B-Tree__ (binary tree'den farklıdır)
  ...
  B-tree dengelidir (flat). Bu sebeple aramalar hızlı olması sağlanır. Fakat ağacın dengesinin sağlanması gereklidir her inser sonrası, bu da ekleme/silmelerde yavaşlık getirir.

  String'ler de B-tree'de indexlenebilir (alfabetik sıra yapılır).

  Fakat JSON, Array gibi data tutan sütunlar B-tree'de indexlenemezler.
...
- __Full-Text__
  ...
  "Full-Text search" yapabilmek için gerekli indexlemedir. ("Full-Text search" başka yerde anlatılmıştır)
...
Veritabanı dünyasında, bir sütunun cardinality'sinin yüksek veya az olduğundan bahsedilir. tekrarlanan veri varsa cardinality düşüktür. tekrarlanan veri az ise cardinality yüksektir. örnek olarak "cinsiyet" sütununun kardinalitesi düşüktür.
