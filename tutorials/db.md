...
`CouchDB`, `IBM`'de çalışan bir developer tarafından geliştiriliyordu. Daha sonra bu developer, kodu açık kaynaklı olarak yayımladı ve `Apache vakfı` projeyi devam ettirdi ve `Apache CouchDB` ismi ile devam etti.

`CouchOne (⟷ old-name:CouchIO)`, `Apache CouchDB` yazılımını geliştiren şirketin ismidir.

`CouchOne` firması, `Membase (⟷ old-name: NorthScale)`  firması ile birleşerek `Couchbase` isimli şirket kuruldu.

`Membase` şirketi, `Membase` isimli product'ı geliştiriyordu. `Membase`'i geliştiren lead developer'lar `Memcached` projesinde de çalışmıştı. `Membase` ürünü, `Memcached` protocol'ünü kullanır.

`Couchbase` firması, `Membase` ve `Apache CouchDB` yazılımlarını birleştirerek `Couchbase Server` isimli yazılımı yarattı. Yeni yazılım `Membase`'in yeni versiyonu olarak tanıtıldı. `CouchDB` ile uyumluluk gibi bir durum söz konusu değildir.

Yıl 2022 itibariyle `Couchbase Server` ve `Apache CouchDB` olarak 2 ayrı proje birbirinden bağımsız şekilde geliştirilmektedir.

`Couchbase Server`'ın hem `Community` hem de `Enterprise` sürümü mevcut.
...
`Couchbase`'in kullandığı query dilidir.
...
Türkçe kaynakların bazılarında `başvuru çizelgesi` olarak kullanılıyor.
...
## 📌 scale (⟷ ölçek) and precision (⟷ hassasiyet)
...
`BSON`, `JSON`'ın desteklemediği date, binary-data gibi formatları da destekliyor. Bunlara `BSON type` deniliyor.
...
## 📌 High-availability clusters (⟷ HA clusters ⟷ failover clusters)
...
`Oracle DB` ürünü (default'ta) sadece tek başına tek 1 instance ve tek 1 cluster olarak çalışır. Diğer her feature ekstra çözüm uygulamak gerekir. Bunlar burada detaylı listelenmiştir: https://docs.oracle.com/cd/B28359_01/server.111/b28281/architectures.htm
...
birçok çeşit Consistency Model'i var. Bu modeller burada anlatılmıştır: https://res.mdpi.com/d_attachment/futureinternet/futureinternet-11-00043/article_deploy/futureinternet-11-00043.pdf Yazar: Miguel Diogo, Bruno Cabral, Jorge Bernardino, "Consistency Models of NoSQL Databases", "Figure 2.Consistency Models based on Reference" görseli.
...
Bu başlıktaki tüm bilgiler için kaynak: `https://riunet.upv.es/bitstream/handle/10251/54786/TFMLeticiaPascual.pdf Yazar: Leticia Pascual Miret, Master's Degree in Parallel and Distributed Computing`

### 📌📌 Passive replication model (⟷ Primary backup)

tüm client'lar sadece "primary server" denilen sunucuya istek atar. bu sunucu diğer replica'lara klonlama işlemini gerçekleştirir. sync yada async yapıp yapmayacağı implementasyona göre değişir. eğer sync backup işlemi yapılıyorsa; client'a geç cevap dönülür. bu model'e `pessimistic` adı verilir. Tersi durumda async backup alınır ve client'a hızlıca cevap dönülür. Bu model'e ise `optimistic (⟷ eager)` adı verilir.

### 📌📌 Active replication model

Passive'de tek bir yükümlü sunucu varken (primary server), burada birden fazla olmaktadır. Dolayısı ile, tutarlılığı sağlamak daha zordur. Fakat bu model'in `fault tolerance (⟷ hata töleransı)` vardır.
...
`partitioning` bir tablonun satırlarının veya sütunlarının ayrı fiziksel lokasyonlara ayırmaktır. Eğer partitioning işlemini farklı makinelere yapıyor isek, bu işlem `sharding` olur. kaynak: https://hazelcast.com/glossary/sharding/ title: "What Is the Difference between Sharding and Partitioning?".
...
Bu fiziksel lokasyonlar farklı DB instance'ı ise; `Horizontal Sharding` yapmış oluruz.

## 📌 Vertical Partitioning (⟷ Dikey Bölümleme)

Aynı tablonun sutunlarının farklı fiziksel yerlerde olma durumu. kaynak: https://subscription.packtpub.com/book/big-data-and-business-intelligence/9781788397186/5/ch05lvl1sec38/vertical-partitioning 1st sentence.

|         | physical location 1 | physical location 2 |
|---------|---------------------|---------------------|
| columns | Name, Surname       | City, Country       |

Bu fiziksel lokasyonlar farklı DB instance'ı ise; `Vertical Sharding` yapmış oluruz.
...
- Dosyalara parçalarken oluşturulan yeni tablolara "shard" veya "partition" adı verilir. kaynak: https://hazelcast.com/glossary/sharding/ 1st paragraph.
- Dosyaların bölündüğü farklı DB instance'larının her birine "shard" denir. kaynak: https://docs.oracle.com/en/database/oracle/oracle-database/18/odpnt/featDataSharding.html 1st paragraph, 2nd sentence.
...
var olan makinelere `CPU`, memory, disk gibi kaynaklar eklenerek güç arttırılması.
...
Bağımsız cluster'daki node'lar, diğer cluster çökmemesine rağmen çökmüş olarak algılama durumuna verilen isimdir.
...
- DB sistemlerinde; `Binary Large OBject (⟷ BLOB)` olarak kullanılıyor. bu objeler media objeleri gibi herhangi bir binary dosyaları olabilir.

## 📌 Spring ile BLOB tanımlama

DB tarafında `CLOB (⟷ Character large object)` olarak kullanılırken, `BLOB` binary objeler tutar.
...
Spring bu ikisi için tek annotation sunar ve field'ın tipinden (String veya byte[]) `CLOB` veya `BLOB` olup olmadığını anlar.
...
`data grid` terimi; '`grid computing`'den gelir.

"data grid" terimi frontend'de de kullanılır. Data'yı filtre, sort, paging gibi gelişmiş seçenekler sunan tablo component'idir.

`grid computing`: Büyük, yoğun ve karmaşık hesaplama görevlerini, tek bir güçlü bilgisayar yerine birçok bilgisayarın gücünü birleştirerek yapmak.

`data grid`; data'nın birçok node'a paylaştırılarak(distributed) yönetilmesini sağlayan sistemlere/mimarilere verilen genel ifadedir.

`in-memory data grid (⟷ IMDG)` terimi ise isminden de anlaşıldığı gibi RAM'de yönetilen data-grid'lerdir.

`key-value DB`'leri, sadece bir key'e karşılık herhangi bir tipte veri tutarlar. Fakat `data structure` DB'leri key-value ile kısıtlı değillerdir. list, set, array gibi daha karmaşık veri yapılarını da destekler. dolayısı ile `data structure` DB'leri aynı zamanda key-value veri yapısını da desteklemiş olur

## 📌 local cache

`In-memory DB`'lerin önemli özelliklerinden biri de local-cache özelliğidir. Redis gibi data store'ları genelde kalıcı data için değilde, çok kısa vadeli data'ları tutmak için kullanılırız. Bu sebeple genelde kayıt atan uygulama aynı data'yı tekrar okur. Bu sebeple local cache özelliği vardır. local-cache okunan bilgiyi local'de tutar ve her güncellemesini de sunucu ile sync tutar. Bu şekilde tekrar okuması gerekirse direk local'den okur. Local-cache policy'leri DB'den DB'ye çok fark edebiliyor.
...
`memcached`, `Redis`'e göre daha az özellik içerir.
...
`MariaDB`, resmi olarak `drop-in replacement of MySql` olarak sunulmaktadır.
...
`Türkçe` ve `İngilizce`'de aynı kelimedir.

`Türkçe`'de eş anlamlısı: `düzgü`.

`Norm`'un sözlük anlamı: `sosyal veya bilimsel bir olay için, yazılı olan veya olmayan standart/pattern/ölçüt/yöntemdir. uyulması beklenen kural, önceden belirlenmiş kalıp`.

Matematikte `norm`, bir vektörün büyüklüğünü veya uzunluğunu ölçen bir fonksiyondur. Yani bir vektörün ne kadar uzakta olduğunu tanımlar. örneğin; `(x,y)` noktasının, sıfır noktasına olan uzaklığı `L2 normu` (fonksiyonu, algoritması) ile hesaplanır. Matematikte birçok `norm` vardır.

`normalization` bilişimde; belli bir standarta uyarlama anlamına gelir. Örnekler:

- DB table `normalization` (`1NF`, `2NF`)
- bir tablodaki veya listedeki verileri standart bir sayı aralığına çevirip (basitleştirmek amaçlı), son kullanıcıya gösterme (bunun örneği başka başlıkta anlatılıyor)
...
normal form: normalleştirilmiş form anlamına gelir.
...
unique sütunlar null değerler alabilir.
...
Bazı node'daki verilerin bir kısmının, diğer node'daki data'lara göre çok daha sık kullanılması problemidir. Örneğin, korona döneminde, "maske" satışı çok olduğu için e-ticaret sitelerinde bu ürünlerin aramaları aşırı derecede artış gösterdi. Bu gibi durumlarda bu ürünleri özel node'lara taşımak ve sadece onları scale etmek gerekebilir.
...
- `Bitmap`
  ...
  Böylece artık `WHERE Cinsiyet = Erkek` sorgusu çalıştırıldığında, sadece `erkek-bit-dizisi`'ne bakılması yetecektir.
...
DB'lerde "UNIQUE INDEX" oluşturabiliriz veya bir sütunu UNIQUE yapıp ona index atayabiliriz. Aralarında çok çok ince farklılıklar var. kaynak: https://docs.microsoft.com/en-us/sql/relational-databases/indexes/create-unique-indexes?view=sql-server-ver15 1st paragraph, 3th sentence.
...
DB sunucusu, SQL sorgularını parse edip optimize eder ve execute eder. Bu işleme `hard parsing` denir. Oysa aynı sorgunun tekrar tüm bu işlemlere girmesine gerek yok. DB sunucusu, her SQL'in optimize edilmiş halini, bellekte tutar. Eğer aynısı gelirse bu sefer `soft parsing` yapar ve execute eder.

`Oracle DB`'de büyük küçük harf dahi (örneğin; Select, SELECT) sorgunun farklı olmasına sebep olmaktadır. kaynak: https://blogs.oracle.com/sql/improve-sql-query-performance-by-using-bind-variables "Why do bind variables matter for performance?" başlığı.

Buna çözüm olarak SQL'lerin elle yazılmaması önerilir. Bu şekilde case-sensitivity sorunu ortadan kalkar. Aynı zamanda 'statement'lara dynamic variable'ları bind ederek'te aynı SQL'in farklı değerlerle çalıştırıldığında, hard parsing yapılmamasını sağlamış oluruz. Çünkü; eğer binding yapmazsak; SQL aynı fakat içindeki herhangi bir integer farklı ise bile hard parsing yapılır. kaynak: https://blogs.oracle.com/sql/improve-sql-query-performance-by-using-bind-variables "Where do bind variables come into this?" başlığı.

Hard parsing ile soft parsing arasında performance olarak büyük fark var. bu kaynakta; https://blogs.oracle.com/sql/improve-sql-query-performance-by-using-bind-variables "What's the impact of hard parsing?" başlığındaki örnekte, `for loop` içerisinde aynı sorgu bind edilerek ve bind edilmeden yollanmış. Bir döngü 4 saniyede tamamlanırken, diğeri 1 buçuk dakikada tamamlanmış.
...
`ANSI` ve `ISO` kurumları `International Electrotechnical Commission (⟷ IEC)`'a bağlıdır.
...
| 1999 | SQL:1999 | SQL3             | Added regex matching, recursive queries (e.g. transitive closure), triggers, support for procedural and control-of-flow statements, nonscalar types (arrays), and some object-oriented features (e.g. structured types), support for embedding SQL in Java (SQL/OLB) and vice versa (SQL/JRT)                                                                                                                                                           |
...
Bu konu için güzel bir kaynak: `http://www.bristle.com/Tips/SQL.htm title: "Differences Between Oracle and MS SQL Server".`
...
sadece 1 statement yolladığımızda, arka planda bizim için 1 transaction oluşturuluyor. kaynak: https://docs.microsoft.com/en-us/sql/t-sql/language-elements/transactions-transact-sql?view=sql-server-ver15 "Autocommit transactions" başlığı altında " Each individual statement is a transaction." yazıyor.
