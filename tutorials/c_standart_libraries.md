...
- `assert.h`
  ...
  if `assert` makro fails, it will call `abort()`. but we can change this behavior via compiler configuration.
...
- `errno.h`
  ...
  `errno` değeri çoğunlukla `thread-local`'dir. bu sebeple multi-thread uygulamalarda her thread için farklı bir instance tutulur. bu sebeple hata kontrollerimizi buradan sorunsuzca yapabiliriz.
...
- `inttypes.h`
  ...
  `stdint.h`'i içinde zaten barındırıyor. bu kütüphane `stdint` içinde gelen fixed-size variable'ların `printf`, `scanf` gibi standart fonksiyonlarla çalışabilmeleri için formatter'ları barındırıyor. örnek:
...
- `sysexits.h`
  ...
  `exit code` tam sayılarını barındırıyor. konu başka başlıkta anlatılıyor.
