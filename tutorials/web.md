...
- işletim sistemi çekirdek adı

- Microsoft Windows için kurulu ".Net Framework" ve "Service Pack" sürümleri
...
- Reverse proxy
  ...
  Fakat "gateway" terimi çok genel (birçok amacı kapsayan) bir terim olduğu için terchi edilmemeli.
...
Burada dikkat edilmesi gereken nokta şu: Buradaki "stream" kavramı file-upload'daki stream'in kavramından farklı. 1 stream bağımsız HTTP requestlerini barındırıyor olabilir. yani; buradaki stream low level (protokol seviyesinde) bir terim, application-level diil.
...
cross-site cookie'ler kullanıcıları takip amaçlı kullanılabildikleri için "cross-site tracking cookie" olarakta isimlendirilirler.
...
- Web storage (local storage + session storage): bazı kaynaklarda "__DOM storage__" olarakta geçer. anahtar - value mantığı ile çalışır.
  ...
  - local sotrage: tarayıcı restart olsa bile bu veri saklanmaya devam eder.

  - session sotrage: her sekme için ayrı ayrı tutulur. sayfa refresh olduğunda silinmez. ama sekme kapatılıp yeniden açılırsa, data silinmiş olur.
