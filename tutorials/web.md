...
- __Uniform interface__
  ...
  __gRPC__: Bu kurala uymaz. CRUD metodları yoktur. Olsa da protokolde tanımlı diildir. Bizim payloadda kendi formatımızı yapmamız gereklidir.
...
- __cacheable__
  ...
  __gRPC__: Bu kurala uymaz. Çünkü bazı istekler karşılıklı streaming yapıyor olabilir. Bunların cache'lenmesi mümkün diildir veya mantıklı diildir.

  __Websocket__: Bu kurala uymaz. sadece payload ile veri paylaşımı olduğu için, tüm payload'ın cache-proxy tarafından dikkate alınması lazım. bu mümkün diil/veya aşırı yorucu (anti-pattern).
...
- __layered system__
  ...
  __gRPC__: Bu kurala uymaz. "cacheable" kuralında anlatığı gibi, gRPC için cache-proxy implemente dilemediğinden dolayı proxy'nin cache yapma olasılığı yok. Belki başka bir görev yapan proxy'ler implemente edilebilir, ama her amaçlı proxy gRPC için mümkün diildir. Dolayısı ile layered bir yapıdan her zaman bahsedilemez.

  __Websocket__: "cacheable" kuralı ile aynı sebepten. sadece payload ile veri paylaşımı olduğu için, tüm payload'ın cache-proxy (veya diğer proxy'ler) tarafından dikkate alınması lazım. bu mümkün diil/veya aşırı yorucu (anti-pattern).
...
- __Code-On-Demand__ (optional)
  ...
  __gRPC__: Bu kurala uymaz. ama opsiyonel oalrak server tarafta bunun için kod yazılabilir. ama protokolden bağımsız olarka destekleniyor olabilir.
