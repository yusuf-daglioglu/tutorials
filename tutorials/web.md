...
Burada dönen cevap bir metodu çağırır. metot bizim JS tarafında tanımlı olduğundan ona veri olarak parametre gönderir. İşte tüm bunları simüle eden bir fonksiyon yapıp HTTP Request-Ajax işlemi yaparmış gibi cross domain kuralını aşarak işlem yapabilir.
...
web tarayıcıları için birbirlerine alternatif eklentilerdir. Eklentiler sayesinde son kullanıcı JS'leri, istediği event (sadece standart JS event'leri değil, ekstra event'ler de kullanılabiliyor) sonrası çalıştırabiliyor. İnternette birçok hazır script paylaşılıyor.
...
DOM, bir programlama lisanı veya sadece JS için ortaya atılmış bir olgu değildir. DOM; HTML ve XML gibi dokümanları temsil etmek oluşturulan modeldir. Yazılım dünyasında "Model" objemizde, o model'in davranışlarına karşılık gelen metotlar olabilir. Bu metotlar ile model'in özellikleri üzerinde işlem yapabiliriz (arama, güncelleme...). Bu durum "HTML DOM"'da da vardır.
...
- Liste şu şekildedir:
  ...
  - __Fingerprinters__ (Firefox'a hard-code gömülü olan ve fingerprint'imizi aldığı düşünülen web sitelerin listesi var. sadece bu listedeki sitelere JS API'leri fake fingerprint döndürüyor (font, resolution...))
...
- __offline web app (web-cache or app-cache)__
...
- __WebRTC__
  ...
  tarayıcı ile tarayıcı arasında direk iletişim protokolü.
...
"__Bootstrap UI (or ng-bootstrap)__" ise, Angular modülüdür. birçok Bootstrap nesnesi Angular ile yazılmıştır. bu şekilde Angular ile daha uyumlu çalışılabilir.
...
Basic auth işlemi her HTTP request'te yapılır. yani header'a her istekte user ve şifre bilgisi yer alır. web tarayıcıları, cookie olarak saklar bu user bilgilerini. bu sebeple her defasında sormaz username ve şifreyi. aynı şekilde Java'da client'lar da bu şekilde davranabilir.
...
Tarayıcılarda plugin gerektirmeden 3d ve 2d oyunlar, nesneler yaratılmasını sağlayan JS API'sidir. OpenGL ES altyapısını kullanmaktadır.
...
- Microsoft'un Internet Explorer tarayıcısı için geliştirdiği PAPI.
- Microsoft Edge (Microsoft Windows 10 ile gelen yeni tarayıcı), ActiveX'i desteklememektedir.
