...
auth işlemi her `HTTP` request'te yapılır. her istekte `username:password` değerinin `base64`'ü alınır ve server'a `HTTP` header'da yollanır:
...
`base64` kullanıldığı için resmi olarak güvensiz bir auth biçimidir.
...
Client auth için özel bir URL'ye gitmez. Her istek direk olarak neye erişilmek isteniyorsa oraya yapılır. Tabi bu durumda her istek farklı path'e gidiyor ise, `uri` değeri değişeceği için `response` değeri de yeniden hesaplanıp sunucuya yollanmalıdır.
