...
```js
// C veya C++ kodunu wasm formatına convert etmiş olmalıyız önceden.
// wasm dosyası suncuu tarafta binary olarak tutuluyor.
...
sadece değişkenler arasındaki ilişkiyi göstermeye yarayan çizgisel anlatım şeklidir.
...
sadece herhangi bir olayın değişimini gösteren grafik. UML diyagramları buna örnektir.

## 📌 Cartogram (⟷ kartogram)

istatistiki bilgilerin harita üzerinde gösterildiği grafiktir.
...
- `HTTP` 2 client, `HTTP` 3 sunucusunun döndüreceği cevabı algılayamaz. yani `HTTP` 3 geriye uyumlu değildir.

- Eskiden `TCP` ile yapılan bazı süreçler artık `UDP`'nin payload'ında yapılmaya başlandı. Böylece süreçler hızlandı. `UDP`'nin payload'ında yapıldığı için bu süreçler user-space tarafından değiştirilebiliyor. Yani güncelleme olursa, `HTTP` kütüphanesi de bunu yapabilicek, `OS`'in `HTTP` desteği beklenmeyecek. Payload'daki bu protokol'e `QUIC (⟷ Quick UDP Internet Connections)` ismi verildi.

- fiziksel bağlantı farklı bir `IP` üzerinden devam etse bile (yani fiziksel olarak kopsa bile), sanal olarak devam ettirilebilir. Artık `IP` bazında değil, farklı ID'lerdne yararlanılarak sunucu client'ı tanıyabiliyor.
...
- domain: hangi domain'lerin buna erişecek yetkisi olup olmadığıdır. normalde tarayıcılar başka domain'den cookie okumaya veya yazmaya izin vermez fakat bu kısım 2 sebepten ötürü yapılmıştır:
  ...
  - subdomain'ler belirtilebilir: her subdomain (a.Google.com, b.Google.com gibi) sadece kendi yada o domain'e bağlı tüm subdomain'ler tarafından erişilebilir cookie set edebilir.
...
Web tarayıcıları veya eklentileri 3üncü party siteleri engellemeye başladığından, bazı internet siteleri kendi sub domain'lere istek yaptırarak diğer 3üncü parti sitelere ulaştırmaktadırlar. Bu yönelndirmenin sebepleri:
- 3üncü parti server'ların reklamlarını göstermek
- 3üncü parti server'ların CDN'lerini kullanmak
- 3üncü parti server'ların Analytics sistemlerini kullanmak
...
ince detaylar fark ediyor ama benzer şeyler yapıyorlar. web tarayıcı, `type`'a bakarak native eklenti çağırıyor. örneğin `PDF` veya video eklentisi (örnek `VLC` `Firefox` plugin).

Eğer `type`, `html` ise, o zaman `iframe` gibi çalışıyor.

`<video>` veya `<audio>` ise `HTML` video player açar ve birçok seçenek sunar developer'a. `CSS` ile bu vide player değiştirilebilir.
...
Bu konu başlığı da okunması faydalı olacaktır: [file:"java_jsp_and_jsf.md" - title:"JSP ile single page application"](./java_jsp_and_jsf.md)
...
svg her çözünürlüğe uygundur. canvas değildir.
