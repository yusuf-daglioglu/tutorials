...
`String` is a class. It is defined as alias: `string`. This predefined by compiler but it's hidden to developers like the other `value types` like `int`, `short` as below:

```c#
using string = System.String
```

`int` has been also predefined in same way:
...
class gibi şeyleri statik olmasını sağlar.
...
`C#`, `Java` ile aynı şekilde `value type` (`Java`'daki primitive'e denk geliyor) tipteki parametreleri `pass by value`, class'ları ise `pass by value of the reference` tekniği ile çalıştırarak yollar.

Eğer `ref` keyword'ünü kullanırsak, artık `value type` olan parametre, `pass by reference` tekniği kullanılarak fonksiyona aktarılır. Artık bu `value type` değere yeni bir değer atadığımızda bu yeni değer fonksiyon bittiğinde de geçerli olacaktır.

`out` keyword'ü de `ref` ile apaynı görevi görüyor. fakat bir fark var: `ref` ile initialize edilmemiş değeler yollanamazken, `out` ile yollanabilir.

## 📌 const vs readonly

`Java`'daki `final`'a benzer mantıkta çalışır.

`const` olarak belirlenen değer compile sırasında init edilmek zorundadır. Bu sebeple fonksiyon çağıramaz. Yani aşağıdaki olamaz:
...
int? b;
System.Console.WriteLine(b == null); // compiler error.
