...
- executable çalıştırıldığında sanal bir dosya sistemi yaratır ve çalıştırılan executable bu dosya sistemini görür. bütün dependency'ler bu dosya sistemindedir.
...
- `Nix`, `Linux` ve `MacOS`'ta native olarak çalışıyor. `MacOS`'ta `Linux-namespace` olmadığı için build paket build'leri `Linux`'taki gibi full izole bir ortamda olmamaktadır.

- `Windows`'ta, `Windows Subsystem for Linux` veya `Docker` içinde de çalışabiliyor.
