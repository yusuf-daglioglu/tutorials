...
- Tek bir dosyada birden fazla `YAML` tutabilmek için; `YAML` dökümanı isteğe bağlı olarak `---` ilen başlayabilir ve `...` ile bitebilir.

- biraz farklı da olsa JSON tarzı bir formatta da veriler tutulabiliyor

```yaml
ornek: {name: Martin Developer}
```
...
Yukarıdaki standartlar az da olsa birbiri arasında fark ediyor. bazı parser ve validator'larda bu bilgi lazım olabilir.
