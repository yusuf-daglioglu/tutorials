...
`JDBC`, `Java` için database spec'idir. (`driver` olmadan bir işe yaramaz.) `JPA` kendi içinde `JDBC` kullanır. `JDBC` database'ye bağlanan ve üzerinde işlem yapmaya yarayan `API`'dir. Her database bunun implementasyonlarını (`driver`'larını) yapar (`MySQL`, `Oracle` gibi). Bunun üzerine `ORM` yeteneğini katarak `JPA` ortaya çıkmıştır.
...
`JDBC`'nin çalışma prensiplerini belirlemektedir. client tarafta URL girilirken `type` belirtilir. örnek: `Oracle` `JDBC` kullanırken `URL` girerken şunu gireriz:
...
- `Type 1 driver`
  ...
  `JDBC`-`ODBC (or Open Database Connectivity)` bridge'i kullanılır.
...
- `Type 3 driver`
  ...
  bizim Java uygulamamız direk database'e bağlanmaz. Onun yerine arada bir proxy görevi gören bir servis vardır.
...
- `Type 4 driver`
  ...
  bu tipte bağlantılar %100 `Java` kodu kullanır. database sunucusu ile `Java` soketi açıp haberleşirler. %100 `Java` olduklarından her tarafta kullanılabilirler.
...
`Oracle`'ın, `OJDBC`, `JDK`, `JDBC` spesifikasyonu sürümleri uyumu için burada resmi compatibility matrix table'lar mevcut:
