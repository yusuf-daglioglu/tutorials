...
- `systemd-journald`
  ...
  `libc`'nin `syslog` fonksiyonu çağrıldığında, fonksiyon bir `UNIX domain socket`'e log'ları `syslog` formatında yollar. `libc`, bu log'ları kimin okuyacağına karışmaz. Örneğin; bu `UNIX domain socket`'ten `systemd-journald` okuma yapar.
...
## 📌 Comparison of collectors and agents (observability agent)
...
| `Splunk Edge Processor`          |                       | ligher then `Universal Forwarder (⟷ UF)` and `Heavy Forwarder (⟷ HF)` | agent & aggregator | only forwards to `Splunk`.         |
| `Universal Forwarder (⟷ UF)`     | `Golang`              |                                                                       | agent & aggregator | only forwards to `Splunk`.         |
| `Heavy Forwarder (⟷ HF)`         | `Golang`              |                                                                       | agent & aggregator | only forwards to `Splunk`.         |
