...
- LLVM-based diller için run-time içeriyor. Yani; "LLVM bitcode" formatında olan dosyalardan native-executable üretebiliyor.
...
- JVM'in içindeki default class-loader'a "`bootstrap class loader (or primordia class loader)`" ismi verilmiştir. bunun asıl amacı "`/lib`" ("`$JAVA_HOME\lib\rt.jar`" ve diğerleri) dizinindeki core JRE kütüphanelerini import etmektir.
...
Super class'taki bir field'ı override edemeyiz. Zaten etmemeliyiz de. Ç

ünkü super class o filed'ı kendi içinde tamamen özel bir logic için kullanıyor olabilir.

Eğer biz o field'a erişmek istiyorsak, "super.name" veya getter-setter metotları aracılığı ile erişmemiz daha doğrudur. Inheritance'ta sadece davranışları override etmeliyiz, yani metotları.
...
## 📌 AWT (or Abstract MS Windows Toolkit) vs SWING
...
## 📌 JAX-WS (or Java API for XML Web Services) vs JAX-RS (or Java API for RESTful Web Services)
...
## 📌 marshalling (or marshaling) vs serialization
