...
AtomicIntegerArray, AtomicInteger, AtomicLong gibi birçok sınıf barındırır. Tümü multithread uygulamalarda aynı variable'ları yönetmek için kullanılır. Bu sebeple buna uygun metotlar içerir. Örnekler:

- AtomicInteger class --> public int addAndGet(int delta)

  integer'a değer ekler ve yeni değeri döndürür. Eğer AtomicInteger olmasaydı bunu bu şekilde kullanacaktık:
...
- AtomicInteger class --> public double doubleValue()
  ...
  integer'ı double'ye cast edip döndürüyor. Bu metod multi-thread için özel tasarlanmamış. Bu şekilde ek yardımcı metotlarda barındırabiliyor.

- AtomicInteger class --> public void lazySet(int newValue)

  Eventually sets to the given value. Farklı thread'ler aynı anda değeri güncelliyor ise, bu metod'un ne zaman işini yapacağı kesin değildir. Fakat eninde sonunda metod çalışacak.
...
kind of Reader that reads only character with buffer mechanism.
...
- Jenerik programlama (or generic programming)
- Şablon programlama (or Template Programming)
...
- notify
- notifyAll
- finalize: object JVM tarafından destroy edilmeden önce çağrılır.
...
```java
@Todo(priority = Todo.Priority.MEDIUM, author = "Ahmet", status = Todo.Status.STARTED)
...
- static forName(String className) : metottur. Bu metoda herhangi bir sınıfın ismi string olarak gönderilebilir. bize gönderdiğimiz sınıf için Class objesini döndürecektir. Örnek: Class myCarClass = Class.forName("Car");
...
- __Non-Heap (or Off-heap)__
  ...
  - __Permanent Generation (or PermGen)__
    ...
    Java version 8 Update: __PermGen__ is replaced with __Metaspace (or Method Area)__ which re-sizes dynamically at runtime. Options __PermSize__ and __MaxPermSize__ have also been removed in JDK 8.
...
- JVM'in içindeki default class-loader'ın ismi "bootstrap class loader (or primordia class loader)"'dır. bunun asıl amacı /lib ($JAVA_HOME\lib\rt.jar ve diğerleri) dizinindeki core JRE kütüphanelerini import etmektir.
...
Super class'taki bir field'ı override edemeyiz. Zaten etmemeliyiz de. Çünkü super class o filed'ı kendi içinde tamamen özel bir logic için kullanıyor olabilir. Eğer biz o field'a erişmek istiyorsak, "super.name" veya getter-setter metotları aracılığı ile erişmemiz daha doğrudur. Inheritance'ta sadece davranışları override etmeliyiz, yani metotları.
...
WebStart; Applet'lerin bir sonraki adımı gibi düşünülebilir. local siteme uygulama (Applet benzeri ufak uygulama) indirme, çevirim dışı daha sonra açmak için işletim sistemine kısa yol atama, kurulan uygulamaları yönetebilme, sürüm güncelleme gibi işlevlerin yapılmasını sağlar. bu uygulamalar tarayıcıdan direk olarak kurulabilir. bu uygulamacıklarda izinler çok kısıtlıdır.
...
Nativelib içerisindeki kütüphaneler işlemcilerin özelliklerinden de yararlanmaktadırlar. örneğin System.arraycopy() metodu array kopyalamayı işlemcinin özelliğinden yararlanarak yapar. bu tarz metotlardan faydalanmak uygulama hızını çok etkilemektedir.
...
JavaEE'de default implementasyon kavramı yoktur. hangi sunucuya atarsak jboss, Tomcat kendi implementasyonunu sunar. üzerinde çalıştırdığımız sunucununkinden farklı implementasyon istersek, biz spesifik jar'ları yükleyip konfigürasyon yapmak gerekecektir.
...
elipse IDE'de herhangi bir projeye sağ tıklayıp, New --> Web Service Client --> seçeneğine tıklayıp WSDL'i veriyoruz bize Java kodu üretiyor. burada Eclipse IDE arka planda Axis'in generator'ını kullanıyor.
...
Uzak makinede yada aynı makinede farklı process'ler arasında metot çağırıp işletme mekanizmasıdır. bu bir konsepttir. protokol değildir. herkes uzaktaki bir metodu çağırıp dönüş alabilir. genelde programlama dilleri kendi protokollerini/standartlarını kullanıyor. bu sebeple cross-language değillerdir.
...
Eclipse ve Intellij debug mode'undayken aşağıdaki butonları sunar:
...
init dildikten sonra, nesnenin içindeki state değiştirilemez olduğunda, o nesne immutable'dır. örneğin unmodifiable collection'lar immutable'dır. çünkü içindeki referans değişkenler (liste elemanları) değiştirilemezler. burada karıştırılmaması gereken nokta;
...
- append-only data okuduk. bu okunan data'yı immutable yaparız ki, ilgili data'nın değiştirelemeyeceğinden emin olmuş oluruz. örnek append only data'lar:
  ...
  - okunan config yml data'sı.
- backup data read-only okunur. ama yine de immutable yapılmalıdır ki, kod içerisinde farkında olmadan bi manipülasyon yapılmışsa, log'da hatayı görebiliriz.
...
- file-system levels/snapshot
  ...
  - union file system (or UnionFS) for Docker
...
- Immutable Infrastructure (or IaC)
...
Ek not: Thread-safe class geliştirildiğinde dikkat edilmesi gereken ince bir husus var: thread-safe olmayan bir sınıf parametre olarak alındığında, ne olacağına özellikle dikkat edilmeli (Yukarıdaki appendTo örneği gibi). Çünkü bu metod 1 satır ve hızlı bakıldığında thread-safe gibi görünüyor. Fakat appendTo bizim geliştirdiğimiz bir metod değil. Dolayısı ile bu tzr ince noktalar karşımıza çıkabiliyor.
...
Java 8 ile gelmiştir. Java 9 ile ek metotlar bu sınıfa eklenmiştir.
...
mapperFactory
    ...
    .field("person_nickname", "nickname")
    .register();
```
...
Mapstruct compile sırasında bu arayüzün bir implementasyonunu oluşturuyor "target" dizini içerisinde:
