...
- Symbolic link (or symlink or soft link) : Microsoft Windows'taki kısa yol
...
UNIX tabanlı olmayan sistemlerde (örnek Microsoft Windows) her kaynağa işletim sistemin sunduğu API'ler aracılığı ile erişilebilir. Örneğin; bir stream'e veri atarken Microsoft Windows'ta MyAPIClass.writeValue("new value"); diye yazılırken, Linux'ta herhangi bir output-stream ile ilgili dosyaya byte veri yollamak yeterlidir.

Stream olmasının en temel yararı stream kütüphaneleri her dilde var olduğundan her dilden API ihtiyacı olmadan işlemleri gerçekleştirebilmemizi sağlar.
...
UNIX-like'larda açık dosya sayısının bir limiti vardır.
...
her dosya kullanımında process içerisinde bu dosyanın adresi (programlama dilindeki obje/nesne) tutulur. bu adres file descriptor'tur. C dilinde bu bir tam sayıdır. Dosyalar sanal olarak /prod/pid/fd (/prod/process-id/file-descriptor) gibi dosyalarda tutulur. örneğin; /prod/1001/48 dosyası bir domain soketine yada bir sürücüye denk geliyor olabilir. bu dosyalar gerçekten de o dizinde yaratılırlar.
...
Ekran çıktısını organize eder.

Pencere açılmasına izin verir.

GUI'ye direk etkileşimli bazı input cihazları yönetir (örnek: fare, klavye, gamepad). çünkü buradaki input'lar direk UI'daki objelere parametre olarak geçilmesi sağlanır.

hem server hem client tarafı olabilir.

## 📌 X Window System (or X)

Piyasada en çok 11'inci sürüm kullanıldığından X11 olarak da isimlendirilmektedir.
...
Window System üzerinde pencerelerin yönetimi (işlevlerini belirleyen: minimize, maximize, always on top) sağlayan yazılımdır.

pencerenin konumu, boyutu, odak durumu gibi meta verileri tutar.

Eğer server-side decoration varsa bu modülün içinde gömülü olmalıdır.
...
- __Compositing Window Manager (or compositor)__
...
- __Stacking window manager (or floating window manager)__
...
- __dynamic tilling (or dynamic)__
...
ekrana bir şeylerin hangi efekt ile gösterileceği, hangi efekt ile sağa sola taşınacağı, gözüktüğünde nasıl gözükeceği (transparanlık) gibi işlevleri manipüle eden yazılımdır. bu yazılım X'te opsiyoneldir (yani hiç olmayabilir), Wayland'ta mutlaka olmalıdır.

__Compositing Window Manager__ ile __compositor__ eş anlamlı değildir. __Compositing Window Manager__ bir window manager grubudur ve içinde gömülü __compositor__ bulundurmak zorundadır.
...
Bu dekorasyon, normalde X server'ın görevidir. Fakat CSD ile, client'lar da artık kendi pencere yapılarını oluşturabiliyor. Eğer bu görevi (client application'ın direk kendisi) bunu üstleniyorsa CSD, eğer üstlenmiyorsa,  __Server-Side Decoration (or SSD)__ denir.
...
## 📌 Widget toolkit (or widget library or GUI toolkit or UX library)
...
- GTK (or old-name:GIMP Toolkit or old-name:GTK+)
- Qt
...
- Debian (deb based)
  ...
  - Raspberry Pi OS (or old-name:Raspbian) (Raspberry cihazında çalışması için tasarlanmış. lxde yüklü.)
...
- Fedora (rpm based)
  ...
  - Red Hat Enterprise Linux (or RHEL) (ticari)
    ...
    - CentOS (or Community Enterprise Operating System) (community yönetiyor. RHEL'den hiçbir farkı yok. sadece lisansı ve logosu vs değiştiriliyor)
...
- Mandriva (or old-name:Mandrake) (RPM based) (bu proje sonlandı. bu sebeple aşağıda 3 fork'u oluşturuldu)
...
- Alpine (package manager: APK (or Alpine Package Keeper) ) (çok ufak oluşu en temel özelliği. prod'da kullanılması önerilir.)
...
Ubuntu türevi değildir. Ubuntu'nun standart ISO'sundan farklı olarak GUI ve birçok paketi yüklü getirmez. son kullanıcılar arasında "minimal ISO" yada "mini.iso" olarak da adlandırılır. bu ISO Ubuntu repo'larına kurulum sırasında bağlanarak indirme yapar. tabi güncel paket kuracağı için update işlemine gerek kalmaz.
...
Linux'larda çıkan bir tanımdır. OS'in olduğu dizinlerin read-only olduğu OS'tur.

Normalde OS dizinleri zaten read-only'dir fakat bazı dizinler örneğin "/etc" ve "/usr" gibi read-only değildir. Fakat immutable distro'larda "/etc" ve "/usr" dizinleri de read-only'dir.

Bazı kaynaklara "immutable" yerine "atomic" terimi kullanılır. Fakat bir biri yerine geçebilen terimler değildir. Immutable OS'ler, aynı zamanda atomic oldukları için bu terimler birileri yerine sıkça kullanılırlar.

Immutable dosya dizinlerinin read-only olduğunu temsil eden bir terimdir. Oysa "atomic", OS'un içindeki uygulamaların güncellemeleri, kurulumları ve silinmeleri atomic yapıdadır. Yani eski ve yeni state olarak tümüyle kaydedilir. Bu şekilde hata olma durumunda eski state'e çok hızlıca gidilebilir.
...
rpm (or Red Hat Package Manager) paket yönetim sisteminin komut satırı uygulamalarıdır. dnf yum'un türevidir. Fedora'nın birçok türevinden kullanılıyor.
...
örneğin Android emülatörleri CPU işlemlerini masaüstü PC'de gerçekleştirdiğinde birebir CPU çevrimi yapmaktadır.

Wine; aynı Microsoft Windows 7'nin, "Microsoft Windows Vista uyumluluk mode'u" mantığı ile çalışmaktadır.
...
Bazı OS'larda "csh" kısa yolu, tcsh'ye yönlendirilmiştir (sh-->bash'teki gibi).
...
UNIX ve benzeri sistemlerde her şey dosyadır ve terminalde bir dosyadır. bir komutun çıktısı ve girdisi dosyadan okunacak şekilde basittir. Oysa powershell bir script dilidir. üst seviyeli API aracılığı ile Linux'ta yapılabilen birçok şeye imkan sunar.
...
En başta belirtildiği gibi: Ekranda renkleri gösteren katman terminal emülatördür. fakat echo ve printf gibi komutlar (or herhangi bir çıktı üreten komutlar), escape karakterini dışarıya yansıtıp yansıtmayacağı önemlidir. çünkü yansıtıp yansıtmayacağına göre, terminal emülatör onun renkli olup kontrol etmektedir. bu sebeple bazen komutun or shell'in nasıl davrandığı da renklendirmek için önemli olabiliyor.
...
Linux'ta GUI masaüstü ekranı açıkken buna basıldığında, tüm ekranda konsol ekranı açılır. arka planda varsayılan olarak 6 adet ekran açıktır. 7inci ekran GUI'nin kendisini (X or Wayland) açmış olan session'dır.
...
bu komutlar birer program değildir. bu komutlar terminal emülatörlerin sunduğu bir özellik değildir. bu komutlar interpreter'ın (Bash, Zsh gibi) sunduğu keyword'lerdir/özelliklerdir. örneğin "which echo" yazdığımızda sonuç alamayız. çünkü echo bir builtin command'dır.
