...
- regular file
  ...
  dosya sistemindeki normal özelliksiz bir dosya

- dizin

  dosya sistemindeki klasör/dizin
...
bir programın kendisi çok bellekten harcarsa (örneğin durmadan integer array'ler oluşturursa) memory doldu hatası alacaktır. bu OS'u etkilemez. fakat program durmadan soket açarsa, OS bunlara karşılık birer dosya oluşturur. bu durumda dosya sistemi bu durumdan etkilenecektir. bunu engellemek için bu limit var.
...
Örnek implementasyon: __X (or X Window System)__ (Piyasada en çok 11'inci sürüm kullanıldığından X11 olarak da isimlendirilmektedir)
...
- __Display Server (or Window Server)__
...
Window System üzerinde pencerelerin yönetimi / işlevlerini belirleyen:
...
Windows manager'ların kurulum paketleri bazen gömülü ek uygulamacıklar olabiliyor. böyle olunca neredeyse bir masaüstü yöneticisi(Gnome) gibi hizmet verebilmektedirler.
...
Bu dekorasyon katmanı, normalde X server'ın görevidir. Fakat CSD ile, client'lar da artık kendi pencere yapılarını oluşturabiliyor. Eğer bu görevi (client application'ın direk kendisi) bunu üstleniyorsa CSD, eğer üstlenmiyorsa,  __Server-Side Decoration (or SSD)__ denir.
...
"OS" ve "OS çekirdeği" farklı kavramlardır.

## 📌 UNIX

OS çekirdeğidir. açık kaynaklıdır.

Çekirdeğinde BSD (Unix fork'u) kullanan 30'dan fazla OS var. Bunlardan bazıları:
...
Apple çok eskiden UNIX OS kullanıyordu. Daha sonra lisans sorunları sebebi ile BSD çekirdeğini kullanmaya başladı. BSD'den türettikleri OS çekirdeğine Darwin adı verildi.

## 📌 BSD (or Berkeley Software Distribution or Berkeley Standard Distribution)

"Berkeley Unix" ilk çıktığındaki ismiydi.
...
__IEEE (or Institute of Electrical and Electronics Engineers)__ tarafından belirlenen OS standartlarıdır.
...
- tarih - sleep() gibi API'ler nasıl davranacak.
...
## 📌 Wine (or Wine Is Not Emulator)

MS Windows executable dosyalarının UNIX-like'larda yürütmeye çalışan yazılım.

Wine ile çalıştırılan bir executable, sistem çağrısı yaptığında çalıştırdığı metot, POSIX'te denk gelen en yakın metottur. bu sebeple stabil çalışamamaktadır.

Wine yukarıdaki sebepten bir emülatör değildir. emülatör olsaydı, sistem çağrıları metotları MS Windows'unki ile birebir aynı şekilde implemente edilmeleri gerekecekti.
...
özellikle X oturumu dosyalarında da yetkilendirme sorunu olabiliyor. Bu sebeple grafik arayüzü uygulamalarını ilgilendiren bir konu. Bu sebeple Gnome 'gksudo', KDE uygulamaları ise 'kdesudo' komutunu kullanıyor.
...
| hyper (or old-name:HyperTerm)     | web altyapısı üzerine kuruludur. hyper uygulaması, HyperTerminal ile karıştırılmamalıdır. HyperTerminal MS Windows için çok farklı amaçta geliştirilmiş bir yazılımın ismidir.                                                    |
| Tilix                             |                                                                                                                                                                                                                                          |
| Terminus                          |                                                                                                                                                                                                                                          |
| Alacritty                         |                                                                                                                                                                                                                                          |
...
### 📌📌 MS Windows Console Host (or Console MS Windows Host)
...
MS Windows 7 öncesi conhost yerine "ClientServer Runtime System Service (or CSRSS)" kullanılıyordu.
...
MS Windows için açık kaynaklı terminal emülatördür. MS Windows'un komut satırı ile hiçbir bağlantısı yoktur. aşağıdaki paketler opsiyonel olarak mintty ile entegreli çalışabilir:
