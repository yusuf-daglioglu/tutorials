...
- 2000-4000 arası olanlar `N3` node'unda --> Bu node'a 1000 satır `N1`'den, 1000 satır'da `N2`'den taşınır. Yani; bölüştürülmüştür.
- 4000-5000 arası olanlar `N2` node'unda

Yani Klasik `HashMap`'lerdeki gibi module alma işlemi yapılmaz. Hard-coded her node hangi alanları ttutuğu sabittir ve tam olarak her node aynı sayıda bilgi tutmak zorunda değildir.

`Consistent hashing` genel bir terimdir. Bu tekniğin birçok implementasyonu vardır. Yukarıda anlatılan `ring-based hashing` isimli implementasyonudur ve detaylı şekilde anlatılmamıştır.

## 📌 rebalancing

bir node silinirse veya eklenirse, data taşımak gerekebilir. bu işleme rebalancing gerekir.

`ring-based hashing` kullanırken, `virtual-host` rebalancing tekniği kullanılabilir. Kullanılması opsiyoneldir.

`virtual-host` bir fiziksel node'un, ring üzerinde birden fazla kez temsil edilmesidir. Yani mutlaka sıralı şekilde olmak zorunda değildir fiziksel node'ların ring'deki görünümü. `virtual-host` varsa şöyle olabilir ring design:

- 1000-2000 arası olanlar `N1` node'unda
- 2000-4000 arası olanlar `N3` node'unda
- 4000-4500 arası olanlar `N1` node'unda --> Bu sırayı bozdu. Bu sebeple bu design `virtual-host`'tur.
- 4500-5000 arası olanlar `N2` node'unda
