...
`Java` tarafında artık `Bean` hazırdır. `message1` değeri otomatik `JavaEE` veya `Spring` tarafından doldurulacaktır:
...
`JavaEE` standardıdır. bu sebeple `annotation`'lar `javax.annotation.*` içerisindedir. `Bean`'lerde initialize ve destroy sırasında çalışacak metotların üzerine konulur. `beans.xml`'de belirtilmemişse, bu `annotation`'lar kullanılabilir.
