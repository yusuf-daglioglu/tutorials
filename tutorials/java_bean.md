...
- `JavaSE` kendi `Bean`'lerine `JavaBean` ismini vermiştir.
- `Spring`, `Spring Bean` diye isimlendirir.
- `JavaEE`, `EJB` framework'ü `EJB (or enterprise javaBean)` olarak isimlendirir.
...
`Java`'nın `DI` spesifikasyonudur. `JavaSE` de dahi kullanılabilir (ve `CDI container`'ı `runtime`'da olmak zorundadır). `kaynak: (source-id: 118) title: "13. Bootstrapping a CDI container in Java SE", 1st paragraph.`
...
- çeşitleri:
  ...
  - `entity Bean`
    ...
    veritabanı modellemesi için yaratılan `Bean`'ler
