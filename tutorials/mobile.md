...
Android'deki yazılımlar bu dosyayı görrse o dizinde son kullanıcyı ilgilendiren media dosyaları olmadığını anlar.

Örneğin; thumbnail dosyalarının bulunduğu bir dizinde bu dosya olmalıdır.

Bu dosyanın içi boştur.
...
`Qualcomm`'un `bootLoader` ismi `LK (⟷ Little Kernel)`'dir.
