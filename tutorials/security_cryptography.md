...
MAC işlemini HASH algoritması kullanarak yapar ve hash algortiması için anahtar gereklidir (yani hash algoritması "cryptographic hash function" klasmanında olması şarttır). Bu şekilde sadece anahtara sahip olan kişi,

- data'nın saf hali
- data'nın HMAC alınmış hali

yukarıdaki 2 datayı karşılaştırabilir. Böylece saf data'nın değiştirip değiştirilmediğini bilebilir.
...
## 📌 doğum günü saldırısı (or birthday atack)

öncelikle olsasılık bilimindeki "doğum günü problemi (or birthday problem)" konusundan bahsetmek gerekli.

bir grup insanın aynı doğum günü olma olsasılığı aşağıdaki gibidir:
...
Kriptografide de, hash alarak bazı işlemler yapılabilmekteder. Örneğin, Eğer SHA-1 gibi zayıf bir hash fonksiyonu kullanılıyorsa, saldırgan 2 farklı mesajın aynı hash değerini üretmesini sağlayarak, dijital imzalarla sahtekarlık yapabilir.

birthday atack terimi de burada ortaya çıkıyor. hash çakışması olması için brute force attack yapılıyor. bu dışarıdan bakıldığında zor olasılık gözükebilir, fakat birtday problem'deki gibi yüksek olasılıkla çakışma da meydana gelebilir (eğer hash algoritması zayıf ise). işte bu benzetme buradan gelmektedir.
