...
- public-private anahtar yapısına dayanır.
...
Bir çeşit __Secret sharing (or secret splitting)__'dir.
...
__coder (or kodlayıcı)__ ile __cypher__ farkı şu.
...
- Gizli mesajlaşmalar. (insanlar arasında birebir anlaşarak kullanılır. akademik ortamda ve public protokollerde kullanılmaz.)
...
- server load balancing
  ...
  örneğin: 5 sunucu varsa, gelen request'i IP'sinin hash'i alınır böylece "modulo 5" işlemine tabi tutulabilir. hangi sunucuya gideceğini bi veritabanı kullanmadan yönlendirebilmiş oluruz.
...
Farklı user'lar, aynı şifreyi kullanarak sistemimize register oldu. Bu durumda ikisinin aynı hash değeri database'de kayıtlı olur. bu bir risk'tir. DB'yi okuyan kişi bunu görebilir. Fakat her şifre için farklı salt kullanırsak bu durumu aşmış oluruz. (Rainbow table attack'tan bizi korumaktadır.)
...
bu veritabanlarını kullanarak hacker'lar, disk'ten kar ediyorlar ama CPU'dan zarar ediyorlar.
...
Blok şifreleme kullanan birçok algoritma vardır. Fakat her blok şifrelemenin de birçok çeşidi (tarzı) vardır. Bunlar "__çalışma kipi (or mode of operation)__" olarak ta adlandırılırlar. örnek çalışma kipleri:
...
| KDE Wallet Manager (or KWallet or KWalletManager) | KDE and many other desktop managers   |
| Keychain                                          | MacOS                                 |
| Windows Credential Store                          | Microsoft Windows                     |
...
"RSA Security" kuruluşu tarafından yayımlanan güvenlik standartları ailesidir.
