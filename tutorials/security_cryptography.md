...
Ron Rivest, Adi Shamir, ve Leonard Adleman tarafından geliştirilmiştir. RSA ismi bu kişilerin soy isimlerinin baş harflerinden gelir.
...
Bir çeşit __Secret sharing (or secret splitting)__'dir. Bir anahtar birden fazla anahtar olarak parçalanır. Ancak bu parçalar bir araya getirilerek master key oluşturulur. Bu şekilde "secret zero" problemine (kısmen) çözüm getirilir.
...
__cryptogram__ bilgisayar olmadan, elle çözülebilecek kadar basit, human-readable karakterlerden oluşan şifrelenmiş metne denir. bu şifrelenmiş metin, bilgisayarsız ortamda da çözülebilsin diye ayarlanmıştır. eğlence amaçlı kullanılmaktadır.
...
örnek; Cassandra arka planda primary key'leri non-cryptographic hash kullanarak saklar.
...
Hacker'ler birçok string'in MD5 hesaplanmış halini tuttukları veritabanları var. bu veritabanlarına "__rainbow table__" denir.

standart bir lookup table mantığında düşünürsek, bu veritabanını çok büyük olur. buna kısmen de olsa bir trick ile çözüm bulmuşlar (bu trick güvenlik konusunun dışında olduğu için bunu anlatmıyorum).

bu veritabanlarını kullanarak hacker'lar, CPU'dan tasarruf ediyorlar, ama disk'ten zarar ediyorlar.

bu veritabanını kullanarak yapılan herhangi bir saldırıya "Rainbow Table Attack" denir.
...
__hash fonksiyonu__ farklı uzunlukta input'lar alıp, sabit boyutta output dönen fonksiyonlardır. Birden fazla girdi, aynı çıktıyı verebileceği için hash fonksiyonları geriye çevrilemezdirler.

## 📌 HMAC (or keyed-hash message authentication code or hash-based message authentication code)

MAC işlemini HASH algoritması kullanarak yapar ve hash algoritması için anahtar gereklidir (yani hash algoritması "cryptographic hash function" klasmanında olması şarttır). Bu şekilde sadece anahtara sahip olan kişi,
...
içerisinde CA'ların sertifikaları bulunduran dosyalara Java dünyasında verilen isimdir. TrustStore bu dosyayı temsil eden sınıfın ismidir. TrustStore içerisindeki tüm sertifikalar ile kurulan bağlantılar güvenli olarak kabul edilir.
...
## 📌 doğum günü saldırısı (or birthday attack)

öncelikle olasılık bilimindeki "doğum günü problemi (or birthday problem)" konusundan bahsetmek gerekli.
