...
- `HTTP` 2 ve `WebSocket` geldi. Eski sürümlerde üçüncü parti kütüphanelerle kullanılıyordu. `JavaEE` ve `Spring` kendi içinde üçüncü parti kütüphaneleri gömülü getiriyor ve onları kullanıyordu. bu sebeple eski sürüm `Java` kullanılsa bile, `JavaEE` veya `Spring` kullanarak WebSocket ve `HTTP` 2 kullanılabiliyordu.
