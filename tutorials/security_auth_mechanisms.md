...
## 📌 SASL (or Simple Authentication and Security Layer)

`SASL` sadece kimlik doğrulama (authentication) sağlar, yetkilendirme (authorization) mekanizmasını içermez.

Hem client hem de server'ın `SASL` desteklemesi şart.

`SASL` client ile server arasındaki haberleşme protokolüdür.

Client kimlik doğrulama isteği yollar. Server desteklediği mekanizmaları yollar. Client kendisinin de desteklediği bir mekanizma ile süreci başlatır. İşte bu seçmek için haberleşme kısmını `SASL` üstlenir.
...
- `DIGEST-MD5`
  ...
  özel bir protokol. güvensiz.

- `GSSAPI`

  `GSSAPI` uyumlu herhangi bir protokolü başlatmıyor. Sadece Kerberos version 5 destekliyor. Bu sebeple isim kafa karıştırıcı olabilir.
...
- `GS2-*`
  ...
  `GSSAPI` uyumlu herhangi bir protokolü yürütebilir.
...
## 📌 JAAS (or Java Authentication and Authorization Service)
...
`JAAS` `LoginModule`'leri (bir diğer değiş ile implementasyonları) (bizdeki örnekte "`org.apache.kafka.common.security.plain.PlainLoginModule`") istenilen herhangi bir `Java` kodunu çalıştırabiliyor. Dolayısı ile `JAAS` protokol'den bağımsızdır. Yani server tarafın JAAS destekliyor olma gibi bir durumu söz konusu değildir.
...
## 📌 Generic Security Service Application Program Interface (or GSS-API or GSSAPI)

Programlama dillerinden bağımsız bir `API`'dir. Her metodun ne yapacağı `RFC`'de yazılmıştır. Bu `API` sayesinde, authantication yapacak mekanizmayı bilmeden authantication olabiliriz.

Hem server hem client tarafın GSSAPI destekliyor olması şarttır.

Sadece kimlik doğrulama yapar. Yetkilendirme özellikleri yoktur.
...
Bu protokolü destekleyen sunucular:

- `Microsoft Active Directory (or Microsoft AD)` (veya sadece `Active Directory (or AD)`)

  SMB protokolünü de destekliyor.
...
- her user için içini bizim doldurduğumuz key-value (işte bu kısım ile yetkilendirme de yapabiliriz)

görebilir.

Fakat query atabilmesi için öncesinde `LDAP`'a doğru şifre ile login olması gereklidir. Bu login aşaması `LDAP` dünyasında "`Bind`" olarak adlandırılır. Tam olarak bu login işlemini saf `LDAP` ile yapabiliriz. Fakat istersek sadece bu login aşaması için `Kerberos` protokolü de kullanılabilir.
...
dosya paylaşım için yetkilendirme protokolüdür.

farklı sürümleri vardır.

## 📌 Samba

`SMB` ve `LDAP` protokollerini destekleyen cross-OS yazılımdır.

`Kerberos` ile login'i de native olarak destekler.

`SMB` yetkilendirme protokolüdür. Login için kendi basit bir mekanizma destekler, ama istenirse `LDAP` ile login olunmasını da sağlayabilir.

## 📌 Kerberos

Sadece Kimlik doğrulama protokolüdür.
