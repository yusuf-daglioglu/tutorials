...
sadece Microsoft Windows'un sunduğu bir format atma şeklidir. tek farkı normal formatın, quick formata ek olarak, diski komple bad sector'lere karşı denetlemesidir.
...
- Microsoft Windows, HP laptop'larında yüklü gelir. Burada OEM Microsoft oluyor.
...
Microsoft Windows, 8 ve sonrası sürümlerde "shut down" işlemi ile "Hybrid Sleep" uygulamaktadır. Microsoft Windows'ta gerçek bir sistem kapanması için "restart" işlemi yapılmalıdır veya Microsoft Windows ayalarında "fast startup" (or Fast Boot) isimli bu özellik devre dışı bırakılabilir.
...
- Microsoft Windows, her kurulumdan sonra sürekli MBR'yi kendi bootloader'ına yönlendirir. Microsoft Windows'un bootloader'ı, diğer OS'ları tanımaz (veya tanısa bile çok zahmetlidir). bu sebeple her Microsoft Windows kurulumu sonrası, Linux ve diğer OS'lar ile Grub'u tekrar kurmak gerekmektedir.
...
- Microsoft, Microsoft Windows'un anahtarını bazı farklı OS'ları firmalarına da veriyor (örneğin Fedora). Bu şekilde o makinada, güncel Fedora ISO'su ile direk OS başlatılabiliyor oluyor.
...
- GPT olarak tasarlanmış bir diski makineye taktığınızda BIOS anakartlar ile buradaki OS'ları başlatamazsınız. Fakat burada yanlış anlaşılma olmasın: Sadece OS boot edemezsiniz. Onun dışında, bir OS yürütülür durumdayken, GPT bölümlenmiş HDD'yi mount edip, hem Microsoft Windows hem de diğer tüm OS'ları ile okuyup yazabilirsiniz (default olarak OS okuyamasa bile üçüncü parti uygulamalarla yapılabilir). Çünkü HDD'yi okuyan bir yazılımdır. BIOS değil. Yani OS boot etmekle, HDD'yi sadece data olarak kullanmak tamamen ayrı kavramlardır.
...
- boyutu değişkendir. Örnek boyut: Ortalama 100 MB sadece Microsoft Windows-10 için yeterlidir. Fakat ileride yapılabilecek ek OS kurulumlarının EFI dosyaları için bu bölümün boyutunu yüksek tutmakta yarar var.
...
| MultibootUSB                                 | Microsoft Windows, Linux | any               | open source (developed via only shell script. fully open source. does not inject blobs on ISOs on runtime.) (note-1) |
...
bir partition OS'ta bir dizinden erişilmek zorundadır. örneğin Microsoft Windows'ta her partition en tepedeki dizinden ayrılır. oysa POSIX'lerin tümünde istenilen dizinden istenilen partition'a erişim sağlanır. bu bağlama işlemine "mounting" adı verilir ve bağlanan dizin adresine de "mounting point" adı verilir.
