...
Fakat sadece DVI dendiğinde de bir standart var. Yani bu türevlerden biri olmak zorunda diil.
...
Bazen bir protokol, eski sürümlerini dahi tunneling üzerinden sağlayabilir. native sağlamak zorunda diildir.
...
- eğer 3D ve/veya surround ses çıktısı istiyorsak aşağıdakilerin hepsi şarttır:
  ...
  - ses dosyasının formatının farklı channel'ları desteklemesi (örneğin MP3 kullanıyorisek, MP3 formatının 7.1 gibi bir sisteme supportu olması)
...
Quantum bilgisayar ile klasik bilgisyarların farkı sadece processing tarafında farklıdır. yani kuantum bilgisayarın sadece CPU ve/veya gpu'su farklıdır. fakat ona adapte olan cihazlarında uyumlu olması gerekebilir.

quantum bilgisayarlar yapısı gereği her algoritmayı çözemiyor. ve maliyetli cihazlar olduklarından sadece ihtiyaç duyduğumuz kod bloklarını orda çalıştırıyoruz.

quantum cihazda çalışan programlama dillerinin ve kütüphanelerin buna göre tasarlanmış olması gerekiyor.

klasik cihazlarda, simulatorler aracılığı ile gerçek quantum cihazında olmadan test çalışmaları/kod denemeleri yapılabiliyor.
...
ACPI Specificationundaki ismi: __S3__

- __Hibernate (or Suspend to Disk or hazırda beklet)__

ACPI Specificationundaki ismi: __S4__

sleep ile aynı mantıktadır. fakat sistemi tamamiyle kapatabilmek için RAM'deki bilgileri harddiskte saklar. açılması zaman almaktadır.
...
Bu modda sistem hem sleep hem suspend moduna geçer. Eğer sistemin şarkı bitmemiş ise (yani kapatılmamış ise), sleep gibi hızlı açılıyor. Eğer kapatılmışsa yavaş açılıyor.

- __Connected Standby (or InstantGo or Suspend to idle or Modern Standby)__

MacOS'te ismi: __Power Nap__

ACPI Specificationundaki ismi: __S0ix (or Low Power S0 Idle)__. Bu state'in sub-state'leri var: S0i1, S0i2 and S0i3. (Sub-state'lerin farklılıkalrını bilmiyorum)

mobile cihazlarda görülen bir yöntemdir. masaüstlerine çok sonradan gelmiştir. sleep ile aynı mantıktadır. tek farkı, ek olarak; isteyen uygulamalar internet ağını kullanacak kod bloklarını sürekli arkaplanda çalıştırabilmeleridir.
...
bootloader, ismi gereği, çok genel bir terimdir/ifadedir. Hem MBR'nin içindeki yazılım, hem de MBR'nin execute ettiği sistemede aslında bootloader denilebilir. çünkü bir yazılımı/sistemi boot/load ediyorlar. bu sebeple bazı insanlar, günümüzdeki sistemelere "2 stage bootloader" olarak isimlendiriyor:
...
MBR çok ufak oldugundan, tüm detaylari üzerinde tutmak mümkün değildir. bu sebeple her mantıksal ve birincil bölümün başında (fiziksel olarak o dosya sisteminin içindeki başlangıç sektörlerinde) VBR olur. VBR'ler de MBR'ler gibidir. bölüm bilgilerini daha detayli şekilde tutarlar.

VDR her partition'da olmak zorunda diildir. Genelde boot edilen partition'larda bulunur.
...
- boyutu değişkendir. Örnek boyut: Ortalama 100 MB sadece Microsoft Windows-10 için yeterlidir. Fakat İlerde yapılabilecek ek işletim sistemi kurulumlarının EFI dosyaları için bu bölümün boyutunu yüksek tutmakta yarar var.
...
- "\EFI" ve "\boot" altındaki bazı dosyalar:
  ...
  > \EFI\fedora\BOOTX64.CSV
    ...
    fallback efi executable'ı şudur: \EFI\Boot\fbx64.efi. fallaback şu zaman devreye girer: Eğer \EFI\boot\bootx64.efi dosyasını bazen Linux'lar override eder. bu override edilen dosya eğer execute edilirken hata alırsa, fallback efi'yi çalıştırır. fallback efi, CVS dosyasını bularak, ilili shim.efi'yi execute eder.

  > \boot\vmlinuz-*
  
    vmlinux, Linux çekirdeklerinin executable image'lerine verilen dosya ismidir. fakat bunu gzip gibi tool'larla sıkıştırınca, vmlinuz olarak adlandırılır. vmlinuz'a suffix olarak çekirdeğin sürümü gelmektedir.
  ...
  > \boot\config-*
    ...
    suffix olarak kernel versionunu alır. text dosyasıdır. suffix olarak aldığı kernel'i, local'de derlediği zaman çalıştırdığı environment parametreleri bu dosyada saklar.
  ...
  > \boot\initramfs-*.img
    ...
    __initrd__ dosyası ise Linux çekirdeğini barındırmaz, sadece dosya sistemnini içeriğini barındırır.

  > \boot\System.map-*

    Text dosyasıdır. suffix olarak Linux çekirdeğinin sürümünü alır.

    Burada suffix'i yazan Linux'un boot edildiğinde kullancağı bazı özel key-value'lar vardır.

  > \boot\symvers-*.gz

    bu dosya gz ile sıkıştırılmıştır. içinde sadece 1 adet text dosyası vardır. suffix olarak Linux çekirdeğinin sürümünü alır.

    Linux derlendiğinde bu dosya oluşturulur. daha sonra farklı Linux-modül'leri compile işlemi sırasında, bu dosyadan yararlanılır.
...
- boot: tüm HDD de sadece bir bölümde bu set edilebilir. ilk o bölümün boot edileceği anlamına gelir. MBR sistemlerde çalıştırılacak bootloader'ın olduğu bölümü tenmsil eder.
...
- legacy_boot: GPT yapılı disklerde, BIOS modunda açılma durumuna hazır olmak için, MBR uyumlu botloader'ın bulunduğu bölüme atanmaktadır.
...
GPT veya MBR şemasının içerisinde paritionların OS içerisinde hangi dizin altına mount edileceği bulunmaz. Her OS runtime sırasında mount edilecek dizine kendi karar verir. Her OS defaultta USB'ler için, internal device partition'lar için kendince isimlendirmeler yaparak dizin isimleri yaratır ve onlara mount işlemini gerçekleştirir.
...
fstype: auto olursa, dosya sistmei otomatik algılanır.
