...
Micro-SIM (3FF) - çipin etrafını çok çok ufak bir çerçeve kaplar
...
uydu yörüngesini tespit maliyetli olduğundan, uydu interneti de maliyetli. Bu sebeple araba gibi cihazları internete bağlarken, araba firmaları, mobil telefon hatları firmaları ile anlaşıp, arabaya mobil hat gömmektedir. Bu şekilde 4g gibi teknolojilerle standart internet kullanılmaktadır.
...
- haritalama hizmetleri
  ...
  Uygular ortalama 30 santime'e kadar dünyadan görüntü alabilirler (2020 yılı).
...
`Google Location Services` DB'si sadece sokak sokak gezerek bilgi toplamıyor. bir kullanıcı, hem `Wi-Fi` hem GPS üzerinden konumunu tespit etmişse; aynı `Wi-Fi`'ye bağlı veya aynı çıkış IP'sine sahip olan her cihazı aynı lokasyonda olarak tanımlıyor. yani bir internet kafede yada şirket içerisinde bir kere lokasyon tanımlatmış biri, 1 hafta boyunca tüm herkese lokasyonun sağlanması için yeterli olabilir.
...
Fakat sadece DVI dendiğinde de bir standart var. Yani bu türevlerden biri olmak zorunda değil.
...
- `USB` 4'ün 2 farklı versiyonu var.
...
- `3.0`, `USB Implementers Forum (⟷ USB-IF)` tarafından `3.1 Gen 1` olarak spesifikasyon ismi değiştirilerek piyasaya sunulmuştur. `Kaynak: https://www.kingston.com/tr/usb-flash-drives/usb-30`
- `3.2` devri başladığı zaman hem `3.1 Gen 1` hem de `3.1 Gen 2` sürümlerinin ayrı ayrı yeni versiyonu çıkarılmıştır. Bu yeni versiyonlar eskisine suffix olarak `x2` eklenerek isimlendirilmiştir. Durum böyle olunca eskilerine de `x1` olarak isim verilmiştir. Bunu yaparken de `3.1` yerine `3.2` yapılmıştır (bunun sebebinin hepsinin `3.2` olarak adlandırılmasını sağlamak olmuş. karışıklığı gidermek için. fakat daha çok karışmış isimlendirme.) Yani:
  - `3.1 Gen 1`, `3.2` devri başlayınca `3.2 Gen 1x1` olarak da isimlendirilmiştir. https://www.kingston.com/tr/usb-flash-drives/usb-30
...
Not: Yukarıdaki her connector çeşidi `Type` keyword'ü olmadan da isimlendirilebilir. örnekler:
...
- `https://www.moddiy.com/product_images/uploaded_images/usb-tutorial-connectors.jpg`
- `https://4.bp.blogspot.com/-fBhVhbqSQwY/WUOR-lZ7ODI/AAAAAAAAC80/j_ROzsYX0XkdI0tJC3GPMEW9lvv0TevuACLcBGAs/s1600/all-usb-connector-2.jpg`
...
Kaç amperlik bir güç akışı olacağını simgeden anlaşılmıyor.
...
`USB` gibi spesifik bir fiziksel teknolojiye bağımlı değildir. Herhangi bir fiziksel bağlantı ile çalışabilen bir standarttır.

`OS` ve takılan cihaz `PnP` destekliyor olması şarttır.

`PnP` sayesinde, takılan cihaz, kendini `OS`'a tanıtabilir. Bu şekilde `OS` hangi driver'ı çalıştıracağına karar verebilir.

Fakat `OS` ve cihaz, `PnP` destekliyor diye, o cihaz sorunsuz çalışacak diye bir kaide yoktur. driver'ların nasıl çalışacağına `PnP` karışmaz. `PnP`, sadece cihaz discovery işlemlerini standartlaştırmıştır.
...
bir port, farklı standartları destekleyebilir. belki desteklediği alternatif protocol'lerin hızının birebir sağlayamaz, fakat protocol'ün çalışabilmesini sağlar. bu özelliğe `tunneling` denir.
...
## 📌 Lo-fi (⟷ lofi ⟷ low-fi ⟷ low fidelity) vs High fidelity (⟷ Hi-Fi ⟷ HiFi)
...
Test temel olarak şunu yapıyor: Birbiri arasında sadece text ile mesajlaşabilen 2 taraf var. Soru soran taraf testi yapan taraftır. Soru soran; karşı tarafın bilgisayar mı yoksa gerçek bir insan mı olduğunu anlamaya çalışır. burada sorular tamamen soru soran kişiye kalmış. makalede sorular ile ilgili detay yok.
...
`Normalized Value` istatistik ve veri madenciliğinde kullanılan bir terimdir. Tablodaki `raw value` değerlerinin birimleri çok farklıdır (yukarıdaki örnekte: saat, adet gibi). Bunları son kullanıcının daha kolay ölçeklendirebileceği birimsiz bir sayıya çeviriyoruz. Bu yeni değer `normalized value` oluyor. Örneğin buradaki değerlendirme 100 en kötü 0 en iyi gibi algılanmasını sağlıyor. Buradaki örnek 100 üzerinden oldu, fakat farklı örneklerde başka max ve min değerler kullanılabilir. `raw value`'nun, `normalized value`'ya çevrilme işlemi istatistikte `normalization (⟷ normalleştirme)` denir.

0 ile 100 arasındaki puanlamalar genel olarak insanlar tarafından standart görüldüğü için `norm` olarak kabul edilmiş oluyor.
...
`AArch64 (⟷ ARM64)` is the 64-bit extension of `ARM`.

## 📌 Apple Silicon

`Apple`, laptoplarında `ARM` tabanlı `CPU`'lara geçmiştir. `M1`, `M2`, `M3` Bunların isimleri ve versiyonlarıdır. `Apple Silicon` bu yeni mimariye verilen isimdir.
...
CPU içerisindeki bu bellek bölgeleri L1, L2, L3 gibi adlandırılırlar. L'nin yanındaki sayı ne kadar küçükse o bellek bölümü o kadar hızlıdır.
...
Bluetooth standartlarına ek olarak bazı standartlar geliştiriliyor.  bu şekilde; örneğin; sadece "stereo kulaklıklar" için standart geliştirilmiş oluyor. bu standartlara Bluetooth dünyasında "profil" deniliyor. Her profilinde kendi içinde versiyonları olabilmektedir.
...
## 📌 Smart kart vs manyetik kart vs çipli kart
...
Nanoteknoloji, nano metre seviyelerinde kimyasal bieleşneleri yönetebilmeyi sağlaya teknolojilerdir.

1 nano metre uzunluğu, 3 - 10 atomun ortalama çapına denk gelmektedir.
...
qubit'in state'ini istediğimiz gibi set'leyebiliriz, fakat okuyamayız. okuduğumuzda ya 1 yada 0 okuruz. quantum-un state'i okuduğu anda yok olur. bu state'in yok olma durumuna bu durumda "lost of Quantum coherence" denir.
...
içinde `ALU` bulunana mantıksal işlemleri yapmak için giriş çıkış birimleri olan elektronik çiptir. genel amaçla tasarlanır. dışarı tarafta neye hizmet edeceklerin bir önemi yoktur.
...
sadece kasa tarafı olan mini bilgisayarlara verilen isim. örneğin: `rasperry pi`.  `Arduino` tam olarak bütün bir bilgisayar kasası işlevini yerine getiren modüllerden oluşmadığından (video kart girişi, ethernet girişi gibi) single-board computer grubuna giremez.
...
en az bir bitlik veri tutabilen devredir. dolayısı ile önceki girişlerinde çıkışa etkisi vardır. bir çıkışın devrenin içinden farklı bir elemana bağlanması sonucunda böyle bir mantık devresi ortaya koyulabilmektedir. bu şekilde çıkış (yani bir önceki durum), yeni girişlere etki edecektir.
...
## 📌 Elektromanyetik tayf (⟷ elektromanyetik spektrum ⟷ EMS)
...
Başka bir firmanın satacağı ürün için ekipman veya parça imalat eden firmaya `OEM` denir. örneğin;

- `MS Windows`, `HP` laptop'larında yüklü gelir. Burada `OEM`, `Microsoft` oluyor.
- `Ford` marka bir araba aldığımızda içindeki `CD` player, `sony`'nin oluyor. `sony` burada `OEM` oluyor.

Bazı bilgisayar satan firmalar (örnek `X` firması) `OEM` ürünlerini sadece birleştirerek satar. Bunu yaparken sattıkları ürünün her parçası `OEM` firmalarına ait oluyor. `X` firması bu parçaları toplu satın alırken kutusuz satın alıyor çünkü kutular lojistik ve kargo ve depo maliyetini yükseltiyor. `X` firması, toplu alınan bilgisayar parçaların bir kısmını bazen tek tek de satabiliyor. Bunları sattığı zaman kutusuz satmak durumunda kalıyor.

`OEM`'li bir ürün, yada kutulu bir ürün farklı servis veya destek veya ek parçalar ile birlikte satılabiliyor. Örneğin; daha uzun yada daha kısa garanti süreli, destek hizmeti olmadan, ekipman parçaları olmadan gibi. Fakat asıl ürününü kendisi tamamiyle orijinal'dir.
...
`OLED` cihaz ekranlarında kullanılan bir teknolojidir. Bu teknolojide her pixel bağımsızca parlatılır. Bu sebeple tamamen siyah olan pixel'ler aydınlatılmaya ihtiyacı olmaz. Bu `OLED` ekranlı bir cihazda, sebeple siyah temalı görüntülerde cihazın pil ömründen kazandırır. Tabi bu tek parametre değil. Örneğin hem beyaz hem siyah kullanmak ve bunlar arasında sık geçiş yapmanın daha çok pil tükettiğini de iddia edenler var.
...
- `PLC`'nin içine programımızı attığımızda gelen input'lara göre çıktılara 1 ve 0'ları yönlendirecektir. Girişlerdeki clock pulse'a göre her daim yazdığımız kodlar tekrar tekrar işletilecektir.
...
- `PLC`'ler özellikle otomasyon sistemlerinde kullanıldığından titreşimlere, darbelere, ve sıcak soğuk gibi hava koşullarına dayanıklı üretilmektedirler. En azından Arduino gibi cihazlara göre çok daha dayanıklı.

### 📌📌 programlama dilleri

birçok yöntem ile programlama yapılabilir:

- `Ladder Diagram (⟷ Merdiven Diyagramı)`
...
- __sleep (⟷ Stand By ⟷ Suspend ⟷ Suspend to RAM ⟷ uyku)__ ile `OS` tüm uygulamaları pause eder ve sistemin az enerji harcamasını sağlar. sistem kapanmaz. bu sebeple saniyeler içerisinde devam edebilir.

`ACPI` Specification'undaki ismi: __S3__

- __Hibernate (⟷ Suspend to Disk ⟷ hazırda beklet)__

`ACPI` Specification'undaki ismi: __S4__

`sleep` ile aynı mantıktadır. fakat sistemi tamamiyle kapatabilmek için RAM'deki bilgileri hard disk'te saklar. açılması zaman almaktadır.

- __Hybrid Sleep (⟷ Hybrid suspend ⟷ Suspend to both)__

`MacOS`'te ismi: __Safe Sleep__

`MS Windows`, 8 ve sonrası sürümlerde `shut down` işlemi ile `Hybrid Sleep` uygulamaktadır. `MS Windows`'ta gerçek bir sistem kapanması için "restart" işlemi yapılmalıdır veya `MS Windows` ayalarında `fast startup (⟷ Fast Boot)` isimli bu özellik devre dışı bırakılabilir.

Bu mode'da sistem hem `sleep` hem `suspend` moduna geçer. Eğer sistemin şarkı bitmemiş ise (yani kapatılmamış ise), `sleep` gibi hızlı açılıyor. Eğer kapatılmışsa yavaş açılıyor.

- __Connected Standby (⟷ InstantGo ⟷ Suspend to idle ⟷ Modern Standby)__

`MacOS`'te ismi: __Power Nap__

`ACPI` Specification'undaki ismi: __S0ix (⟷ Low Power S0 Idle)__. Bu state'in sub-state'leri var: S0i1, S0i2 and S0i3. (Sub-state'lerin farklılıklarını bilmiyorum)

mobile cihazlarda görülen bir yöntemdir. masaüstlerine çok sonradan gelmiştir. sleep ile aynı mantıktadır. tek farkı, ek olarak; isteyen uygulamalar internet ağını kullanacak kod bloklarını sürekli arka planda çalıştırabilmeleridir.

`MacOS`'te eğer cihaz fişe takılı ise, o zaman yüksek `CPU` gerektiren işleri de yapmaya devam etmektedir (Bu durum `MS Windows`'ta da böyle mi bilmiyorum)

## 📌 ACPI Specification

| Global state (Gx) | Name           | Sleep state (Sx) | Description                                            |
|-------------------|----------------|------------------|--------------------------------------------------------|
| G0                | Working        | S0               | The computer is running                                |
| G1                | Sleeping       | S0ix             | Explained above                                        |
| G1                | Sleeping       | S1               |                                                        |
| G1                | Sleeping       | S2               |                                                        |
| G1                | Sleeping       | S3               | Explained above                                        |
| G1                | Sleeping       | S4               | Explained above                                        |
| G2                | Soft off       | S5               | System is powered down.                                |
| G3                | Mechanical Off |                  | power has been totally removed via a mechanical switch |
...
- UEFI sonrası, BIOS uyumlu kurulmuş bir disk üzerinde OS'u başlatma:

> UEFI ----(HDD)----> MBR ----> bootloader ----> OS

## 📌 Uyku ve diğer özel kapanma metotları sonrası tekrar açılan cihazda boot süreçleri nasıl değişiyor

2 farklı şekilde işliyor olabilir:

- standart boot süreci işletilir
- veya özel olarak kapatılan OS, direk olarak tetiklenir (devam ettirilir).

Hangi sürece tabi olduğumuz bu durumlara göre değişir:

Bazı donanımlar bazı özel kapanma işlemlerini native olarak destekliyor. Bu sebeple eğer donanım "Hibernate" destekliyor ise, ve OS öyle kapatılır ise, o zaman OS tekrar açıldığında standart boot süreci işletilmez. Direk OS tetiklenir (devam ettirilir). Ama eğer donanım Hibernate desteklemiyorsa şu 2 durum olabilir:

- OS Hibernate'i sanal olarak destekliyorsa; OS, Hibernate sürecini donanımdan bağımsız yönetir. Yani; cihaz tekrar açıldığında, standart boot süreci tetiklenir. Çünkü donanımın Hibernate yapıldığından haberi yoktur. OS kendi içinde RAM'deki bilgileri bi yere kaydedip kapatır kendini. Açıldığında o bilgileri dosyadan okuyup açılır.
- OS Hibernate'i sanal olarak desteklemiyorsa; OS, Hibernate işlemini son kullanıcıya hiç göstermez.

## 📌 MacOS boot işlemi

Apple, EFI'yi ilk kullanan şirketlerden biri. 2005 yılında, Apple Intel `CPU`'lara geçiş yaptığında klasik BIOS yerine EFI kullanmayı tercih etmişti. Yıllar sonra, UEFI spesifikasyonu sayesinde tüm PC'lerde UEFI kullanılmaya başlandı ve bu bir standart haline geldi. Mac'lerde şu an Apple'ın kendine göre uyarladığı bir EFI bulunmakta. Mac'lerdeki firmware tıpkı normal PC'lerde olduğu gibi UEFI mode'da OS boot edebilir.
...
UEFI'nin 'BIOS compatibility mode'una verilen özel isim. UEFI devre dışı bırakılamaz. UEFI MBR-tarzı bölümlü HDD'lerden OS açabilir. Bu da UEFI'nin geriye uyumlu çalıştığı anlamına gelir. Fakat CSM, her UEFIde olmak zorunda değildir.
...
- HDD ile başlatılırsa; HDD'nin ilk sektörünü okur. Burada okuduğu kısmi execute eder. Execute edilen bu bölge MBR (Master boot record) olarak adlandırılır.
...
- Master Partition Table --> HDD için bölümlerin haritası. en çok 4 bölüm tutabilir. bu sebeple 4 bölümden (__birincil bölüm ⟷ primary partition__) fazlaya ayırmak gerekirse, hard disk'i "__genisletilmis bölüm (⟷ extensible partition ⟷ extended partition)__" özelliğinden yararlanılır. genisletilmis bölüm en fazla 1 adet olabilir. fakat altında sinirsiz bölüm barındırabilir. altında olan her bölüme "__mantıksal bölüm (logical partition)__" denir.
...
- MBR tarafından execute edilebilen bir sistemdir. bu sistem ile isletim sistemleri başlatılır.

- her isletim sisteminin kendi bootloader dosyalarını bir yerde saklaması gereklidir ki, MBR o dizini okusun ve execute etsin.

- bu dizin Ubuntu'da varsayılan olarak /boot/grub'tur. bu dizinin Ubuntu'da, baslatılacak islemim sistemi ile ayni bölümde olma şartı yoktur. örneğin /boot/grub, /dev/sda9'da iken, Ubuntu'nun tüm dosyaları /dev/sda5'te olabilir. eger bölümleri ayırırsak, Ubuntu'yu sildigimizde Grub sistemi de silinmeyecegi için HDD'de kurulu diğer OS'ları hala başlatılabilir olacaktır. fakat Grub, Ubuntu ile ayni partition'da olursa, Ubuntu partition'u silindiginde (yani formatlandığında), Grub başlamayacağı için HDD'de kurulu diğer OS'ları de başlamayacaktır.

- MS Windows'un bootloader'ı "MS Windows Boot Manager (BOOTMGR)" olarak isimlendirilmektedir. Dosyaları; "System Reserved" isimli partition'da durmaktadır.

- herhangi bir OS HDD'ye kuruldugunda, MBR dizini üzerinde degisiklik yapar. bu degisiklikler:

  - MBR içine hangi bölümün master boot (active) oldugu yazılır. yani; hangi bölümdeki bootloader'in başlatılacağının adresidir. (her partition altında bootloader olabilir, ama sadece 1 tanesi başlayacaktır). OS kurulumunda yeni bir bootloader kurulmayacak ise bu işlemin yapılmasına gerek yoktur.

  - GRUB dosyaları EXT gibi dosya sistemlerinde olabilir. Fakat MBR, EXT okuyamaz. Bu sebeple "stage-1.5" vardır. MBR nin ucunda köşesinde biraz daha boş fiziksel alan vardır. GRUB yazılımı, bu boş alana, EXT okuyan yazılımı kurar. Böylece:

    - stage 1: MBR
    - stage 1.5: EXT reader
    - stage 2: GRUB bootloader

    Bu stage'lere "3 stage" denildiği için kafa karışıklığı olabilmektedir.

    GRUB2 ile birlikte stage-1.5 basit olması amaçlı kaldırılmıştır.

- MS Windows, her kurulumdan sonra sürekli MBR'yi kendi bootloader'ına yönlendirir. MS Windows'un bootloader'ı, diğer OS'ları tanımaz (veya tanısa bile çok zahmetlidir). bu sebeple her MS Windows kurulumu sonrası, Linux ve diğer OS'lar ile Grub'u tekrar kurmak gerekmektedir.

## 📌 fixmbr

MS Windows'un komut satırı uygulaması. bu komut ile MBR tamamen sıfırdan değiştirilmektedir. fixmbr komutu sonrası MS Windows kendi bootloader'ına yönlendirme yapacak şekilde MBR'yi yeniden yazmaktadır.

## 📌 ntldr

MS Windows NT için bootloader'dır. günümüzde tamamen yeniden tasarlandığı için artık kullanılmamaktadır. Yeni tasarım MS Windows Boot Manager (BOOTMGR) üzerinedir. BOOTMGR başladığında eski bir MS Windows sürümü seçilirse, BOOTMGR, ntldr'yi başlatmaktadır.

## 📌 GRUB (⟷ GRand Unified Bootloader) vs GRUB2
...
- __EXTLINUX__
SYSLINUX fork'udur. daha fazla `file system` desteklemektedir: FAT12/16/32, NTFS, ext2/3/4, Btrfs, XFS, UFS/FFS.
...
bootloader, ismi gereği, çok genel bir terimdir/ifadedir. Hem MBR'nin içindeki yazılım, hem de MBR'nin execute ettiği sisteme de aslında bootloader denilebilir. çünkü bir yazılımı/sistemi boot/load ediyorlar. bu sebeple bazı insanlar, günümüzdeki sistemlere "2 stage bootloader" olarak isimlendiriyor:

- 1'inci stage: __MBR-loader (⟷ MBR-bootloader)__

  MBR bootloader'ının execution'u.

- 2'inci stage: __OS-loader (⟷ OS-bootloader)__

  GRUB, BOOTMGR gibi yazılımların execution'udur.

## 📌 UEFI (⟷ Unified Extensible Firmware Interface ⟷ old-name:EFI)

Yeni gelen bir sistemdir. BIOS'a alternatif bir firmware'dir. UEFI her konuda çok daha gelişmiştir diyebiliriz. Sadece süreç olarak bakıldığında; MBR gereksinimi ortadan kalkmıştır. Bakınız: "özet boot süreci".

Bazı UEFI Arayüzlerinde "BIOS" kelimesi görülebilmektedir. Bunun sebebi son kullanıcının kafasının karışmaması ve/veya UEFI'nin MBR tabanlı sistemleri de geriye uyumlu desteklemesidir.

## 📌 UEFI boot manager

Bağlı olan tüm diskler içindeki tüm başlatılabilir sistemlerin (OS, bootloader, MBR) listesini tutar ve bunlara ekleme/çıkarma/değişiklik yapılmasını sağlar. kısmen Grub'un görevini görmektedir diyebiliriz. listenin DB anakartın içindedir. Bu listedeki öncelik sırasına göre, anakartı üzerinde olan UEFI ilgili sistemi başlatacaktır.

Bu liste dışarıdan düzenlenebilmektedir. Örneğin Linux'ta "efibootmgr" uygulaması bunu yapıyor. Her OS, kurulumunda bu listeye ekler yada siler yada düzenler. Aynı şekilde anakarttaki UEFI yazılımı GUI'si ile son kullanıcı manuel de ekleme/silme/değiştirme yapabilir.

Listenin içinde BIOS-uyumlu bölümlerde olabilir. Çünkü UEFI, geriye uyumlu; yani; BIOS uyumludur.

örnek bir "efibootmgr" komut satırı çıktısı (her satırın açıklaması aşağısında yazıyor):
...
HDD'yi aç anlamına geliyor. özellikle partition belirtmemiş. bu durumda UEFI native mod'da HDD yi açar. GPT bölümlü olan bu HDD içerisinde, UEFI partition'unu bulur. "Fallback path mode (⟷ fallback mode)" denir buna. yani; native UEFI mode (BIOS değil) fakat HDD karar veriyor hangi partition'un açılacağına. detaylar için ESP ile ilgili başlıklarına bakınız.
...
HDD yi aç der. özellikle partition belirtmemiş. fakat BIOS uyumlu olsun demiş. bu durumda HDD içerisinden MBR yi okur eskisi gibi. Yukarıdaki Boot0001'da, UEFI geri uyumlu mode'da olmadığı için GPT'yi okumuştu. Burada ise MBR okunacak.

Bazı bilgisayarlarda efibootmgr ile yapılan işlemlerde hata almasak da, yapılan işlemin etki etmediğini görebiliriz. bunun sebebi boot option'larının cihaz tarafından hardcode tanımlanmış olmasıdır. böyle durumlarda; boot order'ı takip edip, /EFI altında istenmeyen dizinlerin önüne saçma prefix'ler verilebilir. yukarıdaki örnekten gidelim: boot order 0003 opensuse'yi açacak. eğer opensuse'yi istemiyorsak ve efibootmgr yazılımı etki etmiyor ise; /EFI/opensuse dizinini silebiliriz yada /EFI/_opensuse olarak rename ederiz. bu şekilde yedeğini almış oluruz.

## 📌 Secure boot

- `UEFI` ile gelen bir özellik. `UEFI` ayarlarından manuel olarak devre dışı bırakılabilmektedir. 

- açılacak olan bootloader'ının anahtarının, `UEFI` içerisindeki anahtarlarla uyuşması gerekiyor.

- `Secure boot` ile sadece şunu engellemiş oluruz:
  - `OS` çalışırken kötü niyetli bir yazılım, `HDD`'mizdeki bootloader'ı değiştirdi. O zaman `OS` restart ettiğimiz zaman `UEFI` bu bootloader'ı çalıştırmayacaktır.

- Makine ile kurulu gelen bootloader'ın anahtarı, UEFI içerisine kaydedilmiş oluyor.

- `Microsoft`, `MS Windows`'un `private key`'i ile farklı `OS` kuruluşlarının (örnek `Fedora`) `bootloader` dosyalarını imzalar. Bu imzalanmış dosyaları Fedora gibi firmalar kullanır. Böylece `Microsoft`'un, verify etmek istediği tüm `OS`'lar, `Microsoft` sertifikasının `public key`'ini bulunduran tüm `UEFI`'lerde açılır.

## 📌 UEFI based bootloaders

`UEFI` firmware, direk olarak bootloader'ı çalıştırır. `BIOS` tabanlı sistemlerde ise, `MBR` bootloader'ı çalıştırırdı. `UEFI` çok daha gelişmiş oluğundan execute ettiği bootloader'da daha gelişmiştir. Farklı executable dosyası çalıştırırlar. Bu sebeple bootloader'larda kendilerini güncellemişlerdir. Eğer HDD içerisinde sadece UEFI'nin execute edebileceği bootloader var ise; aynı `HDD`, `BIOS`'lu anakartlarda işletilemeyecektir.

## 📌 Volume Boot Record (⟷ VBR ⟷ volume boot sector ⟷ partition boot record ⟷ partition boot sector)

MBR çok ufak olduğundan, tüm detayları üzerinde tutmak mümkün değildir. bu sebeple her mantıksal ve birincil bölümün başında (fiziksel olarak o `file system`'ın içindeki başlangıç sektörlerinde) VBR olur. VBR'ler de MBR'ler gibidir. bölüm bilgilerini daha detaylı şekilde tutarlar.

VDR her partition'da olmak zorunda değildir. Genelde boot edilen partition'larda bulunur.

## 📌 GUID Partition Table (⟷ GPT)

- `UEFI`'nin tanıdığı, MBR'ye göre çok daha gelismis bir bölüm tutma şemasıdır.

- eski `BIOS` (`MBR`) uyumlu bölümlendirmelere `ms-dos partition table` adı verilir.

- GPT bilgileri diskin en başındadır. GPT olan bir HDD'de MBR'de olabilir. MBR bölümü bırakılırsa, geriye uyumluluk sağlanmış olur.

- GPT olarak tasarlanmış bir diski makineye taktığınızda BIOS anakartlar ile buradaki OS'ları başlatamazsınız. Fakat burada yanlış anlaşılma olmasın: Sadece OS boot edemezsiniz. Onun dışında, bir OS yürütülür durumdayken, GPT bölümlenmiş HDD'yi mount edip, hem MS Windows hem de diğer tüm OS'ları ile okuyup yazabilirsiniz (default olarak OS okuyamasa bile üçüncü parti uygulamalarla yapılabilir). Çünkü HDD'yi okuyan bir yazılımdır. BIOS değil. Yani OS boot etmekle, HDD'yi sadece data olarak kullanmak tamamen ayrı kavramlardır.

## 📌 MS Windows recovery partition

- Bu bölüm MS Windows yüklemesi sırasında otomatik oluşturuluyor.

- Bölüm winRE dosyalarını içeriyor. WinRE MS Windows kurtarma işlemlerini yapan bir yazılım.

## 📌 ESP (⟷ EFI System Partition ⟷ EFISYS)

- Diskte var olan OS'ların EFI dosyaları (bootloader'ları) + Bootloader için gerekli sistem sürücüleri (driver) dosyaları bu bölümdedir.

- FAT32 olmak zorundadır.

- boyutu değişkendir. Örnek boyut: Ortalama 100 MB sadece MS Windows-10 için yeterlidir. Fakat ileride yapılabilecek ek OS kurulumlarının EFI dosyaları için bu bölümün boyutunu yüksek tutmakta yarar var.

- "\EFI" dizini altında her OS'un kendi bootloader'ı ayrı dizinlerde mevcuttur. "\EFI\BOOT" dizini HDD "fallback mode" ile yürütüldüğünde devreye girer. Yani; UEFI, hiçbir partition belirtmeden HDD'yi boot ettiğinde, UEFI, ESP bölümünü bulur. Daha sonra içerisinde "EFI\boot\bootx64.efi" (veya standart önceden belirlenmiş farklı bootloader path'leri) dosyasını execute eder. Yani OS'ların bootloader'ları dışında genel bir UEFI arayüzü (bootloader'ı) mevcuttur. Bu bootloader "default bootloader" olarak da adlandırılmaktadır. Bazı UEFI firmware'leri bu bootloader'ı bulamadığı zaman MS Windows'un default bootloader'ını da açmayı denemektedir: "\EFI\Microsoft\BOOT\bootmgfw.efi".

- "\EFI" dizini altında sürücüler ve OS çekirdekleri de mevcut olabilir. EFI dosyası executable'ı olarak Linux çekirdeği çalıştırılabiliyor. Linux güncel sürümleri bunu desteklemektedir. Bu özellik; "__EFI Boot Stub (⟷ EFI Stub)__" olarak isimlendiriliyor.

  Eğer OS çekirdeği EFI modunda executable çalışmayı desteklemiyor ise, bu durumda OS çekirdeği "\EFI" içinde olmaz.

- "\EFI" altında aynı zamanda sürücü (driver) dosyaları bulunmaktadır. Sürücüler: dosya dizinlerine erişim sürücüleri (EXT, NTFS gibi), network kartların daha geniş özelliklerle kullanılabilmesini, bootloader sırasında takılan external USB cihazların tanınması gibi işler için gerekli olabilmektedirler.

- "\EFI" ve "\boot" altındaki bazı dosyalar:

  > \EFI\Microsoft\BOOT\bootmgfw.efi
  
    MS Windows'un boot ettiği efi
  ...
  > \EFI\boot\bootia32.efi
    ...
    "\EFI\boot\bootx64.efi" ile amaçlıdır. IA-32 `CPU`'lar için geliştirilmiştir. IA-32, 32-bit INtel cihazların mimarisini temsil etmek için kullanılan bir terimdir.

  > \EFI\fedora\shim.efi
  
    fedora secure boot ile başlatılamaz ise diye, onun önünde 3üncü parti bir efi var. shim secure boot'u aşmayı sağlıyor ve farklı bir efi'yi execute ediyor.

  > \EFI\fedora\grubx64.efi
  
    grub EFI'si. secure boot deactivate ise, bu direk boot edilebilir. eğer secure-boot var ise, önce shim boot edilmeli, shim otomatik olarak grub'u boot edecek.

  > \EFI\fedora\shim*.efi
  
    shim'in farklı sistemler için tasarlanmış efi'leri. örnekler: shimppc64 for PowerPC, and shimaa64 for ARM...

  > \EFI\fedora\grub.cfg
  
    text dosyası. grub.efi execute edildiğinde, burada grub için gerekli user-editable-config dosyasının nerede olduğu yazmaktadır. burada hard disk, partition ve path bilgisi mevcuttur.
  ...
  > \EFI\fedora\BOOTX64.CSV
    ...
    Shim geliştiricileri başka EFI'lerden Shim dosyasını bulan 'fallback.efi'ler tasarladı. 'fallback.efi'ler, '\EFI' altındaki tüm BOOT{$ARCH}.CSV dosyalarını ararlar. uygun olan CSV dosyasını okur ve içinde yazan efi'yi execute eder. Örneğin \EFI\fedora\BOOTX64.CSV dosyasının içinde sadece 1 satır vardır:
  
    > shimx64.efi,Fedora,,This is the boot entry for Fedora

    Bu dosyadan hangi EFI'yi koşacağını anlarlar. koşulan efi dosyası shim projesine bağlı bir efi dosyasıdır.

    fallback efi executable'ı şudur: \EFI\Boot\fbx64.efi. fallback şu zaman devreye girer: Eğer \EFI\boot\bootx64.efi dosyasını bazen Linux'lar override eder. bu override edilen dosya eğer execute edilirken hata alırsa, fallback efi'yi çalıştırır. fallback efi, CVS dosyasını bularak, ilgili shim.efi'yi execute eder.

  > \boot\vmlinuz-*
  
    vmlinux, Linux çekirdeklerinin executable image'lerine verilen dosya ismidir. fakat bunu gzip gibi araçlarla sıkıştırınca, vmlinuz olarak adlandırılır. vmlinuz'a suffix olarak çekirdeğin sürümü gelmektedir.

  > \boot\\.vmlinuz-*.hmac
  
  text dosyasıdır. içerisinde aynı dizinde olan ve aynı isimde olan vmlinuz-* dosyasının hash'i yazmaktadır.

  > \boot\vmlinuz-0-rescue-*

    vmlinuz ile aynı işlevi görür. fakat bu çekirdek image'sinde recovery amaçlı özel ayarlar yapılmıştır.

  > \boot\config-*

    suffix olarak kernel versiyonunu alır. text dosyasıdır. suffix olarak aldığı kernel'i, local'de derlediği zaman çalıştırdığı environment parametreleri bu dosyada saklar.

  > \boot\initramfs-*.img

    Suffix olarak Linux'un sürümünü alır.

    "initrd (kısaltma: initial ramdisk)" Linux başlatmanın özel tekniğidir. Linux ilk başlatıldığında, RAM'de oluşturulan geçici bir `file system`'dan başlatılır. Bu tekniğin özel sebepleri var (burada açıklamaya gerek yok).

    __initramfs__ dosyası bu RAM'den açılacak `file system`'ın image'sini + Linux çekirdeğini barındırır.

    __initrd__ dosyası ise Linux çekirdeğini barındırmaz, sadece `file system`'ın içeriğini barındırır.

  > \boot\System.map-*

    Text dosyasıdır. suffix olarak Linux çekirdeğinin sürümünü alır.

    Burada suffix'i yazan Linux'un boot edildiğinde kullanacağı bazı özel key-value'lar vardır.

  > \boot\symvers-*.gz

    bu dosya gz ile sıkıştırılmıştır. içinde sadece 1 adet text dosyası vardır. suffix olarak Linux çekirdeğinin sürümünü alır.

    Linux derlendiğinde bu dosya oluşturulur. daha sonra farklı Linux modülleri compile işlemi sırasında, bu dosyadan yararlanılır.

  > \boot\efi\mach_kernel

    Mac tabanlı sistemler (Bazıları veya tüm cihazlar) bu partition'da boot edilecek bilgi olup olmadığını bu dosyaya bakarak anlıyor. Mac olmayan makineler'e Fedora dahi kursak, bu dosyayı HDD'ye yüklüyorlar. Sebebi muhtemelen diskin çıkarılıp Apple cihaza takılma ihtimalinde, diskin boot edilebilmesini sağlamak.

  > \boot\efi\System\Library\CoreServices\SystemVersion.plist

    text dosyasıdır. \boot\efi\mach_kernel ile aynı durum söz konusudur. Bu dosyada çalışacak sistem hakkında isim gibi 1-2 bilgi bulunur.

## 📌 MSR (⟷ Microsoft System Reserved Partition ⟷ Sistem ayrıldı bölümü)

- sadece GPT disklerde oluşan bir bölümdür.

- ESP'den sonra ve MS Windows kurulu bölümden önce olmak zorundadır.

- bu partition'da `file system` yoktur.

- Boyutu MS Windows sürümü ve disk boyutuna göre değişebilir.

- Bootmgr bu bölümde yüklüdür.

## 📌 Redundant Array of Inexpensive Disks (⟷ Redundant Array of Independent Disks ⟷ RAID)
...
- RAID kendi içerisinde birçok özelliği vardır: RAID0, RAID1 gibi... Bunlardan sadece bir tanesi seçilmek zorundadır.
  ...
  - RAID1: 2 diskimiz var. 2 disk tek bir disk gibi gösteriliyor. 2 tarafta da aynı bilgiler tutuluyor. Bu şekilde birine bir zarar geldiğinde diğerinden devam edilebiliyor. Performans açısından kötü.

- Sadece donanımsal yada sadece yazılımsal çözümlerle sağlanmaktadırlar.

## 📌 Flag (Bayrak)

`GPT` ve `MBR` her partition için bir bayrak bulundururlar. Bayrakların birer anlamı vardır.

- `boot`: tüm `HDD` de sadece bir bölümde bu set edilebilir. ilk o bölümün boot edileceği anlamına gelir. `MBR` sistemlerde çalıştırılacak `bootloader`'ın olduğu bölümü temsil eder.

- `Diag`: o bölümün diagnostics/recovery amaçlı kullanıldığını belirtir.

- `Hidden`: `MS Windows` otomatik mount edilmesini istenmediği bölümlere bu bayrağı atıyor.
...
- `msftdata`: `MS Windows` kendi oluşturduğu dizinlere bunu atıyor. bazı Linux sistemleri de kendi dizinlerine bu flag'i attığı görülüyor.

- `Swap`: `HDD` üzerinde kurulu tüm `Linux` sistemler bu bölümü swap alanı olarak kullanabileceklerini belirtir.

- `BIOS_GRUB`: `GPT` Kurulu bir HDD'de, BIOS modunda açılma durumuna hazır olmak için, sadece "Grub legacy" için gerekli çok ufak bir alandır. bu alanda GRUB legacy dosyaları yoktur. bu alan çok özel ve istisna bir durum için gereklidir. teknik detaya inilmesine ihtiyaç duymadığım bir konu.

- legacy_boot: GPT yapılı disklerde, BIOS modunda açılma durumuna hazır olmak için, MBR uyumlu bootloader'ın bulunduğu bölüme atanmaktadır.

## 📌 Partition name vs partition label

- `name`, diskin `GPT` alanında yazılı olan bir etikettir.

- `label` ise partition'un içinde (`file system`'ın içinde) kayıtlı bir etikettir.

## 📌 /etc/fstab file

GPT veya MBR şemasının içerisinde partition'ların OS içerisinde hangi dizin altına mount edileceği bulunmaz. Her OS runtime sırasında mount edilecek dizine kendi karar verir. Her OS, default'ta USB'ler için, internal device partition'lar için kendince isimlendirmeler yaparak dizin isimleri yaratır ve onlara mount işlemini gerçekleştirir.

OS'un default ayarlarını ve isimlendirmesini partition bazlı override etmek için /etc/fstab dosyasını düzeltmeliyiz. Bu bir text dosyasıdır:

```text
# this line is comment
# <device>      <mount_point>  <fstype>  <options>  <dump>  <fsck_order>

/dev/sda1       /              ext4      defaults   0      1

/dev/sdb1       /mnt/data      btrfs     defaults   0      2

UUID=xxxx-xxxx  /mnt/backup    ext4      noatime    0      2
```

fsck_order: fsck kontrolündeki sırasını belirtir. bunu 0 verirsek fsck kontrolü yapılmaz.

dump: Bu alan, `file system`'ın dump komutuyla yedeklenip yedeklenmeyeceğini belirtir.

fstype: auto olursa, `file system` otomatik algılanır.

## 📌 parted

Linux üzerinde çalışan, libparted kütüphanesini kullanan, disk yönetimi sağlayan komut satırı yazılımıdır.

## 📌 Gparted

- Linux üzerinde çalışan, GUI içeren bir disk yönetim aracıdır. libparted kütüphanesini kullanır.

## 📌 fdisk

"Parted" uygulaması ile aynı görevi gören komut satırı uygulaması. fakat parted GPT tabanlı diskleri desteklerken, fdisk desteklemez.
...
- anahtar işaretli bir bölüm üzerinde işlem yapmak için o bölüm önce unmount edilmelidir. eğer ilgili bölüm, çalıştırılan OS dosyalarını içeriyor ise unmount edilemez. böyle bir durumda ilgili bölüm ancak canlı cd üzerinden, yada aynı HDD üzerinde farklı bir bölümdeki OS çalıştırılarak yapılabilir.

## 📌 bootable USB creators

| name                                          | runs on                                           | supported `ISO` | note                                                                  |
|-----------------------------------------------|---------------------------------------------------|-----------------|-----------------------------------------------------------------------|
| `Unetbootin`                                  | cross-platform                                    | `Linux`-based   | open source. not developing since long time.                          |
| `Rufus`                                       | `MS Windows`                                      | any             | open source                                                           |
| `WinToUSB`                                    | `MS Windows`                                      | `MS Windows`    | closed source                                                         |
| `WoeUSB`                                      | `Linux`                                           | `MS Windows`    | open source                                                           |
| `MkUSB`                                       | `Linux` (command line)                            | `Linux`         | ?                                                                     |
| `Etcher`                                      | any                                               | any             | (read below)                                                          |
| `LinuxLive USB Creator`                       | `MS Windows`                                      | `Linux`         | writes open source but code is not available                          |
| `YUMI (⟷ Your Universal Multiboot Installer)` | `MS Windows`                                      | any             | multi ISO on one USB feature. source code is available on `Zip` file. |
| `UUI (⟷ Universal USB Installer)`             | `MS Windows`                                      | any             | source code is available on `Zip` file.                               |
| `USB-creator (⟷ Startup Disk Creator)`        | `Ubuntu`, `MS Windows`                            | `Ubuntu`        | `Ubuntu`'s official app                                               |
| `MS Windows Media Creation Tool`              | `MS Windows`                                      | `MS Windows`    | `Microsoft`'s official app                                            |
| `MultibootUSB`                                | `MS Windows`, `Linux`                             | any             | open source. only shell script. injects blobs on `ISO`s on runtime.   |
| `Ventoy`                                      | cross-platform                                    | any             | open source. injects blobs on `ISO`s on runtime.                      |
| `Fedora Media Writer`                         | `Linux` via `Flathub` only, `MacOS`, `MS Windows` | `Fedora`        | open source. `Fedora`'s official app.                                 |

## 📌 dd command

bu komut `ISO` dosyasını direk olarak `USB`'ye yazar. Bununla da bootable `ISO` yapılabilir. örneğin bu komut ile, `Fedora` ve `Ubuntu` sorunsuz şekilde bootable yaplabiliyor. Fakat `MS Windows` ve bazı `OS`'lar `ISO` kopyalanmasından sonra ekstra işlemler gerektirebiliyor. Bu sebeple `dd` komutu onlar için yetmiyor.

`gnome-disk-utility`'nin `restore disk image` özelliği ve `balena-etcher`, `dd` komutu gibi sadece diske yazma işlemi yapıyorlar. Bu sebeple `MS Windows` destekleyemiyorlar.

`balena-etcher`, `dd` komutuna ek olarak şunlar sunuyor:
- cross-platform
- easy UI
- verify
...
`Etcher`, isim değişikliği ile `Balena-etcher (⟷ BalenaEtcher)` olmuştur.

## 📌 MS Windows To Go

`MS Windows` 8 ile gelen bir özellik. sadece enterprise edition'larda mevcut. artık `MS Windows` `USB`'ye de kurulabiliyor.
...
bir partition OS'ta bir dizinden erişilmek zorundadır. örneğin MS Windows'ta her partition en tepedeki dizinden ayrılır. oysa POSIX'lerin tümünde istenilen dizinden istenilen partition'a erişim sağlanır. bu bağlama işlemine "mounting" adı verilir ve bağlanan dizin adresine de "mounting point" adı verilir.
...
- quiet: Linux'un output'a log'lamamasını sağlar. quiet disable bırakılırsa, Linux output'a log'ları basar.

- splash: Linux boot olurken ekranda logo görünmesini sağlar. eğer splash açık ise quiet'ın önemi olmayacaktır. quiet vs splash birlikte kapatılmalıdır ki log'lar ekranda gözüksün.

- nomodeset: ekran kartları ile ilgili sorunlar yaşandığında kullanılan bir seçenektir. sistem başlayana kadar (X (pencere yöneticisi) başlayana kadar), video sürücülerinin devre dışı bırakıp kernel'deki video mode'unun kullanılmasını sağlar. nomodeset; __kernel mode setting (⟷ KMS)__yi disable eder.

Bu seçeneklerden birini uygulamak için:

- grup ekranında Linux'u başlatmak için seçeceğimiz satır'da enter tuşuna basmadan önce "e" tuşuna basarız. o satırı düzenlemeyi açar.
...
Ikisi de tamamen aynı görevi görüyor. update-grub sadece bir wrapper.

Grub menü dosyasını default konfigürasyonlara göre re-generate ediyor.
...
`TianoCore EDK`'nın yeni sürümü. Açık kaynaklı `UEFI` implementasyonudur.

## 📌 GNU-EFI

`TianoCore EDK II`'ye alternatiftir. `TianoCore EDK II` çok daha fazla özelliğe sahiptir.

## 📌 Open Virtual Machine Firmware (⟷ OVMF)

`TianoCore EDK II`'yı kullanarak `VM`'lerin `UEFI` yürütebilmesi için gerekli framework'tür. açık kaynaklıdır.

## 📌 Libreboot

açık kaynaklı `BIOS` yazılımıdır. Opsiyonel olarak içine `bootloader` gömülebiliyor. Böylece `bootloader` direk anakartın içine de gömülebiliyor.

`Libreboot` içinde `bootloader` olarak en sık `GRUB2` tercih ediliyor.

## 📌 payload

bu terim `BIOS` dünyasında kullanılmaz. `Libreboot`, `coreboot` gibi projelerin kendi dökümantasyonunda bu terim kullanır.

`Libreboot` ve `coreboot` direk `Linux` kernel'i veya bootloader run edebilir. İşte bu başlattığı şeye `payload` adını veriyorlar.

Not: Standart bir `BIOS` direk `Linux` kernel'i run edemez.

## 📌 coreboot

`Libreboot` projesi `coreboot` projesinden türetilmektedir. en temel farkı `coreboot` projesinde kapalı kaynak `dependency`'ler olmasıdır.
