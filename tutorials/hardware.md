...
- işletim sisteminin USB driver'ın da sorun olması
...
- herhangi bir işletim sistemi HDD'ye kuruldugunda, MBR dizini üzerinde degisiklik yapar. bu degisiklikler:
  ...
  - MBR içine hangi bölümün master boot (active) oldugu yazılır. yani; hangi bölümdeki bootloader'in başlatılacağının adresidir. (her partition altında bootloader olabilir, ama sadece 1 tanesi başlayacaktır). işletim sistemi kurulumunda yeni bir bootloader kurulmayacak ise bu işlemin yapılmasına gerek yoktur.
...
Bu liste dışarıdan düzenlenebilmektedir. Örneğin Linux'ta "efibootmgr" uygulaması bunu yapıyor. Her işletim sistemi, kurulumunda bu listeye ekler yada siler yada düzenler. Aynı şekilde anakarttaki UEFI yazılımı GUI'si ile son kullanıcı manuel de ekleme/silme/değiştirme yapabilir.
...
- UEFI ile gelen bir özellik. manuel olarak devre dışı bırakılabilmektedir. açılacak olan işletim sisteminin bootloader'ının anahtarının, UEFI içerisindeki anahtarlarla uyuşması gerekiyor.
...
- boyutu değişkendir. Örnek boyut: Ortalama 100 MB sadece Microsoft Windows-10 için yeterlidir. Fakat ileride yapılabilecek ek işletim sistemi kurulumlarının EFI dosyaları için bu bölümün boyutunu yüksek tutmakta yarar var.
...
- "\EFI" dizini altında sürücüler ve işletim sistemi çekirdekleri de mevcut olabilir. EFI dosyası executable'ı olarak Linux çekirdeği çalıştırılabiliyor. Linux güncel sürümleri bunu desteklemektedir. Bu özellik; "__EFI Boot Stub (or EFI Stub)__" olarak isimlendiriliyor.
