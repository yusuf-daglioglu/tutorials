...
`Prometheus` her türlü data'yı çekebilir. Data'nın içeriğine karışmaz ama çekeceği data kendi belirlediği syntax'ta olmalıdır. Okuyacağu data kendi beklediği formatta değilse, `Prometheus` içindeki connector data'yı çektiğinde convert edebilir.

### 📌📌 Prometheus support formats

`Prometheus`'un pull edeceği data, aşağıdaki formatlardan biri olmalıdır.

- `Prometheus Text format (⟷ Prometheus Client Data Exposition Format)`

- `OpenMetrics`

  Hem `protobuf`, hemde saf `string` destekliyor.

  `Prometheus Text format`'a aşırı benziyor. fakat `OpenMetrics` daha modern ve gelişmiş.
...
```text
# HELP ABC description for ABC
# TYPE ABC counter
# any other comment here
key_1{label_1="label_value_1",label_2="label_value_2"} value 123456789
```

- yukarıdaki formatta en sağdaki değer timestamp. bu opsiyoneldir. eğer bu değer yok ise, server bu değeri kendi set etmelidir.

- value değeri sayılsal olmalıdır.

- `HELP` ve `TYPE` ile başlayan yorum satırları standart.

- yukarıdaki formatta `counter`, `ABC` metric'inin tipidir. bazı alternatifler: `summary`, `histogram`.

örnek gerçek çıktı:

```text
# HELP http_requests_total stats for http requests
...
Bu değerler topluca storage tarafında böyle tutuluyor:

- `object storage`
  - time range (örnek: 2 hours)
    - `series` = `metric` + `labels` (örnek: http_requests_total{status="200",env="prod"})
      - timestamp + value
      - timestamp + value
...
`OTel` `metric`'lerini `mimir` yazılımı, `time series` `DB`'ye kaydederken aşırı benzer şekilde kaydeder.

Bunun bir örneği `monitoring_tools_and_actuator_request_examples.md` dosyasında anlatılıyor.
...
| `Mimir`         | yes    | no    | no  | no              | no          | no             | yes. data is distributed.         | `OTLP`                                                |
...
on-prem kurulan `Dynatrace` (ismi `Dynatrace Managed`), `OTLP` desteklemiyor. Önüne ayrı bir binary olan `Dynatrace ActiveGate` kullanmak şart. Fakat `Dynatrace` kendi bulut serverlarında `OTLP`'yi direk destekliyor.
...
- `metric`
- `log`
  - `event`: özel bir log türevidir. parseable log diyebiliriz. örnek bir `event`: 
  
    LOG PAYMENT_FINISHED, 10$, SUCCESS

- `trace`

`profiling` data'sını veritabanında saklamak istersek, yukarıdaki 3 model ile alakası yoktur. Bambaşka bir data tipidir.

`APM` data'sını veritabanında `metric` ve `trace` data'sını ilişkilendirerek tutabiliriz. Yani `UI` tarafının `metric` ve `trace` data'sını ilişkilendirerek araması ve gösterebilmesi yeterlidir.

## 📌 pprof

`profiling` bilgilerini tutan dosya formatıdır.

bu dosya `pprof` uzantısı ile diskte saklanır.

açık kaynaklıdır.

dosya içerisindeki bilgiler `protobuf` ile serialize edilir.

Bu format birçok programlama dilini destekliyor (dilden bağımsız).

## 📌 JFR (⟷ Java Flight Recorder)

`profiling` bilgilerini tutan dosya formatıdır.

sadece `Java` için geliştirilmiş format.

## 📌 OTEL profiling

henüz stable değil (yıl 2026).

aynı diğer `OTEL` `signal`'leri gibi, mesaj ile server tarafa client'tan yollanır. Giden mesajlar, yine `OTEL` standartları gibi `resource` ve `scope` gibi field'ları ile wrap edilerek yollanır.

Wrap edilen bu `profiling` data'sının modeli (`JSON`'daki field isimleri) `pprof` ile aşırı benzerdir.

## 📌 Bazı UI component'leri

### 📌📌 Flame graph

alev şeklindedir. her fonksiyonun ne kadar çalıştığı gösteriyor. alevlerin ucuna gittikte, `stack trace`'deki son çağrılan fonksiyonları görürürüz.
...
`JSON`'daki bazı field'lar zorunlu veya opsiyonel veya tamamen custom olabilir. her `OTel` mesajında şu şekilde bir `JSON` ağacı vardır:


- `Resource`. mesajın bilgisinin hangi kaynağa ait olduğu (`pod`, `user-service` `java` app)

  - `Scope`. mesajı oluşturan kaynak (enstümental library) (1 `resource` altında birden fazla `scope` olabilir. `Spring Web`, `JDBC` ayrı ayrı `scope` olabilir.)

    - `Signal`. aslında iletilmek istenen data'nın kendisi (`metric`, `log`, `trace`) (1 `resource` altında birdren fazla `signal` olabilir.)
...
- `metric`
- `baggage`
  - tüm akışta tüm servisler arasında taşınan key value data. distributed context gibi düşünülebilir.
  - `HTTP` ve `GRPC` isteklerinde header'da `baggage` olarak yollanır.
  - bu bilgi monitoring tool'a export edilmek zorunda değildir (hatta default'ta export edilmez).
