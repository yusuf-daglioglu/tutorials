...
bu listedeki hiçbir yazılım `Linux` kernel modülü değildir.
...
bu listedeki hiçbir yazılımlar `Linux`'un içinde native gelen "`cgroups`" ve `Linux namespaces` özelliklerini kullanarak çalışır. Ek olarak `SELinux` ve `AppArmor` gibi özelliklerden de sıkça yararlanır.

### 📌📌 High-Level Runtime (⟷ Container Manager)

| name         | description                                                                             |
|--------------|-----------------------------------------------------------------------------------------|
| `Containerd` | `Runc`'yi çağırıyor. `ctr` komut satırı client uygulamasıdır. Genel amaçlıdır.          |
| `CRI-O`      | `Runc`'yi çağırıyor. sadece `K8s` için tasarlandı. Bu sebeple bazı yetenekleri kısıtlı. |

### 📌📌 Low-Level Runtime

| name             | description                                                                                                                                           |
|------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------|
| `LXC`            | `Linux Containers`'tan türeyen özel bir isimdir.                                                                                                      |
| `Runc`           | `Libcontainer` kütüphanesini çağırır. Hem kütüphane hem kendisi `Golang` ile yazılmıştır.  `Libcontainer`'ı başka herhangi bir yazılımda çağırabilir. |
| `Katacontainers` |                                                                                                                                                       |
| `Railcar`        |                                                                                                                                                       |
| `Crun`           |                                                                                                                                                       |
| `sysbox`         | `Runc`'nin fork'u. nested container support için geliştirilmiştir.                                                                                    |

Örneğin `Docker`, `sysbox`'ı kullanarak nested container açabilir:

```sh
docker run --runtime=sysbox-runc user-service
```

## 📌 Container engines

`container runtime`ı wrap eden programdır.
...
`container engine` ve/veya `container runtime`, `OCI` ile uyumlu olabilir. eğer uyumlu ise, bu `container`'ları herhangi bir `OCI` uyumlu ortamda saklayabilir ve çalıştırabiliriz.

Örneğin `K8s` varsayılan olarak; `OCI` uyumlu `container`'ları çalıştırır.
...
- `Snappy Ubuntu Core`
- `Bottlerocket` (by `Amazon`)
- `Container-Optimized OS` (by `Google`)
- `CoreOS Container Linux` (based on `Gentoo`) (discontinued)
  - fork: `Flatcar Container Linux`
    - fork: `Azure Container Linux` (only for `K8s`)
- `Fedora CoreOS (⟷ FCOS)`
  - fork: `Red Hat Enterprise Linux CoreOS (⟷ RHCOS)`
- `RancherOS`
- `Project Atomic`
- `Photon`
...
`Docker`'ın kullandığı bazı kısımların açık kaynak altında dağıtılıyor. bu projeye `Moby` adı verildi.

## 📌 union file system (⟷ UnionFS)

`UnionFS`, `file system`'in özel ismidir.

`Union mount` ise dosya sistemlerinde olan bir feature'dir. Bu özellik desteklenen dosya sistemlerinde, bir dosyanın farklı branch'leri (versiyonları) olabilmektedir.

`UnionFS`'e birçok alternatif var. bu alternatifleri `Docker` engine'den seçebiliyoruz. bazı dosya sistemleri tüm dosyayı değilde, dosyayı belli bloklara bölüyor ve sadece değişen kısmın bloğunu farklı versiyona taşıyor. bu tarz farklı yöntemler mevcut.
...
- hazır operatörler bulundurur:
  - `CI/CD` Entegrasyonu: Kaynak koddan imaj üreten `Source-to-Image (⟷ S2I)` akışları.
  - logging operatörü ile `vector` uygulaması her pod'dan direk log'ları toplar.
- Built-in image registry
- resmi ücretli destek
- `imagestream`: istenilen container'ların, istenilen repo server'ına referans edecek şekilde ayarlanabilmesi (alias gibi).
- `Route` nesnesi. bununla direk basit routing işlemleri production ortamında dahi yapılabilir.
- `OCP` node'larının kurulu olduğu `OS`'e de resmi destek veriyorlarlar. gerektiğinde onlara da hızlı yama çıkıyorlar.
- ve daha fazlası.
...
aşağıdaki tüm komut satırı uygulamaları `K8s`'ten ve birbirlerinden bağımsızdır. sadece istediğimiz paketin binary'sini indirip local'de çalıştırabiliriz.
...
komut satırı uygulamasıdır. local'de test amaçlı çalışmalar için geliştirilmiştir. local makinede `VM` açmakta ve içerisinde tek node'lu `K8s` yürütmektedir.
