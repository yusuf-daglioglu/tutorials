...
sistemsel modüller dışında kalan modülleri yöneten node'dur.
...
local'den hangi cluster'ı kullanacağımızı komut satırından belirtmemiz gerekmektedir.

## 📌 Resource

`K8s`'de yaşayan ve `API` üzerinden manage edilen her feature'dır. Örnekler: `Pod`, `Service`.

Bu feature'lerden instance yarattığımızda, bu yarattıklarımız artık birer `Objects`'tir. Örneğin `user-service`'imiz bir `Object`'tir.

## 📌 Custom Resource

`K8s`'de gömülü gelmeyen, `K8s` yöneticisinin yarattığı `Resource`'lerdir. Bunları yaratmak için `CRD (⟷ Custom Resource Definition)` `YML` dosyalarını kullanırız.

## 📌 Object

Obje listesi aşağıda verilmiştir.
...
- `Controllers`
  ...
  sistemi `desired state`'te tutmaya çalışır. yani; `controller`, `YML`'de tanımladığımız `resource`'ları açık tutmak için sürekli kontrol eder ve açık değilse açar.

  - `ingress controller` network routing yönlendirmelerini `desired state`'te tutmaya çalışır.
  - `ReplicaSet` aktif `pod`'ları istenilen seviyede tutmaya çalışır.
  - `Deployment` (başka başlıkta nalatıldı)
  - `StatefulSet` (başka başlıkta nalatıldı)
  - `DaemonSet` (başka başlıkta nalatıldı)
  - `Job` belli sürelerde başlayan ama işi bitince sonlanan `Pod`'lar olması sağlar.
...
`kind: template` ve `spec.template` alakasız şeylerdir.

`kind: template` sadece `OCP`'de olan bir özelliktir. `kind: template` `YML` içine birden fazla bağımsız `K8s` resource'u yazılabilir. Normalde apply edeceğimiz `YML` dodsyasında zate alt alta bağımsız resource'ler yazabiliriz. Ama `OCP` burada bunu parametre alacak şekilde yapmış. Böylece komut satırından veya `GUI`'den sadece high-level parametreleri set edip, tüm `YML`'yi apply edebiliyoruz.
...
- Sadece `Openshift`'e ait olan bir nesnedir.
- kendisi direk implementasyon ile gelir.
- `HTTP` seviyesinde routing'ler yapılabilir.
- Basit özellikler sunar.
- production ortamda kullanılabilir.

## 📌 deployment

`deployment` içerisine birçok `replicaSet` barındırabilen bir yapıdır. Böylece, versiyonlama (güncelleme) yapılabilmesini sağlar.

`YAML` içerisinde `kind: deployment` yazmamız gereklidir.

## 📌 StatefulSet

`deployment` stateless `pod`'lar için, `StatefulSet` stateful `pod`'lar için tasarlanmıştır. bunun pratikteki en belirgin farkları:

|            | `Deployment`                                                      | `StatefulSet`                                                  |
|------------|-------------------------------------------------------------------|----------------------------------------------------------------|
| `Volume`   | Tüm `Pod`'lar aynı `Volume`'u ortaklaşa kullanır.                 | Her `pod` kendine özel, ayrı bir `Volume` açar.                |
| `Pod` ismi | bir `pod` kapanırsa, tekrar açıldığında mutlaka farklı ismi alır. | bir `pod` kapanırsa tekrar açıldığında mutlaka aynı ismi alır. |

`YAML` içerisinde `kind: StatefulSet` yazmamız gereklidir.
...
- `Ingress Controller` ve `OCP`'nin sunduğu `Router` aracılığı ile:

  yeni `Ingress Controller` gibi controller'da `YML` dosyası apply edilir. bu dosya eskisi ile aynıdır. tek fark: hangi label'ları `service`lere yönlendirme yapılacağı belirtilir.

- `deployment`

  eski `deployment` `YML`'mizde sadece image'i değiştirip, apply ettiğimiz zaman, `K8s` otomatik olarak update işlemini gerçekleştirir. dosyayı apply etmeden önce, içinde `strategy` field'ı altında ne çeşit strategy izlemek istediğimiz belirtebiliriz.

Saf `K8s` şunları destekler:

- `RollingUpdate`

  yeni `pod`'ları ayağa kaldırır. `probe` değerleri OK ise eskileri kapatır.

- `Recreate`

  tüm eski `pod`'ları siler. yeni `pod`'ları açar. `probe` diğerlerini önemsemez.

## 📌 operator

şunlardan birden fazla içerebilen yapılara verilen isimdir:
- `controller`
- `custom resource` veya (embeded) `resource`
- logic içerebilir (`pod` çalıştırarak, istediği logic'i herhangi bir executable ile çalıştırabilir)
- `OCP` event'lerine subscribe olabilir.

`kind: operator` diye özel bir tanım yoktur. `ClusterServiceVersion` isminde kullanılır. Ama saf `K8s`'te bunu yöneten modüller yüklü gelmez. Yüklmek için `OLM (⟷ Operator Lifecycle Manager)` isimli modül kurulmalıdır. `OCP`'de bu modül yüklü gelir.
...
`ClusterRole` veya `Role`, permission grubunu belirten `YML` dosyasıdır. `K8s` client ve `pod` ve `operator`'lerine yetki vermek için kulanılır.

`Role` içindeki her permission `Rule` olarka isimlendirilir.

`ClusterRole`, Node bilgilerini de içeren permission'ları içerir. oysa `Role` sadece namespace bazında yetkileri içerir.

`RoleBinding` hangi `role`'ün kime bağlı olacağını belirten `YML` dosyasıdır.

`ServiceAccount` `K8s` client'ları dışında kalan herşeydir. `User` ise `K8s` client'tır.

## 📌 autoscale

saf `K8s`'te `HorizontalPodAutoscaler (⟷ HPA)` `YML`'sini apply ederek herhangi bir `pod` grubu için `RAM` veya `CPU` kullanımı belli bir yüzdeliğe geldiğinde replica sayısı otomatk artmasını ayarlayabiliriz.
