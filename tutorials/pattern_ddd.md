...
Aşağıdaki aggregate root'lar 1 mikroservis olsun dedik, çünkü aggregate-root atomic transaction'da ynetilmeli.

- checkout __(domain)__ (composition root - logical consept)
  - basket __(domain)__ (aggregate root) (1 mikroservice)
  ...
  - delivery __(domain)__ (aggregate root) (1 mikroservice)
  - payment __(domain)__ (aggregate root) (1 mikroservice)
  - order __(domain)__ (aggregate root) (1 mikroservice)
- user __(generic domain)__ (1 mikroservice)
- product-search __(core domain)__ (1 mikroservice)
...
- DDD mimarisinde, microservis ile ilgili bir bilgi yoktur. DDD, "modüler monolith" bir yazılımda da kullanılabilir.
...
1 microserviste 1'den fazla domain veya bounded context olabilir ama pek tercih edilmez. Bu durum microservisin temel bağımsızlık argümanını biraz kırar.

Eğer 1 bounded context 1 microserviste olursa, o microservisin ne yaptığı, sorumluluk ayrımı, transaction yönetimi çok netleşir.
...
1 aggregate, 1 aggregate root içerir. "Aggregate" birşeylerin bütün olarak kullanılmasını temsil eden genel bir terim. Biz DDD'de bunu pratiğe döktüğümüzde, karşımıza "aggregate root" kavramı çıkar.
...
teknik isimde olmamalıdırlar. domain expertleri de anlamalıdır.
