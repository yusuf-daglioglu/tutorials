...
Aşağıdaki aggregate root'lar 1 mikroservis olsun dedik, çünkü aggregate-root atomic transaction'da yönetilmeli.

- checkout __(domain)__ (composition root - logical concepts)
  - basket __(domain)__ (aggregate root) (1 mikroservis)

    Bunun altındakiler moduler monolith yapıda olabilir ve/veya hepsinin ayrı bounded-context'i de olabilir.
  ...
  - delivery __(domain)__ (aggregate root) (1 mikroservis)
  - payment __(domain)__ (aggregate root) (1 mikroservis)
  - order __(domain)__ (aggregate root) (1 mikroservis)
- user __(generic domain)__ (1 mikroservis)
- product-search __(core domain)__ (1 mikroservis)
...
- Published Language
  ...
  calendar syntax, contact sync, auth standards (örnek: OAuth) gibi sistemler buna güzel birer örnektir. Belli dökümantasyona/standartlara uyan modellerle çalışıldığında, API'lere de zaten dolaylı yoldan uymuş olacağınız sistemlerdir.
...
- DDD mimarisinde, mikroservis ile ilgili bir bilgi yoktur. DDD, "modüler monolith" bir yazılımda da kullanılabilir.
...
- 1 domain birden fazla mikroservis'ten meydana gelebilir. Veya tam tersi de olabilir.

Genelde beklenen şu yapıdır:

Domain > Bounded context > Microservice

1 mikroserviste 1'den fazla domain veya bounded context olabilir ama pek tercih edilmez. Bu durum mikroservisin temel bağımsızlık argümanını biraz kırar.

Eğer 1 bounded context 1 mikroserviste olursa, o mikroservisin ne yaptığı, sorumluluk ayrımı, transaction yönetimi çok netleşir.
...
public class Order { // this is the "aggregate root" which has all information about "aggregate".
  ...
  // "setters" ve "getters" fonksiyonları dışarıya zorunlu olmadıkça açılmamalı. Daha doğrusu açılabilir, fakat getter ile private field'ı
...
Bazı kaynaklarda DDD'den bahsedilsin veya bahsedilmesin, "entity" terimi direk olarak hem domain model, hem de database nesnesinin yerine  kullanılmaktadır. Bunun 3 temel sebebi vardır:
...
X ve Y veritabanı objelerimiz olsun. ikisinin tüm property'lerinin (isim, soy isim, yaş...) value'ları aynı olsun. buna rağmen bu objeler birbirlerine eşit değildir. çünkü ID'leri farklıdır. bu sebeple bu objeler "value object" değildir.
