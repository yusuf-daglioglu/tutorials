...
JavaEE'nin kullandığı `Java Transaction API (⟷ JTA)` metodlarını çağırarak çalışır. Yine, `Spring-data` da olduğu gibi, Spring önüne wrapper atarak bir API sunar. Dolayısı ile bir `JTA` implementasyonuna ihtiyacımız var:
