...
JavaEE'nin kullandığı `Java Transaction API (or JTA)` metodlarını çağırarak çalışır. Yine, "`Spring data`" da olduğu gibi, Spring önüne wrapper atarak bir API sunar. Dolayısı ile bir `JTA` implementasyonuna ihtiyacımız var:
...
`Lazy` sadece `get` metodu çağrılınca database'den nesneyi çağırır.

`EAGER` ise her zaman direk database'den okunmuş halde objeyi getirir.
