...
- Spring Data Commons (common classes for all other jar below)
- Spring-Data-JPA
- Spring-Data-MongoDB
- Spring-Data-JDBC
- Spring-Data-LDAP
...
Spring JTA, Java EE'nin kullandığı `Java Transaction API (⟷ JTA)` metodlarını çağırarak çalışır. Yine, `Spring-data` da olduğu gibi, Spring önüne wrapper atarak bir API sunar. Dolayısı ile bir `JTA` implementasyonuna ihtiyacımız var:
...
`JTA`, `XA` protocol'ünü destekleyen `JMS` ve `DB` server'larında işlemlerin atomic yapılabilmesini sağlar.
...
`X/Open (⟷ Open Group for Unix Systems)` birçok kuruluş tarafından desteklenen ortak bir camiadır. `XA` protocol'ü, `X/Open` tarafından geliştirilmiştir.

Dökümandaki bazı tanımlar:

- `Application Program`: transaction'ın adımlarını tanımlayan program.
- `Resource Manager`: DB, `file system`, printer service gibi paylaşılacak kaynağı paylaşan yazılımdır.
- `Transaction Manager`: transaction'ın ne durumda olduğunu monitör eden, gerektiğinde rollback alması için ilgili `Resource Manager`'lere protocol üzerinden haber veren yazılımdır.
...
Dökümanda 11inci sayfada `Interface Overview` başlığında belirtildiği gibi `XA`, `DB` ile transaction manager arasındaki protocol'dür. Bu sebeple `DB` tarafından desteklenmesi zorunludur.
...
- bir entity'ye (buna sub-entity'lerde dahil) event atanmış ise, o event'lerin (`@PreRemove`, `@PostRemove` gibi) tetiklenmesi gerekecektir. (`JPA Entity Lifecycle Events` konusu.)
