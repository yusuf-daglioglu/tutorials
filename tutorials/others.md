...
Not: Hashcode'un kendisi de uzun olduğu için, (string aramadan daha avantajlı olsa da) o da aramaları yavaşlatabilir. onunda bir çözümü bu başlıkta anlatılmaktadır.

Bu senaryoyu URL prettify/minifier/shortener servisi üzerinden anlatacağım.

prettify edilmiş string'in hash'ini DB de ayrı bir sütunda tutarız. Burada son 2-3 karakter veya ilk 2-3 karakterini silip kaydetmemiz lazım databaseye. böylece long olan hash'i, integer tipine düşürmüş oluruz.
...
- Son kullanıcılar genelde böyle sitelerde tiny-url yaratıp, hemen 1-2 saat içinde kullanıyor. bu sebeple şu yapılabilir:
  ...
  String findFullPath(String prettifiedUrl){ 
    ...
    // varsa onları bir array içinde saklıyıp kontrol ediyor (Aynı java.util.HashMap'in yaptığı gibi).
...
Buradan anlaşıldığı gibi bize kesin bilgi vermemektedir. İhtimaller ile çalışmaktadır. Bu sebeple business'ımızı buna uygun olması gerekir. Yoksa bu tekniği kullanamayız. Fakat bloom filter'ı gerçek'ten db'ye gitmeden önce 1 kere bu fonskiyon içinde çalıştırabiliriz.
...
- user-name-exist sorgulama servisi, kesin bir dönüş beklenen bilgi servisi olduğu için, burada "Bloom filter" kullanamayız. Fakat bloom filter'ı gerçek'ten db'ye gitmedne önce 1 kere bu fonskiyon içinde çalıştırabiliriz.
...
- email server'lar bir sender'ın spama atılıp atılmayacağının kara listesini tutar. buranın sorgusu için bloom filter kullanılabilir.
- sözlük uygulamaları yazılan kelimenin db'de hiç olup olmadğını önce bloom filter ile kontrol edebilir.
...
- supports:
  ...
  - flow control:
    ...
    Backpressure içinde kullanılabilir, ama flow-control daha geniş bir terim. pseudo kodda görüldüğü gibi istediğimiz gibi akışı duruma göre bekletebiliyoruz, fakat bunu sadece yük binmesinde diil, herhangi bir bussiness içinde uygulayabiliyoruz:
  ...
  - timeouts: client bu süreyi server'a her istekte iletir. server bu sürede işi bitiremezse, server'ın kendisi kendi kendine isteği sonlandırır.
    ...
    Bu durum HTTP'de yoktur. HTTP client timeout parametresi alır, ama client'ın kendisi cevap gelmeyinde kendini kapatır. HTTP'deServer hala response'u işlemeye devam eder.
  ...
  - authentication:
    ...
    - intercepter desteği var. böylece custom istediğimiz autantication'ı yazabiliriz.
  ...
  - metadata
    ...
    HTTP'de olduğu gibi metadata'ları destekler. hem response hem requestte olabilir.

  - trailers

    tüm response döndükten sonra sadece server'ın client'a ilettiği key-value bir yapıdır. Status ve Status-Message her zaman yollanır. burada hata kodu veya başaırlı tarzı bilgi döner.

    bu bilgiler customda verilebilir. yani ek trailer'lar manuel ekleyebiliriz.
...
- 1 grapqh isteğinde birden fazla query/mutation varsa, birden fazla error ve/veya data içinde birden fazla data olabilir.
- errors kısmı standarttır. bu kısım client'ın synxtax error'larını da kapsar, sunucunun 500 hatalarını da kapsar.
...
- "locations" alanı standart. Eğer client syntax hatası yapmış ise, requestteki hatalı olan satır bilgisini yazıyor.
...
type ReadMerchant implements BaseEntity{ # inheritance
    name: String! # ! means: grapql will throw exception if it is null.
    createdAt: Date
    address: ReadAddress
