...
## 📌 Storage types (⟷ storage models)

Aşağıdaki sınıflandırma, fiziksel data tutuş tipine göre değil, erişim tipine göre ayrıştırılmıştır.

`RDMS` veya `NoSQL`'ler storage olmadıkları için aşağıdaki gruplara dahil olamazlar.

- `block storage`

  disk alanı gibi düşünülebilir. `file-system`'i üzerine manuel kurmak gerekli.

- `file storage`

  ağaç hierarşisinde dosya tutma ve yönetebilme hizmetledir. `file-system` buna en iyi bilinen örnektir.
...
`AWS`'nin sunuğu bulut `object storage` servisi.

## 📌 Amazon S3 compatible softwares

Aynı `API`'yi (veya belli bir seviyede uyumlu `API`) sunan on-prem veya bulut yazılımlar da vardır. Bu yazılımların data'yı kendi içinde nasıl tuttuğu implementasyonun kendisine kalmış. Fakat sundukları `API` ortak. örnekler:

- `MinIO`

  açık kaynaklı. geliştirilmesi durduruldu.
...
`Amazon S3` datayı şu hierarşide tutar:

Aşağıda; `key` `string`'i olarak istediğim random `string` kullanabiliriz.

- `Amazon S3` instance
  - `bucket`: `my_bucket_1`
    - `key`: `my_parent_grup1/2025/01/02/metrics1`
    - `key`: `my_parent_grup1/2025/01/02/metrics2`
    - `key`: `my_parent_grup1/2026/01/02/metrics1`
  - `bucket`: `my_bucket_2`
    - `key`: `my_parent_grup1/2025/01/02/metrics1`

`Amazon S3` `API`'si;
- `key`'e denk gelen value içerisinde arama yapabilmemize olanak sunmaz.
- `key` ile arama yaparken sadece prefix verebiliriz. yukarıdaki örnek için;
  - `my_parent` olarak aramama yaparsak sonuç döner,
  - ama `metrics` diye aratırsak hiç sonuç dönmez.
- `key`'e denk gelen value'yu tümüyle veya sadece bir range'i (start byte index - last byte index aralığında) çekebilmemizi sağlar.

## 📌 PromQL and TraceQL

`PromQL`, `Mimir`'in, `TraceQL` ise `Tempo` yazılımlarının query language'leridir. `Mimir` ve `Tempo`, `Amason S3`-compatible `API`'lerde data saklayıp query atabilmemiz sağlarlar.

`Mimir` ve `Tempo`, data'ları aşağıdaki şekilde `Amason S3`'te saklarki, sonra atılacak query'lerde hızlı ve sadece belli bir range'i çeken sorgular atabilelim:

(aşağıdaki bilgiler konseptsel anlatılmıştır. detaylar çok farketmektedir.)

- `bucket`: `my_traces`

  - `key`: `tenant_1/b1/meta.json`

    içeridği bazı bilgiler:
  
    `tenant_1/b1` için zaman aralığı.

  - `key`: `tenant_1/b1/bloom`

    bu kısım `bloom-filter` uygulamak içindir.

    bu bir bit array'dir. sorgulanacak trace-id'nin `tenant_1/b1/data_parquest`'de olup olmadığı bilgisi şu şekilde tutulur:

    false: kesin `b2`'de yok.
    true: muhtemelen var.

  - `key`: `tenant_1/b1/index`

    `tenant_1/b1/data_parquest` kendi içindeki `trace-id`'leri sıralı tutar. bu `index` dosyasında `trace-id`'ler fixed size'lara'lara bölünerek index'i tutulur. böylece eğer `trace-id` burada var ise, tüm `data_parquest`'in client tarafından çekilmesin gereke kalmaz. sadece ilgili byte array çekilmesi yetecektir. 

  - `key`: `tenant_1/b1/data_parquest`

    `tenant_1/b1` data'sının kendisi.

  - `key`: `tenant_1/b2/meta.json`

    yukarıdakiler ile aynı şekilde farklı bir block için bilgileri tutar.

  - `key`: `tenant_1/b2/bloom.json`

  - `key`: `tenant_1/b2/index.json`

  - `key`: `tenant_1/b2/data_parquest.json`
...
`dump` kelime anlamı: çöplük yığını.

çalışan executable'ın:
- `CPU` register'larındaki (belleklerindeki) bilgileri
- `RAM`'deki bilgisi
- tüm `File Descriptor`
- process ve thread ID'ler ve meta verileri

Yukarıdaki tüm bilgilerini veya sadece bazılarını dosyaya kaydedebiliriz.

bazı `dump` formatları:

- `hprof`

  sadece `java` için `dump` formatı. yukarıdakilerden sadece `RAM` bilgisivnini ve bazı thread bilgilerini tutar.

- `core`
  
  Unix-like'larda program kill edilince bu formatta kaydedilir.

- `dmp`

  `MS Windows`'larda kullanılan formattır. `Minidump` olarak isimlendirilir.

  tüm `RAM`'i kaydetmek yerine, sadece hata ayıklama için gerekli bilgilerin bir kısmını barındırmaktadır.

## 📌 RAM dump != executable

executable dosyası run edildiğinde, `RAM`'e yüklenir (`memory structure` başlığında anlatılıyor). Bu sebeple `dump` içerisinde executable'ı bulabiliriz. Bu sebeple bazı kaynaklarda, `dump`'ın formatının, executable formatı olduğu yazar. fakat bu tam olarak doğru değil. `dump`'ta, executable dosyaya ek olarak extra bilgiler (`RAM`'deki diğer bölgeler) de vardır.

## 📌 profiling

`profiling` dosyası, `dump` dosyasını içerir, üstüne ek olarka farklı data'lar içerebilir.

`profiling`; belli bir zaman aralığında, bir executable'ın performans ve bellek analizi bilgilerini tutar.

`profiling`; temelde her saniye `dump` alınıp, bu bilgiler birleştirerek oluşturulan bir data'dır.

bazı `profiling` formatları:
...
isteğe bağlı olarak; `dump` dosyasını da parametre verirsek, ayağa kaldırdığı uygulamayı direk `dump` dosyasındaki state ile başlatır:
...
- `JVM`'in sorunlu kapanması ile oluşan dosyaya `Java dump (⟷ Java core)` denir.

- `JVM`, `dump`'ı dosyaya yazarken `hprof` formatını kullanır.

## 📌 JvisualVM

`JvisualVM` ile `GUI` üzerinden herhangi bir `Java` uygulamasının aşağıdaki bilgilerini görebiliriz:
- hangi parametrelerle başlatılmış
- `System.properties`'te nelerin olduğu
- historical `CPU` and memory usage
- historical threads with states (waiting, running gibi)
- and others.

## 📌 Java thread dump

`JvisualVM` ile bir uygulamanın `thread dump`'ını alırsak, her thread'in hangi statüde olduğunu ve her thread'deki `stack trace`'i görebiliriz. Şu programımızı örnek alalım:
...
`profiler`, runtime'da çalışan uygulamamıza `Bytecode` ekleyerek müdahale eder. oysa `sampler` böyle bir yöntem uygulamaz. bu yüzden `sampler` daha hızlıdır. fakat `profiler` daha çok fazla bilgiler sunar.
...
- `script language` olduğundan, `VM` gerektiriyor.
...
`Httpbin` test amaçlı kullanılan, hazır `HTTP` metotları sunan sunucu yazılımlara verilen genel bir prefix veya suffix'tir. piyasada birçok bağımsız `Httpbin` servisi vardır:
...
They all require another backend service, because `VNC`, `RDP` and `SPICE` protocols does not work with `HTTP` or `websocket`. they use `TCP`.
...
- Running: `CPU`'da işlem görüyor. Kernel mode ve User mode olarak çalışırlar. "User mode" kernel instructions'ları çağıramaz.

- Blocked: `CPU`'ya girse de bir işe yaramayacak durumdadır, çünkü dış donanımlardan (kaynaklardan) cevap bekleniyordur.

- Terminated: işi bitmiş process. parent process "exit code" değerini okumadığı için henüz, process bu statüde bekler. bu şekilde çok process olabileceğinden bunlara "zombie process" denir.
...
`simultaneous (⟷ tr:simultane)` terimi anında anlamına geliyor. örneğin; anında tercüme. Fakat farklı kod thread'leri simultaneous olarak çalışıyor denildiğinde paralel ile aynı anlama çıkmaktadır. `source: <https://docs.oracle.com/cd/E19455-01/806-5257/6je9h032b/index.html> Bu linkte aşağıdaki yazmaktadır:`

- `Parallelism: A condition that arises when at least two threads are executing simultaneously.`
- `Concurrency: A condition that exists when at least two threads are making progress. A more generalized form of parallelism that can include time-slicing as a form of virtual parallelism.`

## 📌 OS thread (⟷ kernel-space thread ⟷ kernel-level thread) vs Green thread (⟷ user-space thread ⟷ user-level thread)

`user-level thread`'lerden `OS`'un haberi olmaz. Kütüphaneler aracılığı ile yapılır. Örnekler:

- `Java` 21 ile gelen `virtual thread`.
- `golang`'de baştan beri olan `goroutine`.

`user-level thread` avantajları:
- Daha az `RAM` kullanıyor. çünkü `OS` `thread` bilgisi kadar bilgiye ihtiyaç yok.
- Daha hızlı. çünkü `Kernel Mode`'a geçişe gerek kalmıyor. geçilmemesi için `system call` yapılmıyor.

`Heavyweight process`; `solaris` dünyasında `process` anlamında kullanılıyor.

`Lightweight process`; `solaris` dünyasında `thread` anlamında kullanılıyor.

`fiber` terimi; developer'ın kod yazarak `yield` (veya benzeri isimde) fonskiyonunu çağırarak thread switch yapılabilmesini sağlayan `user-level thread`'lere verilen bir terimdir.

- eğer `user-level thread`'i sunan kütüphane `cooperative scheduler` kullanıyor ise:

  - Eğer kütüphane arkaplanda multiple `kernel-level thread` açarsa:

    - `yield` gibi manuel fonksiyonlar çağırılırsa veya IO işlemi yapılırsa

      paralel çalışır.

    - else case

      concurrent bile imkansız.

   - Eğer kütüphane arkaplanda multiple `kernel-level thread` açmazsa:

     - `yield` gibi manuel fonksiyonlar çağırılırsa veya IO işlemi yapılırsa

        concurrent çalışır. paralel imkansız.

     - else case

       concurrent bile imkansız.

- eğer `user-level thread`'i sunan kütüphane `preemption scheduler` kullanıyor ise:

  `yield` gibi manuel fonksiyonlar çağırmaya gerek hiç yok. çünkü `CPU`'nun sunduğu `timer interrupt`, `signal`, `safepoint` gibi yeteneklerden yararlanarak thread'i bölebilir. (ama bu bölmeler sonrası `CPU`'nun hangi thread'den devam edeceği belirsizdir. yani diğer `user-level thread`'e geçecek diye bir kural yok.)
  ...
  - Eğer kütüphane arkaplanda multiple `kernel-level thread` açmazsa:
    ...
    paralel imkansız. concurrent çalışır.
...
devops süreçlerini baştan sona aynı kişiler yönetemiyor. çok geniş çünkü. bu sebeple sadece deployment, jenkins süreçlerini yönetebilen ekipler çıktı. bu kişiler Platform Engineer diye adlandırılıyor.

## 📌 SRE (⟷ Site Reliability Engineer)

ortamların availability, latency, performance, efficiency, monitoring, emergency response, and capacity planning gibi konuları ile ilgilenen mühendistir. sadece prod ortam değil, tüm ortamlardan sorumludur.
...
- `absolute path`
  ...
  `file system`'ın root'undan olmalıdır fakat noktalama ile yönlendirmeler içerebilir.

- `canonical path`

  `C:\a\b`

  `~/b` (Bu da `canonical`'dır. çünkü `~` işareti sadece bir kısaltmadır.)

  `file system`'ın root'undan alınan fakat noktalama işaretleri ile yönlendirmeler içermeyen path.

- `relative path`

  `.\a`
  
  `file system`'ın başından değilde, o anda runtime sırasında bulunan dizinden itibaren verilen path'tir.

## 📌 canonical form vs normal form yada standard form

`canonical` kelime anlamı: resmi kurallara ve kanunlara uygun (gelenekleri kapsamaz).

herhangi bir matematik objesini veya bilimsel ifadeyi farklı notasyonlarla gösterebiliriz. Eğer kullandığımız notasyon, aynı zamanda, objenin tekilliğini ifade ediyor ise (yani o hali başka bir şeye denk gelemez ise), bu notasyon (form biçimi) `canonical`'dır.

örneğin; `canonical path`, `canonical form`'da bir ifadedir.

`normal form`, belli `norm`lara (kurallara, geleneklere) uyan formdaki ifadedir. bu kurallar (`norm`lar) sadece şirket içi, veya kendi yazdığımız yazılımın bildiği kurallar olabilir.

`standard form`, topluluklarca kabul edilmiş standartlara uyan formdaki ifadelerdir.

piyasada `canonical` `UUID`, `11111111-2222-3333-4444-555555555` gibi tireli gösterilir, fakat bu gösterim `canonical` değildir. bu gösterim `RFC` de geçmez. topluluk kabulü olduğu için `standard form` diyebiliriz. `canonical` `UUID` aslıda hiç tiresiz olan `UUID` olmalıdır: `11111111222233334444555555555` gibi.

## 📌 Uniform Resource Name (⟷ URN)

İlgili kaynağın nasıl erişilebileceğini gösteren bir bilgi sunmaz. Sadece ilgili kaynağı tanımlayan bir bilgidir.
...
`script language`; bir environment üzerindeki yetenekleri kullandırtmak için developer'a çok pratik ve kısa kod yazarak yaptırtabilen dillerdir.
...
- `Groovy`
  ...
  - çoğu kontrol runtime sırasında (comile time'da değil). örneğin type kontrolleri.
  - `DSL` benzeri synxtax'ı var.
  - Az kod ile çok işlem yapılabiliyor.
  - type zorunluluğu yok.
  - main metodu olmak zorunda değil. direk yaz çalıştır. REPL ile kullanmak için daya uygun.
...
## 📌 ECMAScript neden script language?

`ECMAScript` ilk çıktığında web sayfası yeteneklerini kolayca ve hızlıca birkaç satırlık kodla yapılabilmesi içinçıkmıştı. Böyle düşünüldüğpünde `script language` olur. Fakat artık genel amaçlı olduğu için `script language` diyemeyiz.
...
Teknik ve teknik olmayan konularda standartlar belirleyen komisyon.
...
- `TS` terimi `Technical Specification` olduğunu belirtiyor. `ISO` standartlarında bu gibi birçok çeşit içerik tipi mevcut:
...
Bu örnekte olmasa da, `ISO/IEC` gibi terimlerde görebiliyoruz. Bu durumda bu standardın 2 farklı bir organizasyon (hem `ISO` hem `IEC (⟷ International Electrotechnical Commission ⟷ Uluslararası Elektroteknik Komisyonu)`) ile birlikte geliştirildiğini belirtiyor.
...
`AI`, yapay zekanın kendini geliştirmesidir. Yapay zeka bir girdiye karşılık bir çıktı verebilir. Fakat kendini eğitebilmesi konusu ayrıdır ve `ML` konusuna girer.
...
`model`, matematiksel bir fonskiyon. input olarak `vector` alır, çıktı olarak yine `vector` verir.
...
her `LM` bir fonskiyon. bu fonskiyonları açık kaynaklı `gguf` formatında dosyada tutarız. `ollama`'yı çalıştırdığımızda:
- `gguf` dosyasını okur,
- optimizasyon yapar,
...
`YSA` için `API gateway`'dir. herhangi bir `YSA` ile `API` üzerinden text ve görsel gibi medyalarla iletişim kurabilmemizi sağlar. Açık kaynaklı değildir. Ve sadece bulutta çalışır. lokale indirebileceğimiz bir yazılım değildir.

Bu yazılıma alternatif lokale kurulabilecek açık kaynaklı yazılımlar: `LiteLLM`, `Bifrost`.
...
`ai agent`'ın içinde olabilir veya `LM`'e direk manuel prompt vererek yapılabilir.

`LM`'e gitmeden internal data kaynaklarımızıdaki dataların toplanıp `LM`'e o şekilde soru sorulma akışını temsil eder.

dışarıdan bilgi almak için `MCP` protocol'ü kullanılabilir.

## 📌 AI agent

bu yazılımın kendisinde `LLM`'e benzer ufak ta olsa bir `YSA` barındırmak zorunda değildir.

kendi başına iterasyonlarla görevi işletebilen yazılımdır. arkaplanda, `MCP`, `RAG` ve `AI`'ı çağırır (kullanır).
...
`MCP`'nn sunduğu her `API` fonskiyonu 2 çeşit olabilir:

### 📌📌 tool

execution yapar. dış sistemde `side-effect` yaratacak diye bir zorululuk yoktur. örneğin `maven` ile dependency tree'yi çekmeye kalkmak bu gruptadır. çünkü bir maven komutu execute ediliyor ama read-only.

### 📌📌 resource

query amaçlı atılır. varolan dış sistemden bilgi okunur. read-only olması şarttır.
...
son kullanici ile ilk temasa gecen yazılımdır. Bu sebeple `AI application` olarakta adlandırılırlar.

birçok görevi olabilir:

- sorular soruyu işi en iyi bilen `AI`'a yöneldirmek (bunun için kendi içinde ufak bir `AI` barındırmalı)
- önceki konuşmaları hatırlaması için memory yönetimi.
- `AI`'a, `MCP` lerin sunduklarını yollar. `AI`, `MCP` server'dan bir şey isterse, `MCP` server'a `AI assistant` istek atar ve cevabı `AI` verir.
- `AI agent` özelliği barındırabilir.
...
flowchart LR
    ...
    AI_ASSISTANT["AI_ASSISTANT<br><br>2<br><br>EmbeddingFunction(end<br>_user_input_text)=VECTOR<br>_VALUE"]

    END_USER --> |"1<br>end_user_input_text"| AI_ASSISTANT
    AI_ASSISTANT --> |"3<br>VECTOR_VALUE"| VECTOR_DB
    VECTOR_DB --> |"4<br>similar_text"| AI_ASSISTANT
    AI_ASSISTANT --> |"5<br>similar_text <br>+<br> end_user_input_text"| LLM
    LLM --> |"6<br>answer"| AI_ASSISTANT
    AI_ASSISTANT --> |"7<br>answer"| END_USER
```
...
Open source komut satırı uygulamasıdır. user'ın tanımladığı `YML` dosyasını (`Ansible Playbook (⟷ Ansible script)`) okur ve ve bu dosyadaki yapılacak işlemleri remote makinelerde yapar. Bunu yaparken `ansible` remote makinelere `SSH` ile gider. Bu sebeple yöneteceğimiz uzak makineye (`Managed node`'a), agent yazılımı kurmak gerekmez. bu sebeple agentless mimarisi vardır.

`Control machine` uzak makinaları yönetecek olan (onlarda değişiklik yapacak olan) bilgisayarlara verilen isimdir.
...
bir sıfat gibi kullanılıyordu. İlk zamanlar tüm `OS`'lara böyle diyorlardı. Çünkü farklı disk platformları içerisinde işlem yapabiliyordu.
...
`X` karakteri birçok terimin kısaltması olabildiği için piyasada terminoloji çakışması mevcut.
...
mobil uygulamalar için bulut sistemi hizmetleridir. örneğin notification yönetimi, app için analitik bilgi toplama işlemleri, crash hakkında bilgi toplama servisleri, kolayca erişilebilir `DB`'ları.

Örnek hizmet: `Google` `Firebase`.
...
genel olarak; uygulama geliştirmek, çalıştırmak ve yönetmek için runtime ve geliştirme `SDK`'ları sunarak süreci hızlandırır kolaylaştırır.

Örnekler: `Heroku`, `AWS` `Elastic Beanstalk`.

## 📌 iPaaS (Integration Platform as a Service)

`aPaaS`'a ile aynı seviyede çalışır fakat amacı farklıdır. burada sunulan feature'ler, üçüncü parti sistemler ile entegrasyondur. örneğin farklı bir e-ticaret platformuna otomatik entegre olabilmemizin sağlanması gibi.

## 📌 on-premises software (⟷ on-prem software)

yanlış kullanım: `on-premise software`
...
`interop` kelimesi `IT`'ciler tarafından ortaya atıldı ve `interoperability`'nin kısaltması olarak kullanılıyor.
...
## 📌 domain expert'ler ile yapılan görsel toplantılar
...
aynı sınıftaki bilgilerin karşılaştırılması için çizilen gösrsellerdir.

çok kullanılan bazı chart çeşitleri:

- `Plot`, x ve y ekseni üzerine çizilen her türlü koordinat tabanlı görselin (saçılım grafiği, eğri vb.) genel adıdır.
...
## 📌 diagram (⟷ tr:diyagram ⟷ çizenek)

farklı kavramlara ait bilgilerin bir arada gösterildiği görsellerdir. örnekler:

- `UML (⟷ Unified Modeling Language)`
- `Mind map (⟷ Zihin Haritası)`
- `Venn diagram (⟷ Venn Şeması)`
- `Flowchart (⟷ Akış Şeması)`

## 📌 scheme (⟷ şema)

teknik görsellerdir. örnek:

- elektronik devre şeması

## 📌 Cartogram (⟷ kartogram)

istatistiki bilgilerin, direk haritanın üzerinde gösterildiği görsellerdir.
...
istediğimiz bir programlama dilinde kod yazarak, o kodu `wasm` binary'si olarak compile (build) ederiz. bunun için; özel (bu işi yapan) bir compiler (builder) kullanmamız şart.

bu `WASM` dosyasını istediğimiz ortamda execute edebiliriz. piyasada execute eden ortamlarlar:

- Web browser'lar `JS` içinde, `wasm` dosyalarını run edebilmemizi sağlıyor.
- `Envoy` proxy, `wasm` dosyalarını plugin gibi yürütüp, gelen giden request'leri manipüle edebilmemizi sağlıyor.
- `Ethereum`, `wasm` kodu çalıştırabiliyor

örnek `JS` kodu:

```js
// wasm dosyası sunucu tarafta binary olarak tutuluyor.
const response = await fetch("math.wasm");
const bytes = await response.arrayBuffer();

// modül ve instance oluştur
const wasmModule = await WebAssembly.compile(bytes);
const wasmInstance = await WebAssembly.instantiate(wasmModule);

// wasm içindeki fonksiyonu çağır (örnek: iki sayıyı toplama)
const result = wasmInstance.exports.add(5, 3);

console.log(result); // 8
```

`wasm` genel olarak çok hızlıdır.
