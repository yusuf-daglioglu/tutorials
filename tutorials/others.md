...
- OS seviyesinde bakılırsa Chrome; panel'dir (Microsoft Windows'taki taskbar, başlat menüsü gibi).
...
- Google baskınlığında geliştirilen açık kaynaklı bir protokoldür.
...
- supports:
  ...
  - __bidirectional streaming__ (bunun basit kod örneği başka bir başlıkta var)
  ...
  - __blocking or non-blocking bindings__
...
Microsoft Windows'ta bu hata tipine __stop error (or blue screen of death or blue screen or BSoD)__ denir.
...
Dynatrace firması tarafından geliştirilen "Application Performance Monitoring" yazılımıdır. örneğin; Java web uygulamamızı takip eder ve ona gelen istekleri gruplayıp, dashboard'larda detayları gösterir. stacktrace'e kadar analiz edilmesini sağlar.
...
diğer arama motorlarını kullanarak sonuçları yansıtan arama motorlarıdır. kendi veritabanlarını bulundurmazlar.
...
- network'teki birçok protokol ile (örnek: Samba) paylaşılmış media dosyalarını direk açma
- yayın protokollerini izleme
...
İşletim sistemi ile kullanıcı programları arasında tanımlı olan API'dir.
...
Microsoft Windows 10 ile isimlendirme değişti.

## 📌 Windows 10 Büyük Güncellemeler
...
### Windows 11 Büyük Güncellemeler
...
- "__Android studio: Make__" ve "__Intellij idea: build__"
...
- ExFAT (or Extensible File Allocation Table)
  ...
  FAT32'nin fork'udur. sadece minimum dosya boyutu arttırıldı. Bu Microsoft Windows 7 ve sonrasında default support ediliyor.
...
hard disk'lerde en küçük kaydedilebilir birim'e "allocation unit" denir. dosya sistemimizin "allocation unit" boyutu 4096 KB olduğunu varsayalım. bizim dosyamız ise 512 KB olsun. bu durumda bizim dosyamız 4096 KB "size on disk" değerinde olacaktır.
...
avantajı şu: txt formatı gibi her editörle açılabiliyor. aynı zaman excel ve alternatifi uygulamalar CSV açarken her virgül arasını bir sütuna koyup açtığı için aynı dosya excel ile de sütunlara bölünmüş şekilde açılabiliyor.
...
get/set prefix'leri gerçek getter-setter amaçlı kullanılmayan metotlarda kullanmamaya gayret edilmelidir. Bu sebeple get yerine:
...
- YML dosyasına çeviren
- YML dosyasından server/client kodu hazırlayabilen 
...
Swagger YML dosyasında bir standartıdır. bu WSDL dosyasının benzeri bir YML dosyası.
...
## 📌 Swagger version history (or OpenAPI version history)
...
".Net" bir runtime'a ihtiyaç duyuyor. Aynı JVM gibi. bu runtime'ın ismi __Common Language Runtime (or CLR)__. JVM, Bytecode'u çalıştırabilirken, ".Net", __Microsoft intermediate language (or MSIL)__'ı çalıştırmaktadır.

## 📌 .Net core

".Net" framework'ün sadece Linux, MacOS ve Microsoft Windows'ta çalışılabilir bir türevidir. sadece Microsoft Windows'ta çalışacak bir uygulama yazılacaksa (classic) ".Net" tercih edilmelidir. classic ".Net", ".Net core" framework'üne göre daha çok özellik içeriyor.
...
Windows SDK'dan farklıdır. Farklı windows SDK'ları için ortak API sunan framework'leri kapsar.
...
Oysa dialog, sayfada bağımsızca açılan bir "chat" (reklam amaçlı) kutusu bile olabilir. Onu hiç kullanmadan sayfada fakrlı bir iş yapabiliriz.
...
- Handles: Microsoft Windows uygulamanın açtığı bazı kaynakları tutmak/yönetmek Handle kavramından yararlanır. Soket, dosya, pencereler, pencere objeleri gibi. Bunların yazılımdaki "sınıf" karşılığı gibi düşünülebilir. Bunların toplam sayısı bu alanda yazmaktadır.
...
NTFS, 255 Unicode karakter kabul ediyor. fakat eski dosya sistemleri etmiyor. bunun önüne geçmek için güncel Microsoft Windows OS'ları her dosya için 2 farklı dosya isim tutuyor. örneğin;
...
- level 2 support (or second level support): yardım alınan kişinin veritabanı gibi yerlere manuel sorgu atması gibi IT bilgisi vardır.
...
- standart bir PC'de OS'ları binlerce thread aynı anda açık tutar. bu thread'lerin hepsi sanaldır. aslında her biri teker teker (daha doğrusu CPU sayımız kadarı) CPU'ya girerler. fakat OS, üzerinde çalışan yazılımlar durmasın/hata vermesin diye onlara thread açar fakat beklemeye alır.
...
1 veya 2 boyutlu bir resimde kare kare siyah ve beyaz noktalar mevcut. Bunların her biri birer ikiliğe (byte) denk geliyor. Bu değerler resimden okunduğu (tarandığı) zaman, sonucunda uzunca bir byte array elde ediliyor. bu artık istenilen formatta anlamlandırılıyor. bu byte-array'i anlamlandırmak için birçok standart var. "__QR Code (or Quick Response Code)__" ise bu standartlardan biridir.
...
- __CRM (or customer relationship management)__
  - Satış programı
  - Pazarlama programı
...
herhangbir bir kaynaktan (API, veritabanı gibi) data okur, data'yı manipüle eder ve herhangi bir kaynağa yollar.

`extract`, `transform`, `load` (3 işlemin kısaltması: `ETL`) sürecini yönetmemize yarayan bir yazılım.

kendi başına bir process. sadece config'lerle yönetilebiliyor. config hazırlak için WebUI sunmaktadır.
...
## 📌 Endüstri 4.0 (or 4. Endüstri Devrimi or Industry 4.0 or the fourth industrial revolution)
...
- örneğin; Syslogd, OS'ta system log'larını tutan arka plan uygulamasıdır.
...
publisher çok fazla data sağlar ve subscriber bu bilgileri işlerken yavaş ise, subscriber, publisher'a bir şekilde daha az data yollamasını iletebilmelidir. bu yeteneğe back-pressure denir.

back-pressure sistemi yavaşlatacaktır fakat sistemin daha stabil çalışmasını sağlayacaktır.
...
## 📌 CI (or Continuous Integration) sistemleri
...
Oyun motorları her OS için ayrı derleme gerektirir. Örneğin; Unity arka planda Windows için DirectX, Linux için OpenGL kullanıyor olabilir.
...
|--------------------------------------------|-----------------|-------------|-----------------------------------------|
| OpenGL                                     | yes             | yes         |                                         |
| OpenGL ES (or OpenGL for Embedded Systems) | yes             | yes         |                                         |
...
- felaket senaryoları (doğal afet, Single point of failure)
...
DSL sayesinde belirtilen syntax'ta ki metinler/kodlar, DSL parser tarafından direk işletilir veya fakrlı bir formata çevrilir.
...
| __Kebab Case (or spinal case or param case or Lisp case or dash case)__ | user-login-count | kebap yemeğindeki gibi şişteki her et parçasının arasında çubuk (tire işareti) vardır        |
| __Pascal Case__                                                         | UserLoginCount   | ilk Pascal programlama dilinde bu şekilde variable tanımlamaları vardı                       |
| __Camel Case (or camel caps or medial capitals)__                       | userLoginCount   | develerin sırtı ortadaki çıktıya göre düşük seviyededir. sadece ortasında bir çıkıntı vardır |
...
- Ticket server
  ...
  - Single point of failure vardır.
...
her yazılım, dışarıya birçok servis açabilir. her servis ID'si tüm OS'ta tekil olmalıdır. bu servislerin her birine "bus" denir. örnek bir bus:
...
- TCP/IP soket kuralları ile bu işin olması tercih edilmeyebilir. network ağına bağlı olmayan sistemlerde, TCP/IP modülleri yüklü olmayabilir.

- 7011 portuna herkes istek yapabilir. bu güvensizdir. dışarıya kapatmak için ekstra config gerekebilir.
...
- protokol bilgisi içermez. cross-IPC bir standarttır.
...
COM sadece DTO için standartlaştırıcı bilgiler içerirken, DCOM baştan sonra network protokolü sunar.
