...
`model`, bir fonskiyon. input olarak `vector` alır, çıktı olarak yine `vector` verir. bu fonksiyonun içinde çarpım işlemleri vardır.

`model`in içindeki `Weight` sayısı (ve daha ek parametreler) büyüklüğüne göre kategorize edilir. gerçek örnek değerler:
...
- `GPT`, `GPT` (company: `OpenAI`)
- `Gemini` (company: `Google`)
- `Sonnet` (company: `Anthropic`)
...
- `gguf` dosyasını okur,
- optimizasyon yapar,
- `HTTP` `API` açar ve bu fonksiyonun çağrılabilmesini sağlar.

- `ollama` compatible bazı `LM`'ler: <https://github.com/ollama/ollama?tab=readme-ov-file#model-library>

- `ollama` compatible bazı `UI` yazılımları: <https://github.com/ollama/ollama?tab=readme-ov-file#web--desktop>
...
son kullanici ile direk temasa gecer. görevi dogru `ai agent`'a veya direk `MCP` server'a yönlendirmektedir. `ai asistant`'ta kendi icinde basit bir `LLM` olmalidir ki, dogru yere yönlendirsin.
