...
dışarıdan bilgi almak için `MCP` protokolü kullanılabilir.

## 📌 AI agent

bu yazılımın kendisinde `LLM`'e benzer ufak ta olsa bir beyin yoktur.

kendi başına iterasyonlarla görevi işletebilen yazılımdır. arkaplanda, `MCP`, `RAG` ve `AI`'ı çağırır (kullanır).

## 📌 mcp (⟷ Model Context Protocol)

açık kaynaklı protokoldür. bir program `TCP` `API`'den bu protocol ile hizmet sunar. `agent` external yerlerden bilgi toplamak icin bu `API`'lerden yararlanmir.
...
`AI assistant` bu `skill` dosyalarını `AI`'a yollar. Artık `AI`, eğer son kullanıcı unit test yazdırmak isterse tüm buradaki kurallara mutlaka uyarak yapar.

## 📌 Github Copilot customizations

bu tanımlar sadece `github` `copilot` (`vscode` eklnetisi vem masaüstü application'ı) için geçerli terimler. bu terimler genel `AI` dünyasında farklı anlamlara gelebiliyor.
...
sadece gerektiğinde `ai agent` tarafından okunur. burası biraz `ai agent`'ın yorumuna kalmış. projede ihtiyaca istinaden okunabilecek dökümantasyonlardır.

### 📌📌 instruction

her chat'e mutlaka otomatik eklenir. örnek:
...
bu yazılımın kendisinde `LLM`'e benzer ufak ta olsa bir beyin olmak zorunda değil. Neleri desteklediğine bağlı değişir.
