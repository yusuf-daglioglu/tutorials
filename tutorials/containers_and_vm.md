...
`Virtual Machine Manager (⟷ virt-manager)` özel bir uygulama ismidir.
...
- `pod` içinde birçok process var ama bu process'lerin her biri ayrı `container`'larda çalıştırılıyor. bunları manage eden özel yapıya `pod` deniliyor.
  ...
  - fakat içinde birçok framework'ü barındıran, ve onlarla çok bütünleşik çalışan, kurulumu kompleks yazılımlar `system container` tabanlı olabilir. örnek:
    - `distrobox` içinde birden fazla process çalıştırabiliriz: https://github.com/89luca89/distrobox/blob/main/docs/useful_tips.md#using-init-system-inside-a-distrobox
...
yazılımlar şu yöntemlerle birbiri ile haberleşebilir:

- network soket üzerinden

  network cihazı yok ise sanal olan `loopback` gibi interface üzerinden.

- `IPC`

  `RAM`'den bellek paylaşma ve `IPC` kuyrukları ile mesajlaşmayı kapsamaktadır.
...
- process signal
  ...
  bir process diğer process'e büyük bilgi atamaz ama basit Signal'ler yollayabilirler.

  `PID namepaces`'e dayanır.

Yukarıdakilerin tümü `pod` içerisinde paylaştırılabiliyor.
...
- `container` içinde `SSH` server açabilmek için. `kaynak: (source-id: 243) 2inci paragraf.`

  burada `pod` çözümünün yetmesi için tüm namespace'leri `pod` içindeki tüm `container`'lara açmak gerekir.

- `crond`

  `pod` çözümü yeterli. tüm `container` volume'unu paylaşmak yeterli.
...
- eski bir uygulamayı container'da çalıştırmak istediğimizde, onun dependency'lerini de çalıştırmak zorunda kalabiliriz. bunun case'i ne olabilir bilmiyorum.

  burada şu da olabilir: `distrobox` gibi araçlarda tüm process'ler kapalı olabilir.
