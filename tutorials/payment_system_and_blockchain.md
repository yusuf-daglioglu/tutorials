...
- oylama sistemi. herkesin oyları digital imzalı şekilde merkezi olmayan bir sistemden görülebilir olacak. üstüste kayıtlar olduğundan dolayı, kimse eski kayıtları değiştiremeyecek. bu şekilde kimse oy çalamayacak.
...
- noter işlemleri. iki taraf karşılıklı onayladığı dökümanları blockchain'e yollar. artık ikisinin onayladığı gerçeği kimse tarafından değiştirilemez. eğer bu sistem resmi olarak devlet tarafından tanınırsa, notere gerek kalmaz.
...
her 1 input, 1 adet source address'tir. yani Bitcoin'in hangi addresten çekileceğidir.

her 1 output ise, Bitocin'in transfer edileceği adrestir.

source adreslerdeki tüm para harcanmak zorundadır. eğer source'taki paranın bir kısmını harcayacak isek, o zaman output listesinde, kendi source adresimizi de bulundurmak zorundayız.

bir output eğer hiçbir farklı yere input olarak eklenmediyse, __unspent transaction outputs (or UTXO)__ olarak kalır.

### 📌📌 UTXO-based vs Account-based

2'side blockchain tabanlı veritabanı üzerinde tutulan mantıksal modellerdir.
...
UTXO veritabanında, address'ler tamamen sanaldır. Herhangi bir UTXO'da olan addrese ait bir private key'imiz var ise, o private key'i, son kullanıcı, hesapmış gibi algılamaktadır.
...
Bitcoin'de, tüm transaction (input ve output'lar) blockchain'e kaydedilir. Böylece history, sürekli valide edilebilmiş kalır. Fakat son kullanıcı işlem yapmak istediğinde, son kullanıcının adresini tüm blockian'deki output'larda aramak gerekir. Sonrada o output'un UTXO mi; yani harcanmış olup olmadığınıda, tüm diğer input'ları kontrol ederek valide etmek gerekir. Bu işlem aşırı uzun süreceğinden, Bitcoin mine eden node'lar, ayrı bir indexed-veritabanında sadece UTXO'leri tutar. Burada ikincil veritabanlarının nasıl çalışacağı ve hangi veritabanı olacağı gibi konular, tamamen Bitcoin node'unun implementasyonuna bırakılmıştır. remsi bir yöntem yoktur.
...
(Aşağıdaki Solidity örnek kodu, Javascript syntax'ı ile renklendirilmiştir)
...
contract ExampleDapp {
    ...
    string dapp_name; // state variable (or storage variable) --> these are expensive. because they are sotred in blockchain.
    ...
    // "constant" keyward wa using on older versions of solidity.
...
// interact with Enthereum nodes (servers) via "window.ethereum" object's API.

// Current user may have defined it's wallet inside Metamask extension on the current web browser.

// when we call "request" function from JS, the MetaMask extension will show a popup to end-user and it will ask for permisson to show the account to the current web page.
...
Web3.js Javascript kütüphanesi eklentisiz şekilde Ethereum sunucusuna istek atabilmemizi sağlıyor:
