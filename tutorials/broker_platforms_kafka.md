...
- # partition
    ...
    Kafka does not store which client/consumer readed which data. Therefore each client should remember it's own offset (topic-offset ve partition-ID) he read last. But kafka has another feature: It stores the last readed record (offset) for each "__consumer group__". Therefore, as a client, if you dont have a spesific reason, you don't have to manage offsset.
...
- # offset
    ...
    Yeni partition'lar 0 offfset'i ile yaratılır. Fakat birçok mesaj geldikten sonra, o partititon'daki eski mesajlar silinirse, o partition'un first offset'i 0 olmaz. silinen mesajların sayısı kadar olur. 30 mesaj silindiyse, first-offset 30 olur. çünkü ilk index 30 uncu record'a reference etmektedir.
...
- # acknowledgment (or ACK)
  ...
  - 0: client isteği kafkaya atar. isteğin bir yere ulaşıp ulaşmadığı ile hiç ilgilenmez.
  - 1: client sadece kafka-leader'ın isteği aldığından emin olana kadar bekler.
  - all: o partition için tüm diğer in-sync replica kafka nodelarının bu record'u aldığından emin olana kadar bekler.
