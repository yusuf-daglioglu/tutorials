...
Consumer'lar sadece spesifik 1 partition ve topic'ten de çekebilir:
...
Consumer kafka'ya mesajı işlediğine dair mesaj atabilir. bu mesaj standarttır ve farklı tipi yoktur. fakat client kendi içinde kafka node'una ne zaman döneceğini kendi belirler:
...
- At Most Once: spring'de ENABLE_AUTO_COMMIT_CONFIG ayarını true yaparsak mesaj işlendikten sonra Spring otomatik olarak Kafka'ya "commited" sinyalini yollar (burada Spring, "listen" metodumuz hata fırlatsa dahi, "commited" mesajını Kafka'ya yollar. yoksa kafka sürekli aynı mesajı atacak.)
...
"Partition ID" and topics are both unique together. In other words, "user-created-topic" has partition ID "1", but "warnings-topic" may also have partition ID "1". Both these partitions are completely different and independent.

Topics are logical concepts, partition are physically divided portions.
...
Kafka does not store which client/consumer has read which data. Therefore each client should remember it's own offset (topic-offset ve partition-ID) he read last. But kafka has another feature (__which is optional__): Kafka stores the last read record (offset) for each "__consumer group__". Therefore, as a client, if you don't have a specific reason, you don't have to manage offset.
...
- 1 topic'ten birden fazla consumer-grup birbirlerinden bağımsız şekilde, bağımsız offset'lerle, aynı topic'ten mesaj okuyabilir.
- kafka consumer grup bilgisini server'da tutarken şu şekilde tutar:

  - topic:X, partition:1, last-read-offset:99
  - topic:X, partition:2, last-read-offset:88
  - topic:Y, partition:1, last-read-offset:77
...
Yeni partition'lar 0 offset'i ile yaratılır. Fakat birçok mesaj geldikten sonra, o partition'daki eski mesajlar silinirse, o partition'un first offset'i 0 olmaz. silinen mesajların sayısı kadar olur. 30 mesaj silindiyse, first-offset 30 olur. çünkü ilk index 30 uncu record'a reference etmektedir.
...
- 0 (at most once): client isteği Kafka'ya atar. isteğin bir yere ulaşıp ulaşmadığı ile hiç ilgilenmez.
- 1 (at least once): client sadece kafka-leader'ın isteği aldığından emin olana kadar bekler.
- -1 (all): o partition için tüm diğer in-sync replica kafka node'larının bu record'u aldığından emin olana kadar bekler.
...
Fakat Kafka arka planda eski data'ları ilgili partition'lara move etmez. Kafka'dan consume edilen mesajlar zaten consumer tarafından işlenmiştir. Eski mesajlar özellikle okunmuyor ise, eski data'ları move etmeye ihtiyacımız yok demektir.
...
Kafka default'ta (tek başına) multi DC hizmeti mevcut değil. Bunun için Kafka sunucusundan bağımsız yazılımlardan yararlanmak gerekiyor. Bunlardan biri de MirrorMaker.
...
- __Source connector__
  ...
  Kafka connect, değişiklikleri okuyacağı sistemin connector'üne ihtiyaç duyar. örnek "MySQL connector jar" dosyasını path'e eklemek zorundayız. Yukarıda "Debezium" gibi aracı uygulamalar kullanmak durumundayız.
