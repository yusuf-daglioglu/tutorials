...
`tüzel kişiliktir (⟷ juridical person)`.

## 📌 nominal değer (⟷ stated value ⟷ eng:nominal value ⟷ par Value ⟷ face Value)

örneğin bir arazinin nominal değeri `100.000 TL`'dir. oysa `gerçek (⟷ reel)` değeri bundan daha az yada çok olabilir.

## 📌 reasürans (⟷ reassurance ⟷ reinsurance)
...
## 📌 İkili Sigorta (⟷ Müşterek Sigorta ⟷ Coinsurance)

mşterinin birden fazla sigorta firması ile anlaştığı durumdur.
...
`döviz işlemi (⟷ kambiyo)` ile diğer ülkelerin para birimleri arası çevirme işlemidir. `Kambiyo` aslında, herhangi bir `kambiyo senedi`nin veya paranın, para veya başka bir `kambiyo senedi` ile değiştirilme işlemidir.
...
## 📌 bid-ask spread ( ⟷ buy-sell spread ⟷ alış satış farkı)

halk ağzıyla `makas` olarak bilinen şey.

- `Alış (⟷ Bid)` (`enstrüman`ı alım fiyatı)
- `satış (⟷ Ask ⟷ Offer)` (`enstrüman`ı satma fiyatı)
...
`policy` terimi politikadan gelmektedir. `gizlilik politikası (⟷ privacy policy)` gibi terimlerde kullanılır.

Aşağıda açıklanmaya çalışılan belge, `Türkçe`'den `İngilizce` çevirilerde dahi `policy` olarak geçse de, `Türkiye` dışında `policy` olarak kullanılmaz. Aşağıdaki açıklanan belge `Türkiye` dışında `bill of exchange` olarak kullanılır:
...
- `lehtar (⟷ beneficiary)` - `senet`te paranın ödeneceği kişidir. Birçok kaynakta `lehdar` olarak geçer fakat `TDK`'da `d` harfi ile bir kelime yoktur.
...
`hisse senedi` aracı kuruluşlardadır. aracı kuruluşlar, bir müşterinin `hisse senedi`ni diğer müşteriye kiralayabilir ödünç verebilir. bu durumda; kiralayan müşteri günlük kira parası alır.
...
- borsada örneğin `Dolar` satın alacağız. aldığımız kur onaylandığında onay sayfasına gideriz. onay sayfasında belli bir süre tölerans tanınır. örneğin 10 dakika belirlediğimiz kurdan belirlediğimiz miktar `Dolar`'ı satın alabiliriz. burada belli adet `Dolar` miktarını belli bir kurdan `rezerv` etmiş oluruz.
...
`kart kuruluşu` (örnek `VISA`), `3DS` işlem sonucundan haberdar değillerdir. Bu sebeple `ACS`, asenkron olarak işlem sonucunu `AHS`'a yollar. Bu mesaj `PATransReq` adı verilir. `Kart kuruluşu`, `AHS`'lerden yararlanırlar. `kaynak: https://medium.com/baybaynakit/3d-secure-2-0a-yak%C4%B1ndan-bir-bak%C4%B1%C5%9F-bcd777c38c04`
...
`Modirum` firması aynı zamanda `ACS` ve `DS` yazılımı da geliştirmektedir. `kaynak: https://www.modirum.com/products/ Authentication History Server (AHS) ile başlayan paragraf.`
...
aşağıdaki çoğu bilgilerin birçoğu buradan alınmıştır: https://www.modirum.com/3dsecure/
...
Takas komisyonu yüzdeleri `Türkiye`'de `BKM` ve rekabet kurulu tarafından belirlenir. farklı ülkelerde farklı otoriteler bu görevi üstlenebilir. `kaynak: https://bkm.com.tr/faydali-bilgiler/komisyon-ve-ucretler/bankalararasi-takas-komisyonlari/ aynı sayfanın İngilizcesi: https://bkm.com.tr/en/useful-information/commissions-and-fees/interbank-interchange-commissions/`
...
## 📌 Taksit (⟷ installment ⟷ instalment ⟷ hire purchase)
...
para transferi anlamına gelir. wire transfer ve virman; yani tüm transferi kapsamaktadır. bankaya verilen para transferi talimatı anlamında kullanılır.
...
`MSU` kullanan `merchant`', `VPOS`'u bankadan kendi almış ve `MSU` web panelinde bu bilgileri girmiş olması şarttır.
...
| `BKM Ekspress`                                                   | `BKM`               |
| `MSU`                                                            | `Payten`            |
| `Google Pay (⟷ old-name:Pay with Google ⟷ old-name:Android Pay)` | `Google`            |
| `Apple Pay`                                                      | `Apple`             |
| `BKM Express`                                                    | `BKM`               |
| `Samsung Pay`                                                    | `Samsung`           |
| `Microsoft Wallet`                                               | `Microsoft`         |

Online cüzdanlar kart bilgilerini kendi sunucularında tutar ve bunların yönetimi için GUI sağlarlar. Ödeme yapılmak istendiğinde eğer son kullanıcı online cüzdan seçerse, online cüzdan sunucusundan kartını seçer. kart seçilince online cüzdan sitesi, bu kart için bir token `merchant`'ne döner. `merchant` de normal ödeme sürecini başlatır. normal ödemede `kart no`, `ccv` gibi bilgiler `payment gateway`'e (veya `payment processor`'e) yollanırken, burada token yollanması yeterlidir. eğer kullanılan `payment gateway` (veya `payment processor`) token tipini destekliyorsa gidip bu token'a karşılık gelen kart bilgilerini üçüncü parti `Token Service Provider (⟷ TSP)`'dan alacaktır.
...
- `VISA Checkout` `merchant` isterse direk kart bilgilerini de döndürebilirken, isterse token'da döndürebiliyor. her 2 seçeneği de sunuyor.
...
- __Pass-through Wallet__
  ...
  `merchant`'ne token dönen servistir.

- __Staged Wallet__

  ödeme işlemini tamamlayan sistemlerdir. `merchant` açısından alternatif ödeme gibi gözükse de (çünkü `merchant` sitesinde kart girdirtmiyor), büyük resimden bakıldığında; kart bilgisini `staged wallet`'ın kendisi istiyor. bu sebeple alternatif ödeme yöntemi değildir.
...
`ISO` standartlarında numerik değerlerdir. örneğin hava yolları firması, otel firması bir işlem yaptığında `MCC` değerini ödeme kuruluşuna yollar. bunun birçok sebebi vardır: yasal kontroller, istatistik/analiz toplama, kampanya koşulları doğrultusunda hizmet veya servis veya indirim sunabilme gibi.

## 📌 RFC 4112

`Electronic Commerce Modeling Language (⟷ ECML)`'dir.
...
buradan detaylı field'lar incelenebilir: https://datatracker.ietf.org/doc/html/rfc4112.html
...
Banka ile `Kart Kuruluşu` arasındaki haberleşme protocol'üdür.

`nestpay` bankaların `VPOS`'u olduğu için, `Kart Kuruluşu` firmaları `nestpay` ile haberleşir. bu sebeple `ISO 8583` protocol'ünü nestpay yazılımı da kullanılır.
...
- `Bitmap`
  ...
  - `0x23` değerinin bit karşılığı: `0010 0011`. yani; 11, 15 ve 16ıncı alanlar dolu.

  - `0x00` = `0000 0000`. yani; Burada dolu alan yok.

  - `0x41` = `0100 0001`. yani; 26 ve 32. alanlar dolu.
...
- Data Elements
  ...
  - Field 3: `Processing Code`
    ...
    işlemin ne işlemi olduğunu buradan anlarız. Örneğin bakiye sorgulama için farklı, para çekme için farklıdır. Bankaya gelen mesajlar bu alana bakılıp adreslenir, işlemler yapılır ve işleme özel mesajlar üretilir. 6 haneli numeric alandır.

  - Field 4: Transaction Amount
  
    işlem hangi ülkenin para birimiyle yapıldıysa bu alana yazılır. Numeric 12 haneli alandır.

  - Field 6: `Card holder` Billing
  
    `card holder`'ın kuruyla hesaplanmış tutardır. Örneğin 5 `Dolar`'lık işlem için `19,5` `TL` (Kur `3.9`) olarak yani `000000001950` değeri set edilir. Numeric `12` haneli alandır.

  - Field 7: Transmission date & time
  
    işlemi başlatan tarafın işlemi başlattığı anın olduğu alandır. MMDDhhmmss formatındadır.

  - Field 11: `STAN`
  
    işlemi trace edebilmek için verilen numaradır. Numeric 6 hanelidir.

  - Field 12: Time
   
    `hhmmss` formatında işlemin saatidir.

  - Field 13: Date
  
    `MMDD` formatında işlemin tarihidir.

  - Field 18: `Merchant` Type
  
   `mcc` kodudur. Numeric 4 hanelidir.

  - Field 22: `POS` Entry Mode
  
    Numerik 3 hanelidir. İlk 2 hanede işlemin nasıl yapıldığını son hanede de şifre kullanılıp kullanılmadığını alır.

    05: Çipli kartla işlem
  ...
  - Field 43: Card Acceptor Name/Location, işlemin yapıldığı terminalin yerinin olduğu alandır. ilk 23 hanesi adresi, 24 ile 36 karakterler arası şehri, 37 ve 38inci karakterler eyalet kodunu, 39 ve 40ıncı karakter ülke kodunu barındırır. 40 karakterli alfanümeriktir.
  ...
  - Field 54: Additional Amount ek tutar alanıdır. İşleme özel değer alır. Örneğin başka `ATM`'den bakiye sorgulamada `kartın bankası`ndan müşterinin bakiyesine bu alana basması beklenir.

  - Field 55: `ICC` Data, kart çipli ise `EMV` bilgisi bu alana basılır.
...
`Türkiye`'de devlete bağlı resmi kurumdur.

## 📌 Kişisel Verileri Koruma Kurumu (⟷ KVKK)

`Türkiye`'de resmi kanun çıkarmakla yükümlü kurumdur. Her ülkede farklı kurumlar tarafından benzer kanunlar çıkarılır. örneğin; `GDPR (⟷ General Data Protection Regulation)` `Avrupa Birliği` hukukunda; tüm `Avrupa Birliği` ve `Avrupa Ekonomik Alanı` içerisinde yer alan bireyler için veri koruma ve gizliliğine ilişkin bir yönetmeliktir. `GDPR`, `avrupa birliği` dışına çıkarılan data'lar hakkında kanunlar içermektedir.
...
- DB tasarlandığında özellikle ilişkileri bozmayacak şekilde silme mümkün olmalı
- DB backup'ları içerisinden de gerektiğinde data silinebilmelidir (bugün hassas olmayan data 5 yıl sonra hassas olduğu kanunlaştığında, o bilgiyi backup'lardan silebilmeliyiz)
- log'lardaki çıktılar pek kolay silinemediği için, ileride hassas olabilecek bir data kesinlikle log'lanmamalıdır.

## 📌 OTP (⟷ One Time Password ⟷ Tek Kullanımlık Şifre ⟷ TKŞ)

İşlemlerin doğrulanması için tek seferlik kullanım amacıyla üretilen ve/veya gönderilen şifrelerdir. `3DS` sürecinde bankadan cep telefonunuza gelen şifre veya internet bankacılığı kullanıcı girişlerinde üretilen anahtar şifreler bu uygulamaya örnektir.
...
direk olarak bankalar arası iletişim/mesajlaşma protocol'üdür. bu protocol sayesinde bankalar birbirlerine mesaj atabilir. mesaj atarken birbirleri arasında elektronik para transfer ederler. elektronik paradan kasıt, sanal ortamda birbirlerine para transfer ettiklerini onaylarlar. fiziksel olarak paranın ellerine geçmesini beklemelerine gerek kalmaz. fiziksel para aracılar tarafından kendilerine ulaştırılacaktır. `Swift` standartları tüm bankalarca ortak bir kuruluş altında belirlenmektedir. her banka bu ortak kuruluşa üye olmak için belli bir ücret vermektedir. `Swift`'in sunucuları da vardır. bu sunucular `Swift` protocol'leri ile banaklar haberleşirken bu mesajları denetler/log'lar/valide eder.
...
## 📌 BIC kodları (⟷ Bank Identifier Codes ⟷ SWIFT Kodları)
...
`Sanal kart`, gerçek bir karta bağlıdır. Ödeme yapılan iş yerine kart bilgisi yerine `sanal kart` bilgisi verilir. `Sanal kart` gerçek kart üzerinden işlemi yapar. Sadece verilen `kart no`, son kullanma tarihi gerçek değildir.

fiziksel olarak var olmadığından, sadece temazsız işlemlerde (telefon, online gibi) işlemlerde kullanılabilir.

`Sanal kart`lar çabuk açılıp kaldırılabilir. Her işlem için bile farklı kart kullanılabilir.
...
birbirlerine alternatif, birer alternatif ödeme sistemledir. bu sistemler her bankayla ayrı ayrı entegrasyon sağlamışlardır. ödeme sayfasında, son kullanıcı bankanın ismini seçer. bankasını seçtikten ve o bankaya login olduktan sonra ödeme işlemi tamamlanır. ödeme sistemi banka ile anlaşmalı olduğu için direk ödemeyi bankadan alır. daha sonra bankadan aldığı parayı, `merchant`'ne ay sonu (veya anlaşılan süre içinde) yollar.

`Paypal`'dan farklı çalışmaktadırlar.

## 📌 Temassız Kartlar (⟷ Contactless Cards)

`POS` cihazının da uyumlu olması gerekir.

## 📌 FinTech

`Financial technology`'nin kısaltmasıdır.
...
günümüzde `PIN` istenen kartlarda slip üzerinde imza kısmı çıkmamaktadır. çünkü yasal olarak artık gerek yoktur.
