...
Eski bir makalede yayımlanmış standartın ismidir. Makalede her permission, role, subject gibi terimlerin karşılığı nettir. Fakat piyasada birçok yazılım RBAC destekliyoruz demesine rağmen bu isimlendirmeyi veya standartı kullanmaz. çünkü RBAC özel isim olarak kullanılmayabilir (çünkü genel bir ifadedir).
...
`Public Key Pinning` 2 şekilde yapılabilir:

- `TOFU` (web browser'larda bu teknolojiye `HPKP` ismi verildi.)

- development aşamasında client'a `public key` gömülür.
...
Bu listeye kaydolmak ücretsiz. bu bilgileri web tarayıcıları sürekli local'de güncel tutuyor ve bu listedeki sitelere gitmek isteyen kullanıcıları direk HTTPS'e yönlendiriyor.
