...
HCL (HashiCorp Configuration Language kısaltmasıdır) formatındadır. HCL, JSON, YML'ye alternatif olarak geliştirilmiştir. örnek syntax:
...
Aynı protokolün farklı sürümleridir ama isim değişikliği yaşamıştır.
...
`mTLS` bir protokol değil. Genel bir pattern'dir. Yani nasıl yapılacağı ile ilgili bilgi içermez.
...
## 📌 Zero trust architecture (or ZTA or perimeterless security)
...
- API gateway gelen `JWT`'yi açıp, internal'da açık yollamamalı. Her mikroservis kendi `JWT`'yi açmalı.
- internal'da her mikroservis `TLS` ile haberleşmeli.
- `mTLS`
