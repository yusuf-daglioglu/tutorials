...
Enumeration<InetAddress> nifAddresses = nif.getInetAddresses(); // NI'ın IP 4 ve IP 6 adresleri array olarak dönmektedir.

Socket socket = new Socket(); //bu satır değişmedi
socket.bind(new InetSocketAddress(nifAddresses.nextElement(), 0)); // IP 4 adresi ile soketimizi bu NI'a bind etmiş oluruz. bu kısım olmazsa OS routing table'lara bakarak bizi uygun NI'ye attach edecekti.
...
ağ üzerinde güvenli veri iletimi sağlamak için geliştirilmiş protocol.
