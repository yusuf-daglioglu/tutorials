...
`east-west traffic`, aynı trust-boundary tanımında olan app'lerin birbiri arasındaki trafiği kapsar.

`north-south traffic`, 3rd party (veya partner firmanın network2ündeki) bir app'e atılan isteklerdir.

trust-boundry'nin ne olacağı tamamen sistem admin'lere kalmış. Örneğin bir şirket içinde `OpenShift` dışındaki her app, `OpenShift` içindeki her app için farklı bir trust-boundary'de olup olmadığı sistem admin'lerin kararına (fikrine) kalmış.

fiziksel data center'ın ayrı olması traffic type'ını belirlemez.
