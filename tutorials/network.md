...
Bir LAN'da hem IPv6 hem de IPv4 birlikte çalışabilir. Fakat böyle bir durumda o LAN'da iki farklı birbirinden bağımsız network olmuş olur.
...
- __Bandwidth (or bant genişliği)__: belli bir sürede platformun (kablonun, diskin...) taşıyabileceği maximum data miktarıdır.

- __Throughput (or işlem hacmi)__: belli bir süre içinde geçen data'nın ortalamasıdır.

  belli bir süre içinde geçen en yüksek Throughput değerine "maximum Throughput" denir.

- __latency (or gecikme)__: bir data'nın bir noktadan diğer noktaya taşınma süresidir.
...
## 📌 Captive Portal (or Kısıtlama Portali)
...
network arayüzüne denk gelen fiziksel kart.
...
örnek: A arayüzinin sub-arayüzleri olabilir. A'ya gelen istekler, hem A hem de onun sub'larına klonlanarak dağıtılır.
...
Ikisi de birer mail sunucu ile haberleşme protokolleridir. imap, pop3'ten daha gelişmiştir.
...
mail sunucusu diğer email sunucularıyla haberleşen yazılımdır. tabi client'lar ile de haberleşir. mailleri client'lardan alır ve kaşı tarafa email atar. bunları backup'ını da tanımlı veritabanında saklar. fakat saklamak zorunda değildir.
...
IANA ortak bir veritabanı tutmaktadır: <https://www.iana.org/assignments/media-types/media-types.xhtml>
...
Bazı kaynaklarda "classic telephone system (or klasik telefon sistemi)" olarak da isimlendirilmektedir.
...
Elektronik cihazların internet ağı üzerinde birbirini otomatik algılaması için geliştirilen bir protokoldür. Ağ'ı yöneten router'ın UPnP desteği olması ve çalışan yazılımın (elektronik cihaz içerisindeki) UPnP support'u olması şarttır.
...
- __message authentication code (or mac or authentication tag)__ kriptografide kullanılan, mesajın hash değeridir. bu şekilde mesajın değişip değişmediği doğrulanabilir. MAC değeri key ile imzalandığı için aynı zamanda, mesajın istenilen alıcıdan geldiği de doğrulanmış olur.
...
SSH yapısı ile tamamiyle bütünleşik FTP protokolüdür. arka planda kendisi SSH bağlantısı açıp, bağımsız olarak o bağlantı üzerinden FTP ile dosya yollama işlemi yapar.
...
Web sayfalarının yayınlanmasını ve web uygulamalarının çalışmasını sağlayan, istemcilerden HTTP ve FTP üzerinden gelen talepleri Microsoft Windows sunucu tabanlı OS'larda karşılayan birimdir.
